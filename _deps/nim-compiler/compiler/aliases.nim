...
import ast, astalgo, types, trees, intsets

when defined(nimPreviewSlimSystem):
  import std/assertions

type TAnalysisResult* = enum
  arNo
  arMaybe
  arYes
  ...
  of nkRecCase:
    ...
    for i in 1 ..< n.len:
      ...
      of nkOfBranch, nkElse:
        ...
        discard "isPartOfAux(record case branch)"
  of nkSym:
...
proc isPartOfAux(a, b: PType, marker: var IntSet): TAnalysisResult =
  ...
  if a == nil or b == nil:
    return
  if containsOrIncl(marker, a.id):
    return
  if compareTypes(a, b, dcEqIgnoreDistinct):
    return arYes
  case a.kind
  of tyObject:
    if a[0] != nil:
    ...
    if result == arNo:
      result = isPartOfAux(a.n, b, marker)
    ...
    of nkBracketExpr:
      ...
      if a.len >= 2 and b.len >= 2:
        ...
        if result == arYes and isDeepConstExpr(a[1]) and isDeepConstExpr(b[1]):
          ...
          # if they are
          var x =
            if a[1].kind == nkHiddenStdConv:
              a[1][1]
            else:
              a[1]
          var y =
            if b[1].kind == nkHiddenStdConv:
              b[1][1]
            else:
              b[1]
