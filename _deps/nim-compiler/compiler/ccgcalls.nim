...
proc preventNrvo(p: BProc, dest, le, ri: PNode): bool =
  ...
  if le != nil:
    ...
    if canRaise(ri[0]) and locationEscapes(p, le, p.nestedTryStmts.len > 0):
...
proc fixupCall(p: BProc, le, ri: PNode, d: var TLoc, callee, params: Rope) =
...
proc genOpenArraySlice(
    p: BProc, q: PNode, formalType, destType: PType, prepareForMutation = false
): (Rope, Rope) =
  var a, b, c: TLoc
  initLocExpr(p, q[1], a)
  initLocExpr(p, q[2], b)
  ...
  of tyArray:
    ...
    if first == 0:
      result = ("($3*)(($1)+($2))" % [rdLoc(a), rdLoc(b), dest], lengthExpr)
    else:
      var lit = newRopeAppender()
      intLiteral(first, lit)
      result = ("($4*)($1)+(($2)-($3))" % [rdLoc(a), rdLoc(b), lit, dest], lengthExpr)
  of tyOpenArray, tyVarargs:
    if reifiedOpenArray(q[1]):
      result = ("($3*)($1.Field0)+($2)" % [rdLoc(a), rdLoc(b), dest], lengthExpr)
    else:
      result = ("($3*)($1)+($2)" % [rdLoc(a), rdLoc(b), dest], lengthExpr)
    ...
    of tyString, tySequence:
      ...
      if ntyp.kind in {tyVar} and not compileToCpp(p.module):
        ...
        result.add "($4) ? ((*$1)$3) : NIM_NIL, $2" %
          [a.rdLoc, lenExpr(p, t), dataField(p), dataFieldAccessor(p, "*" & a.rdLoc)]
      else:
        result.add "($4) ? ($1$3) : NIM_NIL, $2" %
...
proc withTmpIfNeeded(p: BProc, a: TLoc, needsTmp: bool): TLoc =
  ...
  if needsTmp and a.lode.typ != nil and
      p.config.selectedGC in {gcArc, gcAtomicArc, gcOrc} and
...
proc genArgStringToCString(
    p: BProc, n: PNode, result: var Rope, needsTmp: bool
) {.inline.} =
  var a: TLoc
  initLocExpr(p, n[0], a)
  appcg(
    p.module, result, "#nimToCStringConv($1)", [withTmpIfNeeded(p, a, needsTmp).rdLoc]
  )

proc genArg(
    p: BProc, n: PNode, param: PSym, call: PNode, result: var Rope, needsTmp = false
) =
  var a: TLoc
  ...
  elif p.module.compileToCpp and param.typ.kind in {tyVar} and n.kind == nkHiddenAddr:
...
proc genThisArg(p: BProc, ri: PNode, i: int, typ: PType, result: var Rope) =
  ...
  # skip the deref:
  var ri = ri[i]
  while ri.kind == nkObjDownConv:
    ri = ri[0]
  let t = typ[i].skipTypes({tyGenericInst, tyAlias, tySink})
  if t.kind in {tyVar}:
    let x =
      if ri.kind == nkHiddenAddr:
        ri[0]
      else:
        ri
    if x.typ.kind == tyPtr:
  ...
  while i < pat.len:
    ...
    of '#':
      if i + 1 < pat.len and pat[i + 1] in {'+', '@'}:
        ...
        while arg.kind in {nkAddr, nkHiddenAddr, nkObjDownConv}:
          arg = arg[0]
    ...
    of '\'':
      ...
      if scanCppGenericSlot(pat, i, idx, stars):
        ...
        if t == nil:
          ...
          result.add(getTypeDesc(p.module, t))
    else:
      let start = i
      while i < pat.len:
        if pat[i] notin {'@', '#', '\''}:
...
proc isInactiveDestructorCall(p: BProc, e: PNode): bool =
  ...
  code for 'return'. ]#
  result =
    e.len == 2 and e[0].kind == nkSym and e[0].sym.name.s == "=destroy" and
    notYetAlive(e[1].skipAddr)
