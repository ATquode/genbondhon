...
type TTraversalClosure = object
  p: BProc
  visitorFrmt: string

const visitorFrmt = "#nimGCvisit((void*)$1, $2);$n"

proc genTraverseProc(c: TTraversalClosure, accessor: Rope, typ: PType)
proc genCaseRange(p: BProc, branch: PNode)
proc getTemp(p: BProc, t: PType, result: var TLoc, needsInit = false)

proc genTraverseProc(c: TTraversalClosure, accessor: Rope, n: PNode, typ: PType) =
  if n == nil:
    return
  case n.kind
  of nkRecList:
  ...
  of nkRecCase:
    ...
    let disc = n[0].sym
    if disc.loc.r == "":
      fillObjectFields(c.p.module, typ)
    if disc.loc.t == nil:
      internalError(c.p.config, n.info, "genTraverseProc()")
...
proc genTraverseProc(c: TTraversalClosure, accessor: Rope, typ: PType) =
  ...
  var p = c.p
  case typ.kind
  of tyGenericInst, tyGenericBody, tyTypeDesc, tyAlias, tyDistinct, tyInferred, tySink,
      tyOwned:
  ...
  of tyArray:
    ...
    freeze oldCode
    linefmt(p, cpsStmts, "for ($1 = 0; $1 < $2; $1++) {$n", [i.r, arraySize])
  ...
  of tyObject:
    ...
    if typ.n != nil:
      genTraverseProc(c, accessor, typ.n, typ)
  of tyTuple:
  ...
  of tyProc:
    if typ.callConv == ccClosure:
      lineCg(
        ...
        cpsStmts,
        visitorFrmt,
        [ropecg(c.p.module, "$1.ClE_0", [accessor]), c.visitorFrmt],
      )
  else:
    discard
...
proc genTraverseProcSeq(c: TTraversalClosure, accessor: Rope, typ: PType) =
  ...
  lineF(p, cpsStmts, "for ($1 = 0; $1 < $2; $1++) {$n", [i.r, lenExpr(c.p, a)])
  let oldLen = p.s(cpsStmts).len
...
proc genTraverseProc(m: BModule, origTyp: PType, sig: SigHash): Rope =
  ...
  if hcrOn:
    ...
    m.s[cfsDynLibInit].addf(
      "\t$1 = (N_NIMCALL_PTR(void, )(void*, NI)) hcrRegisterProc($3, \"$1\", (void*)$2);\n",
...
proc genTraverseProcForGlobal(m: BModule, s: PSym, info: TLineInfo): Rope =
  ...
  let generatedProc =
    "$1 {$n$2$3$4}$n" % [header, p.s(cpsLocals), p.s(cpsInit), p.s(cpsStmts)]
