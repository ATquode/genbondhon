...
type TypeDescKind = enum
  dkParam #skParam
  dkRefParam
    #param passed by ref when {.byref.} is used. Cpp only. C goes straight to dkParam and is handled as a regular pointer
  dkRefGenericParam
    #param passed by ref when {.byref.} is used that is also a generic. Cpp only. C goes straight to dkParam and is handled as a regular pointer
  dkVar #skVar
  dkField #skField
  dkResult #skResult
  dkConst #skConst
...
proc isKeyword(w: PIdent): bool =
  ...
  case w.id
  of ccgKeywordsLow .. ccgKeywordsHigh, nimKeywordsLow .. nimKeywordsHigh, ord(wInline):
    return true
  else:
    return false
...
proc mapType(conf: ConfigRef, typ: PType, isParam: bool): TCTypeKind =
  ...
  of tyGenericBody, tyGenericInst, tyGenericParam, tyDistinct, tyOrdinal, tyTypeDesc,
      tyAlias, tySink, tyInferred, tyOwned:
  ...
  of tyEnum:
    if firstOrd(conf, typ) < 0:
      ...
      of 8:
        ...
        result = ctInt32
  of tyRange:
    result = mapType(conf, typ[0], isParam)
  of tyPtr, tyVar, tyLent, tyRef:
    ...
    of tyOpenArray, tyArray, tyVarargs, tyUncheckedArray:
      result = ctPtrToArray
    of tySet:
      if mapSetType(conf, base) == ctArray:
        result = ctPtrToArray
      else:
        result = ctPtr
    else:
  ...
  of tySequence:
    result = ctNimSeq
  of tyProc:
    result = if typ.callConv != ccClosure: ctProc else: ctStruct
  of tyString:
    result = ctNimStr
  of tyCstring:
    result = ctCString
...
proc isImportedCppType(t: PType): bool =
  ...
  result =
    (t.sym != nil and sfInfixCall in t.sym.flags) or
...
proc getTypeDescAux(
  m: BModule, origTyp: PType, check: var IntSet, kind: TypeDescKind
): Rope

proc isObjLackingTypeField(typ: PType): bool {.inline.} =
  result =
    (typ.kind == tyObject) and
    ((tfFinal in typ.flags) and (typ[0] == nil) or isPureObject(typ))
...
proc isInvalidReturnType(conf: ConfigRef, typ: PType, isProc = true): bool =
  ...
  if rettype == nil or
      (isAllowedCall and getSize(conf, rettype) > conf.target.floatSize * 3):
    result = true
  else:
    case mapType(conf, rettype, false)
    of ctArray:
      result =
        not (skipTypes(rettype, typedescInst).kind in {tyVar, tyLent, tyRef, tyPtr})
    of ctStruct:
      let t = skipTypes(rettype, typedescInst)
      if rettype.isImportedCppType or t.isImportedCppType:
        return false
      result =
...
const CallingConvToStr: array[TCallingConvention, string] = [
  "N_NIMCALL",
  "N_STDCALL",
  "N_CDECL",
  "N_SAFECALL",
...
proc addAbiCheck(m: BModule, t: PType, name: Rope) =
  if isDefined(m.config, "checkAbi") and
      (let size = getSize(m.config, t); size != szUnknownSize):
    ...
    m.s[cfsTypeInfo].addf(
      "NIM_STATIC_ASSERT(sizeof($1) == $2, $3);$n", [name, rope(size), msg2.rope]
    )
...
proc getSimpleTypeDesc(m: BModule, typ: PType): Rope =
  const NumericalTypeToStr: array[tyInt .. tyUInt64, string] = [
    "NI", "NI8", "NI16", "NI32", "NI64", "NF", "NF32", "NF64", "NF128", "NU", "NU8",
    "NU16", "NU32", "NU64",
  ]
  case typ.kind
  of tyPointer:
  ...
  of tyCstring:
    result = typeNameOrLiteral(m, typ, "NCSTRING")
  of tyBool:
    result = typeNameOrLiteral(m, typ, "NIM_BOOL")
  of tyChar:
    result = typeNameOrLiteral(m, typ, "NIM_CHAR")
  of tyNil:
    result = typeNameOrLiteral(m, typ, "void*")
  of tyInt .. tyUInt64:
    result = typeNameOrLiteral(m, typ, NumericalTypeToStr[typ.kind])
  of tyDistinct, tyRange, tyOrdinal:
    result = getSimpleTypeDesc(m, typ[0])
  of tyStatic:
    if typ.n != nil:
...
proc structOrUnion(t: PType): Rope =
  ...
  if tfUnion in t.flags: cachedUnion else: cachedStruct
...
proc getTypeForward(m: BModule, typ: PType, sig: SigHash): Rope =
  ...
  of tySequence, tyTuple, tyObject:
    ...
    internalError(m.config, "getTypeForward(" & $typ.kind & ')')

proc getTypeDescWeak(
    m: BModule, t: PType, check: var IntSet, kind: TypeDescKind
): Rope =
...
macro unrollChars(x: static openArray[char], name, body: untyped) =
  result = newStmtList()
  for a in x:
    result.add(newBlockStmt(newStmtList(newConstStmt(name, newLit(a)), copy body)))

proc multiFormat*(
    frmt: var string, chars: static openArray[char], args: openArray[seq[string]]
) =
  var res: string
  unrollChars(chars, c):
    res = ""
...
proc genMemberProcParams(
    m: BModule,
    prc: PSym,
    superCall, rettype, params: var string,
    check: var IntSet,
    declareEnvironment = true,
    weakDep = false,
) =
  let t = prc.typ
  ...
  if isCtor:
    ...
    if rettype == "":
      ...
      rettype = runtimeFormat(
        rettype.replace("'0", "$1"), [getTypeDescAux(m, t[0], check, dkResult)]
      )
  var types, names, args: seq[string]
  if not isCtor:
    var this = t.n[1].sym
    fillParamName(m, this)
    fillLoc(this.loc, locParam, t.n[1], this.paramStorageLoc)
  ...
  for i in firstParam ..< t.n.len:
    if t.n[i].kind != nkSym:
      internalError(m.config, t.n.info, "genMemberProcParams")
    var param = t.n[i].sym
    ...
    fillLoc(param.loc, locParam, t.n[i], param.paramStorageLoc)
...
proc genProcParams(
  ...
  for i in 1 ..< t.n.len:
    if t.n[i].kind != nkSym:
      internalError(m.config, t.n.info, "genProcParams")
    var param = t.n[i].sym
    var descKind = dkParam
    ...
    while arr.kind in {tyOpenArray, tyVarargs}:
      ...
      if param.typ.kind in {tyVar, tyLent}:
        param.loc.storage = OnUnknown
...
proc genRecordFieldsAux(
  ...
  of nkRecCase:
    if n[0].kind != nkSym:
      internalError(m.config, n.info, "genRecordFieldsAux")
      ...
      of nkOfBranch, nkElse:
        ...
        if k.kind != nkSym:
          ...
          if a != "":
            ...
            if tfPacked in rectype.flags and
        ...
        internalError(m.config, "genRecordFieldsAux(record case branch)")
    if unionBody != "":
      result.addf("union{\n$1};$n", [unionBody])
  of nkSym:
    let field = n.sym
    if field.typ.kind == tyVoid:
      return
    ...
    if not isImportedCppType(rectype):
      ...
      if fieldType.kind == tyUncheckedArray:
        ...
        result.addf(
          "\t$1$3 $2;$n",
          [getTypeDescWeak(m, field.loc.t, check, dkField), sname, noAlias],
        )
      elif field.bitsize != 0:
        result.addf(
          "\t$1$4 $2:$3;$n",
          [
            getTypeDescAux(m, field.loc.t, check, dkField),
            sname,
            rope($field.bitsize),
            noAlias,
          ],
        ...
        if fieldType.isOrHasImportedCppType():
          result.addf(
            "\t$1$3 $2{};$n",
            [getTypeDescAux(m, field.loc.t, check, dkField), sname, noAlias],
          )
        else:
          result.addf(
            "\t$1$3 $2;$n",
...
proc genMemberProcHeader(
  m: BModule, prc: PSym, result: var Rope, asPtr: bool = false, isFwdDecl: bool = false
)
...
proc getRecordDescAux(
    m: BModule, typ: PType, name, baseType: Rope, check: var IntSet, hasField: var bool
): Rope =
  if typ.kind == tyObject:
    if typ[0] == nil:
      ...
      if typ.isException and m.config.exc == excCpp:
        when false:
          appcg(m, result, "virtual void raise() { throw *this; }$n", [])
          ...
          if typ.sym.magic == mException:
            ...
            appcg(
              m,
              cfsProcs,
              "inline $1::~$1() {if(this->raiseId) #popCurrentExceptionEx(this->raiseId);}$n",
              [name],
            )
      hasField = true
    else:
...
proc getRecordDesc(m: BModule, typ: PType, name: Rope, check: var IntSet): Rope =
  # declare the record:
  var hasField = false
...
proc getTupleDesc(m: BModule, typ: PType, name: Rope, check: var IntSet): Rope =
  result = "$1 $2 {$n" % [structOrUnion(typ), name]
  var desc: Rope = ""
  for i in 0 ..< typ.len:
    desc.addf("$1 Field$2;$n", [getTypeDescAux(m, typ[i], check, dkField), rope(i)])
  if desc == "":
    result.add("char dummy;\L")
  else:
    result.add(desc)
  result.add("};\L")
...
proc getOpenArrayDesc(
  ...
  if kind == dkParam:
    ...
    if result == "":
      ...
      m.s[cfsTypes].addf(
        "typedef struct {$n$2* Field0;$nNI Field1;$n} $1;$n", [result, elemType]
      )
...
proc getTypeDescAux(
  ...
  if t.sym != nil:
    useHeader(m, t.sym)
  if t != origTyp and origTyp.sym != nil:
    useHeader(m, origTyp.sym)
  ...
  of tyRef, tyPtr, tyVar, tyLent:
    var star =
      if t.kind in {tyVar} and tfVarIsPtr notin origTyp.flags and compileToCpp(m):
        "&"
      else:
        "*"
    var et = origTyp.skipTypes(abstractInst).lastSon
    var etB = et.skipTypes(abstractInst)
    if mapType(m.config, t, kind == dkParam) == ctPtrToArray and
        (etB.kind != tyOpenArray or kind == dkParam):
      if etB.kind == tySet:
  ...
  of tyEnum:
    ...
    if result == "":
      ...
      if not (
        isImportedCppType(t) or (sfImportc in t.sym.flags and t.sym.magic == mNone)
      ):
        m.typeCache[sig] = result
        var size: int
        if firstOrd(m.config, t) < 0:
          ...
          case size
          of 1:
            m.s[cfsTypes].addf("typedef NU8 $1;$n", [result])
          of 2:
            m.s[cfsTypes].addf("typedef NU16 $1;$n", [result])
          of 4:
            m.s[cfsTypes].addf("typedef NI32 $1;$n", [result])
          of 8:
            m.s[cfsTypes].addf("typedef NI64 $1;$n", [result])
          else:
            internalError(m.config, t.sym.info, "getTypeDescAux: enum")
        when false:
          ...
          if not gDebugInfo.hasEnum(t.sym.name.s, t.sym.info.line, owner):
            ...
            gDebugInfo.registerEnum(
              EnumDesc(
                size: size, owner: owner, id: t.sym.id, name: t.sym.name.s, values: vals
              )
            )
  of tyProc:
    result = getTypeName(m, origTyp, sig)
    ...
    if not isImportedType(t):
      if t.callConv != ccClosure: # procedure vars may need a closure!
        ...
        m.s[cfsTypes].addf(
          "typedef struct {$n" & "N_NIMCALL_PTR($2, ClP_0) $3;$n" &
            "void* ClE_0;$n} $1;$n",
          [result, rettype, desc],
        )
  of tySequence:
      ...
      if not isImportedType(t):
        if skipTypes(t[0], typedescInst).kind != tyEmpty:
          const
            ...
            cSeq = "struct $2 {$n" & "  #TGenericSeq Sup;$n"
          if m.compileToCpp:
            appcg(
              m,
              m.s[cfsSeqTypes],
              cppSeq & "  $1 data[SEQ_DECL_SIZE];$n" & "};$n",
              [getTypeDescAux(m, t[0], check, kind), result],
            )
          else:
            appcg(
              m,
              m.s[cfsSeqTypes],
              cSeq & "  $1 data[SEQ_DECL_SIZE];$n" & "};$n",
              [getTypeDescAux(m, t[0], check, kind), result],
            )
        else:
  ...
  of tyArray:
    ...
    if not isImportedType(t):
      ...
      m.s[cfsTypes].addf("typedef $1 $2[$3];$n", [foo, result, rope(n)])
  of tyObject, tyTuple:
    ...
    if isImportedCppType(t) and tt.kind == tyGenericInst:
      ...
      if not incompleteType(t):
        let recdesc =
          if t.kind != tyTuple:
            getRecordDesc(m, t, result, check)
          else:
            getTupleDesc(m, t, result, check)
        if not isImportedType(t):
  ...
  of tySet:
    ...
    if not isImportedType(t):
      ...
      of 1, 2, 4, 8:
        ...
        m.s[cfsTypes].addf(
          "typedef NU8 $1[$2];$n", [result, rope(getSize(m.config, t))]
        )
...
type TClosureTypeKind = enum ## In C closures are mapped to 3 different things.
...
proc getClosureType(m: BModule, t: PType, kind: TClosureTypeKind): Rope =
  ...
  if not isImportedType(t):
    if t.callConv != ccClosure or kind != clFull:
      ...
      m.s[cfsTypes].addf(
        "typedef struct {$n" & "N_NIMCALL_PTR($2, ClP_0) $3;$n" & "void* ClE_0;$n} $1;$n",
        [result, rettype, desc],
      )
...
proc finishTypeDescriptions(m: BModule) =
  ...
  while i < m.typeStack.len:
    let t = m.typeStack[i]
    if optSeqDestructors in m.config.globalOptions and
        t.skipTypes(abstractInst).kind == tySequence:
...
proc parseVFunctionDecl(
    val: string,
    name, params, retType, superCall: var string,
    isFnConst, isOverride: var bool,
    isCtor: bool,
) =
...
proc genMemberProcHeader(
  ...
  parseVFunctionDecl(
    prc.constraint.strVal, name, params, rettype, superCall, isFnConst, isOverride,
    isCtor,
  )
  ...
  result.addf(
    "$1$2($3, $4)$5$6$7$8",
    [
      rope(CallingConvToStr[prc.typ.callConv]), asPtrStr, rettype, name, params,
      fnConst, override, superCall,
    ],
  )
...
proc genTypeInfoAuxBase(
    m: BModule, typ, origType: PType, name, base: Rope, info: TLineInfo
) =
  var nimtypeKind: int
  ...
  if not containsGarbageCollectedRef(typ):
    flags = flags or 1
  if not canFormAcycle(m.g.graph, typ):
    flags = flags or 2
  ...
  if isDefined(m.config, "nimTypeNames"):
    var typename = typeToString(
      if origType.typeInst != nil: origType.typeInst else: origType, preferName
    )
    if typename == "ref object" and origType.skipTypes(skipPtrs).sym != nil:
      typename =
        "anon ref object from " & m.config $ origType.skipTypes(skipPtrs).sym.info
    m.s[cfsTypeInit3].addf("$1.name = $2;$n", [nameHcr, makeCString typename])
    cgsym(m, "nimTypeRoot")
    m.s[cfsTypeInit3].addf("$1.nextType = nimTypeRoot; nimTypeRoot=&$1;$n", [nameHcr])

  if m.hcrOn:
    m.s[cfsStrData].addf("static TNimType* $1;$n", [name])
    m.hcrCreateTypeInfosProc.addf(
      "\thcrRegisterGlobal($2, \"$1\", sizeof(TNimType), NULL, (void**)&$1);$n",
      [name, getModuleDllPath(m, m.module)],
    )
  else:
    m.s[cfsStrData].addf("N_LIB_PRIVATE TNimType $1;$n", [name])

proc genTypeInfoAux(m: BModule, typ, origType: PType, name: Rope, info: TLineInfo) =
  var base: Rope
...
proc genTNimNodeArray(m: BModule, name: Rope, size: Rope) =
  if m.hcrOn:
    ...
    m.hcrCreateTypeInfosProc.addf(
      "\thcrRegisterGlobal($3, \"$1\", sizeof(TNimNode*) * $2, NULL, (void**)&$1);$n",
...
proc genObjectFields(
    m: BModule, typ, origType: PType, n: PNode, expr: Rope, info: TLineInfo
) =
  case n.kind
  of nkRecList:
    if n.len == 1:
      ...
      m.s[cfsTypeInit3].addf(
        "$1.len = $2; $1.kind = 2; $1.sons = &$3[0];$n", [expr, rope(n.len), tmp]
      )
    else:
  ...
  of nkRecCase:
    ...
    assert L > 0
    if field.loc.r == "":
      fillObjectFields(m, typ)
    if field.loc.t == nil:
      internalError(m.config, n.info, "genObjectFields")
    m.s[cfsTypeInit3].addf(
      "$1.kind = 3;$n" & "$1.offset = offsetof($2, $3);$n" & "$1.typ = $4;$n" &
        "$1.name = $5;$n" & "$1.sons = &$6[0];$n" & "$1.len = $7;$n",
      [
        expr,
        getTypeDesc(m, origType, dkVar),
        field.loc.r,
      ...
      of nkOfBranch:
        ...
        for j in 0 ..< b.len - 1:
          if b[j].kind == nkRange:
            ...
            m.s[cfsTypeInit3].addf(
              "$1[$2] = &$3;$n", [tmp, rope(getOrdValue(b[j])), tmp2]
            )
      of nkElse:
        m.s[cfsTypeInit3].addf("$1[$2] = &$3;$n", [tmp, rope(L), tmp2])
      else:
        internalError(m.config, n.info, "genObjectFields(nkRecCase)")
  of nkSym:
    var field = n.sym
    # Do not produce code for void types
    if isEmptyType(field.typ):
      return
    if field.bitsize == 0:
      if field.loc.r == "":
        fillObjectFields(m, typ)
      if field.loc.t == nil:
        internalError(m.config, n.info, "genObjectFields")
      m.s[cfsTypeInit3].addf(
        "$1.kind = 1;$n" & "$1.offset = offsetof($2, $3);$n" & "$1.typ = $4;$n" &
          "$1.name = $5;$n",
        [
          expr,
          getTypeDesc(m, origType, dkVar),
...
proc genTupleInfo(m: BModule, typ, origType: PType, name: Rope, info: TLineInfo) =
  ...
  if typ.len > 0:
    ...
    for i in 0 ..< typ.len:
      ...
      m.s[cfsTypeInit3].addf("$1[$2] = &$3;$n", [tmp, rope(i), tmp2])
      m.s[cfsTypeInit3].addf(
        "$1.kind = 1;$n" & "$1.offset = offsetof($2, Field$3);$n" & "$1.typ = $4;$n" &
          "$1.name = \"Field$3\";$n",
        [tmp2, getTypeDesc(m, origType, dkVar), rope(i), genTypeInfoV1(m, a, info)],
      )
    m.s[cfsTypeInit3].addf(
      "$1.len = $2; $1.kind = 2; $1.sons = &$3[0];$n", [expr, rope(typ.len), tmp]
    )
  else:
    m.s[cfsTypeInit3].addf("$1.len = $2; $1.kind = 2;$n", [expr, rope(typ.len)])
...
proc genEnumInfo(m: BModule, typ: PType, name: Rope, info: TLineInfo) =
  ...
  m.s[cfsTypeInit1].addf(
    "static char* NIM_CONST $1[$2] = {$n$3};$n", [enumArray, rope(typ.n.len), enumNames]
  )
  m.s[cfsTypeInit3].addf(
    "for ($1 = 0; $1 < $2; $1++) {$n" & "$3[$1+$4].kind = 1;$n" &
      "$3[$1+$4].offset = $1;$n" & "$3[$1+$4].name = $5[$1];$n" &
      "$6[$1] = &$3[$1+$4];$n" & "}$n",
    [counter, rope(typ.n.len), m.typeNodesName, rope(firstNimNode), enumArray, nodePtrs],
  )
...
proc genDeepCopyProc(m: BModule, s: PSym, result: Rope) =
  genProc(m, s)
  m.s[cfsTypeInit3].addf(
    "$1.deepcopy =(void* (N_RAW_NIMCALL*)(void*))$2;$n", [result, s.loc.r]
  )
...
proc declareNimType(m: BModule, name: string, str: Rope, module: int) =
  ...
  if m.hcrOn:
    ...
    m.s[cfsTypeInit1].addf(
      "\t$1 = ($3*)hcrGetGlobal($2, \"$1\");$n", [str, getModuleDllPath(m, module), nr]
    )
  else:
...
proc generateRttiDestructor(
  ...
  if theProc.typ[1].kind != tyVar:
    body.add newTreeI(
      ...
      newDeref(
        newTreeIT(
          nkCast, info, castType, newNodeIT(nkType, info, castType), newSymNode(dest)
        )
      ),
    )
  else:
    ...
    body.add newTreeI(nkCall, info, newSymNode(theProc), addrOf)
  n[bodyPos] = body
  result.ast = n
...
proc genHook(
    m: BModule, t: PType, info: TLineInfo, op: TTypeAttachedOp, result: var Rope
) =
  ...
  if theProc != nil and not isTrivialProc(m.g.graph, theProc):
    ...
    if op == attachedDestructor:
      let wrapper = generateRttiDestructor(
        m.g.graph, t, theProc.owner, attachedDestructor, theProc.info, m.idgen, theProc
      )
      genProc(m, wrapper)
...
proc genTypeInfoV2OldImpl(m: BModule, t, origType: PType, name: Rope, info: TLineInfo) =
  ...
  var flags = 0
  if not canFormAcycle(m.g.graph, t):
    flags = flags or 1
  ...
  if t.kind in {tyObject, tyDistinct} and incompleteType(t):
    localError(
      m.config,
      info,
      "request for RTTI generation for incomplete object: " & typeToString(t),
    )
  ...
  addf(
    typeEntry,
    "; $1.size = sizeof($2); $1.align = (NI16) NIM_ALIGNOF($2); $1.depth = $3; $1.flags = $4;",
  ...
  if objDepth >= 0:
    ...
    m.s[cfsVars].addf(
      "static $1 $2[$3] = $4;$n",
      [
        getTypeDesc(m, getSysType(m.g.graph, unknownLineInfo, tyUInt32), dkVar),
        objDisplayStore,
        rope(objDepth + 1),
        objDisplay,
      ],
  ...
  if t.kind == tyObject and t.len > 0 and t[0] != nil and
      optEnableDeepCopy in m.config.globalOptions:
...
proc genTypeInfoV2Impl(m: BModule, t, origType: PType, name: Rope, info: TLineInfo) =
  ...
  addf(
    typeEntry,
    ", .size = sizeof($1), .align = (NI16) NIM_ALIGNOF($1), .depth = $2",
...
proc genTypeInfoV1(m: BModule, t: PType, info: TLineInfo): Rope =
  ...
  of tyStatic:
    if t.n != nil:
      ...
      internalError(m.config, "genTypeInfoV1(" & $t.kind & ')')
  of tyUserTypeClasses:
  ...
  of tyPtr, tyRange, tyUncheckedArray:
    genTypeInfoAux(m, t, t, result, info)
  of tyArray:
    genArrayInfo(m, t, result, info)
  of tySet:
    genSetInfo(m, t, result, info)
  of tyEnum:
    genEnumInfo(m, t, result, info)
  of tyObject:
  ...
  if optTinyRtti in m.config.globalOptions and t.kind == tyObject and
      sfImportc notin t.sym.flags:
    let v2info = genTypeInfoV2(m, origType, info)
    addf(
      m.s[cfsTypeInit3],
      "$1->typeInfoV1 = (void*)&$2; $2.typeInfoV2 = (void*)$1;$n",
      [v2info, result],
    )
...
proc genTypeSection(m: BModule, n: PNode) =
  ...
  for i in 0 ..< n.len:
    ...
    for p in 0 ..< n[i][0].len:
      if (n[i][0][p].kind != nkSym):
        continue
      if sfExportc in n[i][0][p].sym.flags:
        discard getTypeDescAux(
          m, n[i][0][p].typ, intSet, descKindFromSymKind(n[i][0][p].sym.kind)
        )
        if m.g.generatedHeader != nil:
          discard getTypeDescAux(
            ...
            n[i][0][p].typ,
            intSet,
            descKindFromSymKind(n[i][0][p].sym.kind),
          )
