...
import ast, types, hashes, strutils, msgs, wordrecg, platform, trees, options, cgendata
...
proc ccgIntroducedPtr*(conf: ConfigRef, s: PSym, retType: PType): bool =
  ...
  if optByRef in s.options:
    return true
  elif sfByCopy in s.flags:
    return false
  elif tfByRef in pt.flags:
    return true
  ...
  if s.position == 0 and retType != nil and retType.kind == tyLent:
    result =
      not (
        pt.kind in {tyVar, tyArray, tyOpenArray, tyVarargs, tyRef, tyPtr, tyPointer} or
