...
import
  ast, ropes, options, intsets, tables, ndi, lineinfos, pathutils, modulegraphs, sets
...
type
  ...
  TCFileSection* = enum # the sections a generated C file consists of
    ...
    cfsSeqTypes
      # section for sequence types only
  ...
  TCProc = object # represents C proc that is currently generated
    ...
    finallySafePoints*: seq[Rope]
      # For correctly cleaning up exceptions when
      # using return in finally statements
    labels*: Natural # for generating unique labels in the C proc
    blocks*: seq[TBlock] # nested blocks
    breakIdx*: int
      # the block that will be exited
      # with a regular break
    options*: TOptions
      # options that should be used for code
    ...
    splitDecls*: int
      # > 0 if we are in some context for C++ that
  ...
  CodegenFlag* = enum
    ...
    frameDeclared
      # hack for ROD support so that we don't declare
    ...
    useAliveDataFromDce
      # use the `alive: IntSet` field instead of
  ...
  BModuleList* = ref object of RootObj
    ...
    modulesClosed*: seq[BModule]
      # list of the same compiled modules, but in the order they were closed
    ...
    nimtvDeclared*: IntSet
      # so that every var/field exists only once
      # in the struct
  ...
  TCGen = object of PPassContext # represents a C source file
    ...
    cfilename*: AbsoluteFile
      # filename of the module (including path,
      # without extension)
    tmpBase*: Rope # base for temp identifier generation
    typeCache*: TypeCache # cache the generated types
    typeABICache*: HashSet[SigHash]
      # cache for ABI checks; reusing typeCache
    ...
    extensionLoaders*: array['0' .. '9', Rope]
      # special procs for the
      # OpenGL wrapper
...
template vccAndC*(p: BProc): bool =
  p.module.config.cCompiler == ccVcc and p.module.config.backend == backendC
...
proc newProc*(prc: PSym, module: BModule): BProc =
  result = BProc(
    prc: prc,
    module: module,
    options: if prc != nil: prc.options else: module.config.options,
    blocks: @[initBlock()],
...
proc newModuleList*(g: ModuleGraph): BModuleList =
  BModuleList(
    typeInfoMarker: initTable[SigHash, tuple[str: Rope, owner: int32]](),
    config: g.config,
    graph: g,
