...
import
  ast, msgs, idents, renderer, magicsys, lowerings, lambdalifting, modulegraphs,
  lineinfos, tables, options
...
type Ctx = object
  ...
  unrollFinallySym: PSym
    # Indicates that we're unrolling finally states (either exception happened or premature return)
  ...
  exceptionTable: seq[int]
    # For state `i` jump to state `exceptionTable[i]` if exception is raised
  hasExceptions: bool # Does closure have yield in try?
  curExcHandlingState: int # Negative for except, positive for finally
  nearestFinally: int
    # Index of the nearest finally block. For try/except it
...
proc newStateAccess(ctx: var Ctx): PNode =
  if ctx.stateVarSym.isNil:
    result = rawIndirectAccess(
      newSymNode(getEnvParam(ctx.fn)), getStateField(ctx.g, ctx.fn), ctx.fn.info
    )
  else:
...
proc newUnrollFinallyAccess(ctx: var Ctx, info: TLineInfo): PNode =
  if ctx.unrollFinallySym.isNil:
    ctx.unrollFinallySym =
      ctx.newEnvVar(":unrollFinally", ctx.g.getSysType(info, tyBool))
  ctx.newEnvVarAccess(ctx.unrollFinallySym)

proc newCurExcAccess(ctx: var Ctx): PNode =
  if ctx.curExcSym.isNil:
    ctx.curExcSym =
      ctx.newEnvVar(":curExc", ctx.g.callCodegenProc("getCurrentException").typ)
...
proc transformBreaksAndContinuesInWhile(
    ctx: var Ctx, n: PNode, before, after: PNode
): PNode =
  result = n
  case n.kind
  of nkSkip:
  ...
  of nkWhileStmt:
    discard
  # Do not recurse into nested whiles
  of nkContinueStmt:
    result = before
  of nkBlockStmt:
    inc ctx.blockLevel
...
proc addElseToExcept(ctx: var Ctx, n: PNode) =
  if n.kind == nkStmtList and n[1].kind == nkIfStmt and n[1][^1].kind != nkElse:
    ...
    block: # :curExc = getCurrentException()
      branchBody.add(
        newTree(
          nkAsgn, ctx.newCurExcAccess(), ctx.g.callCodegenProc("getCurrentException")
        )
      )
...
proc lowerStmtListExprs(ctx: var Ctx, n: PNode, needsSplit: var bool): PNode =
      ...
      if isExpr:
        ...
        for i in 1 ..< n.len:
          ...
          of nkElse:
            ...
            internalError(
              ctx.g.config, "lowerStmtListExpr(nkCaseStmt): " & $branch.kind
            )
        result.add(n)
        result.add(ctx.newEnvVarAccess(tmp))
    ...
    if ns:
      ...
      if n[0].kind == nkSym and n[0].sym.magic in {mAnd, mOr}:
        ...
        for i in 0 ..< n.len:
          ...
          if n[i].kind in nkCallKinds:
            # XXX: This should better be some sort of side effect tracking
  ...
  of nkDiscardStmt, nkReturnStmt, nkRaiseStmt:
    ...
    if ns:
      ...
      n[0] = ex
      result.add(n)
  of nkCast, nkHiddenStdConv, nkHiddenSubConv, nkConv, nkObjDownConv, nkDerefExpr,
      nkHiddenDeref:
    var ns = false
...
proc newEndFinallyNode(ctx: var Ctx, info: TLineInfo): PNode =
  ...
  nilnode.typ = curExc.typ
  let cmp = newTree(
    nkCall, newSymNode(ctx.g.getSysMagic(info, "==", mEqRef), info), curExc, nilnode
  )
  ...
  let nullifyExc =
    newTree(nkCall, newSymNode(ctx.g.getCompilerProc("closureIterSetupExc")), nilnode)
...
proc newArrayType(
    g: ModuleGraph, n: int, t: PType, idgen: IdGenerator, owner: PSym
): PType =
...
proc createExceptionTable(ctx: var Ctx): PNode {.inline.} =
  ...
  result.typ = ctx.g.newArrayType(
    ctx.exceptionTable.len, ctx.g.getSysType(ctx.fn.info, tyInt16), ctx.idgen, ctx.fn
  )
...
proc newCatchBody(ctx: var Ctx, info: TLineInfo): PNode {.inline.} =
  ...
  block:
    ...
    let getNextState =
      newTree(nkBracketExpr, ctx.createExceptionTable(), ctx.newStateAccess())
    ...
    if c.blocks.len == 0:
      ...
      if fin >= 0:
        ...
        for i in countdown(c.finallys.high, fin):
          var vars = FreshVarsContext(
            tab: initTable[int, PSym](), config: c.config, info: n.info, idgen: c.idgen
          )
...
proc transformClosureIterator*(
    g: ModuleGraph, idgen: IdGenerator, fn: PSym, n: PNode
): PNode =
  var ctx: Ctx
  ctx.g = g
  ctx.fn = fn
  ...
  ctx.stateLoopLabel =
    newSym(skLabel, getIdent(ctx.g.cache, ":stateLoop"), idgen, fn, fn.info)
