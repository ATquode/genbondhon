...
proc semConceptDecl(c: PContext, n: PNode): PNode =
  ...
  of nkCommentStmt:
    ...
    localError(
      c.config,
      n.info,
      "unexpected construct in the new-styled concept: " & renderTree(n),
    )
    result = n
...
type MatchCon = object ## Context we pass around during concept matching.
  inferred: seq[(PType, PType)]
    ## we need a seq here so that we can easily undo inferences \
    ## that turned out to be wrong.
  marker: IntSet ## Some protection against wild runaway recursions.
  potentialImplementation: PType
    ## the concrete type that might match the concept we try to match.
  magic: TMagic
    ## mArrGet and mArrPut is wrong in system.nim and
...
proc existingBinding(m: MatchCon, key: PType): PType =
  ...
  for i in 0 ..< m.inferred.len:
    if m.inferred[i][0] == key:
      return m.inferred[i][1]
  return nil
...
proc matchType(c: PContext, f, a: PType, m: var MatchCon): bool =
  ...
  const ignorableForArgType =
    {tyVar, tySink, tyLent, tyOwned, tyGenericInst, tyAlias, tyInferred}
  case f.kind
  of tyAlias:
        ...
        if f.len > 0 and f[0].kind != tyNone:
          ...
          if result:
            when logBindings:
              echo "A adding ", f, " ", ak
            m.inferred.add((f, ak))
        elif m.magic == mArrGet and
            ak.kind in {
              tyArray, tyOpenArray, tySequence, tyVarargs, tyCstring, tyString
            }:
          when logBindings:
            echo "B adding ", f, " ", lastSon ak
  ...
  of tyBool, tyChar, tyInt .. tyUInt64:
    ...
    result =
      ak.kind == f.kind or ak.kind == tyOrdinal or
...
proc conceptMatch*(
    c: PContext, concpt, arg: PType, bindings: var TIdTable, invocation: PType
): bool =
    ...
    for (a, b) in m.inferred:
      if b.kind == tyGenericParam:
        ...
        while true:
          ...
          if dest == nil or dest.kind != tyGenericParam:
            break
        if dest != nil:
