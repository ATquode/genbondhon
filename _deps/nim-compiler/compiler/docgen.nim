...
type
  ItemFragment = object
    ## A fragment from each item will be eventually
  ...
  Item = object
    ## Any item in documentation, e.g. symbol
    ## entry. Configuration variable ``doc.item``
    ## is used for its HTML rendering.
    descRst: ItemPre
      ## Description of the item (may contain
      ## runnableExamples).
    ...
    name: string
      ## short name of the symbol, not unique
  ...
  ModSection = object ## Section like Procs, Types, etc.
    secItems: Table[string, seq[Item]] ## Map basic name -> pre-processed items.
  ...
  ExampleGroup = ref object ## a group of runnableExamples with same rdoccmd
  ...
  TDocumentor = object of rstgen.RstGenerator
    ...
    tocTable: array[TSymKind, Table[string, seq[TocItem]]] # ...otherwise (e.g. procs)
    toc2: TocSectionsFinal
      # TOC `content`, which is probably wrapped
      # in `doc.section.toc2`
    ...
    exampleCounter: int
    emitted: IntSet
      # we need to track which symbols have been emitted
...
proc add(dest: var ItemPre, rst: PRstNode) =
  dest.add ItemFragment(isRst: true, rst: rst)

proc add(dest: var ItemPre, str: string) =
  dest.add ItemFragment(isRst: false, str: str)
  ...
  while iA < aLen and iB < bLen:
    if isDigit(a[iA]) and isDigit(b[iB]):
      var
        ...
        limitB = iB
      while limitA < aLen and isDigit(a[limitA]):
        inc limitA
      while limitB < bLen and isDigit(b[limitB]):
        inc limitB
...
proc presentationPath*(conf: ConfigRef, file: AbsoluteFile): RelativeFile =
  ...
  of docRootDefault:
    ...
    if not dir.isEmpty:
      ...
      if not result2.isEmpty and
          (result.isEmpty or result2.string.len < result.string.len):
        result = result2
    if result.isEmpty:
      bail()
  of "@pkg":
...
template declareClosures(currentFilename: AbsoluteFile, destFile: string) =
  proc compilerMsgHandler(
      filename: string, line, col: int, msgKind: rst.MsgKind, arg: string
  ) {.gcsafe.} =
    # translate msg kind:
    var k: TMsgKind
    case msgKind
    ...
    of meExpected:
      k = errXExpected
    of meMissingClosing:
      k = errRstMissingClosing
    of meGridTableNotImplemented:
      k = errRstGridTableNotImplemented
    of meMarkdownIllformedTable:
      k = errRstMarkdownIllformedTable
    of meIllformedTable:
      k = errRstIllformedTable
    of meNewSectionExpected:
      k = errRstNewSectionExpected
    of meGeneralParseError:
      k = errRstGeneralParseError
    of meInvalidDirective:
      k = errRstInvalidDirectiveX
    of meInvalidField:
      k = errRstInvalidField
    of meFootnoteMismatch:
      k = errRstFootnoteMismatch
    of meSandboxedDirective:
      k = errRstSandboxedDirective
    of mwRedefinitionOfLabel:
      k = warnRstRedefinitionOfLabel
    of mwUnknownSubstitution:
      k = warnRstUnknownSubstitutionX
    of mwAmbiguousLink:
      k = warnRstAmbiguousLink
    of mwBrokenLink:
      k = warnRstBrokenLink
    of mwUnsupportedLanguage:
      k = warnRstLanguageXNotSupported
    of mwUnsupportedField:
      k = warnRstFieldXNotSupported
    of mwUnusedImportdoc:
      k = warnRstUnusedImportdoc
    of mwRstStyle:
      k = warnRstStyle
    {.gcsafe.}:
      let errorsAsWarnings =
        (roPreferMarkdown in d.sharedState.options) and not d.standaloneDoc
        # not tolerate errors in .rst/.md files
  ...
  proc docgenFindRefFile(
    ...
    result.linkRelPath =
      relativePath(result.targetPath.splitFile.dir, fromDir).replace('\\', '/')

proc parseRst(
    text: string, line, column: int, conf: ConfigRef, sharedState: PRstSharedState
): PRstNode =
  result = rstParsePass1(text, line, column, sharedState)

proc getOutFile2(
    conf: ConfigRef, filename: RelativeFile, ext: string, guessTarget: bool
): AbsoluteFile =
...
proc newDocumentor*(
  ...
  let destFile =
    getOutFile2(conf, presentationPath(conf, filename), outExt, false).string
  new(result)
  ...
  if not standaloneDoc:
    options.incl roNimFile
  # (options can be changed dynamically in `setDoctype` by `{.doctype.}`)
  result.hasToc = hasToc
  result.sharedState = newRstSharedState(
    options, filename.string, docgenFindFile, docgenFindRefFile, compilerMsgHandler,
    hasToc,
  )
  initRstGenerator(
    result[],
    (if conf.isLatexCmd: outLatex else: outHtml),
    conf.configVars,
    filename.string,
    docgenFindFile,
    compilerMsgHandler,
...
<script>
  ...
  result.onTestSnippet = proc(
      gen: var RstGenerator, filename, cmd: string, status: int, content: string
  ) {.gcsafe.} =
    if conf.docCmd == docCmdSkip:
      return
    inc(gen.id)
    ...
    if filename.len == 0:
      ...
      outp =
        getNimcacheDir(conf) / "snippets".RelativeDir / RelativeDir(nameOnly) /
    ...
    proc interpSnippetCmd(cmd: string): string =
      ...
      if cmd.startsWith "nim ":
        result = "$nim " & cmd[4 ..^ 1]
      else:
        result = cmd
      # factor with D20210224T221756
      result =
        result.replace("$1", "$options") % [
          "nim",
          os.getAppFilename().quoteShell,
          "libpath",
          quoteShell(d.conf.libpath),
          "docCmd",
          d.conf.docCmd,
          "backend",
          $d.conf.backend,
    ...
    if gotten != status:
      rawMessage(
        conf,
        errGenerated,
        "snippet failed: cmd: '$1' status: $2 expected: $3 output: $4" %
          [cmd, $gotten, $status, output],
      )
...
template dispA(
    conf: ConfigRef, dest: var string, xml, tex: string, args: openArray[string]
) =
  if not conf.isLatexCmd:
    dest.addf(xml, args)
  else:
    dest.addf(tex, args)
...
proc genRecCommentAux(d: PDoc, n: PNode): PRstNode =
  ...
  if result == nil:
    if n.kind in {
      nkStmtList, nkStmtListExpr, nkTypeDef, nkConstDef, nkTypeClassTy, nkObjectTy,
      nkRefTy, nkPtrTy, nkAsgn, nkFastAsgn, nkSinkAsgn, nkHiddenStdConv,
    }:
...
proc genRecComment(d: PDoc, n: PNode): PRstNode =
  ...
  if result == nil:
    if n.kind in {
      nkProcDef, nkFuncDef, nkMethodDef, nkIteratorDef, nkMacroDef, nkTemplateDef,
      nkConverterDef,
    }:
...
proc nodeToHighlightedHtml(
  ...
  var tokenPos = 0
  var procTokenPos = 0
  template escLit(): untyped =
    esc(d.target, literal)
  ...
  while true:
    ...
    of tokKeywordLow .. tokKeywordHigh:
      if kind in {
        tkProc, tkMethod, tkIterator, tkMacro, tkTemplate, tkFunc, tkConverter
      }:
    ...
    of tkSymbol:
      ...
      if procTokenPos == tokenPos - 2 and procLink != "":
        dispA(
          d.conf,
          result,
          "<a href=\"#$2\"><span class=\"Identifier\">$1</span></a>",
          "\\spanIdentifier{$1}",
          [escLit, procLink],
        )
        ...
        result.addf "<a href=\"$1#$2\"><span class=\"Identifier\">$3</span></a>",
          [changeFileExt(external, "html"), literal, escLit]
      else:
        dispA(
          d.conf,
          result,
    ...
    of tkHideableStart:
      template fun(s) =
        dispA(d.conf, result, s, "\\spanOther{$1}", [escLit])

      if renderRunnableExamples in renderFlags:
        fun "$1"
      else:
...
<span class="pragmawrap">""".replace(
    ...
    of tkHideableEnd:
      ...
      if renderRunnableExamples in renderFlags:
        ...
        fun "</span>"
    of tkCurlyDotLe:
      dispA(d.conf, result, "$1", "\\spanOther{$1}", [escLit])
    of tkCurlyDotRi:
      dispA(d.conf, result, "$1", "\\spanOther{$1}", [escLit])
    of tkParLe, tkParRi, tkBracketLe, tkBracketRi, tkCurlyLe, tkCurlyRi, tkBracketDotLe,
        tkBracketDotRi, tkParDotLe, tkParDotRi, tkComma, tkSemiColon, tkColon, tkEquals,
        tkDot, tkDotDot, tkAccent, tkColonColon, tkGStrLit, tkGTripleStrLit, tkInfixOpr,
        tkPrefixOpr, tkPostfixOpr, tkBracketLeColon:
      dispA(
        d.conf, result, "<span class=\"Other\">$1</span>", "\\spanOther{$1}", [escLit]
      )

proc exampleOutputDir(d: PDoc): AbsoluteDir =
  d.conf.getNimcacheDir / RelativeDir"runnableExamples"
...
proc runAllExamples(d: PDoc) =
  ...
  for _, group in d.exampleGroups:
    if group.docCmd == docCmdSkip:
      continue
    let outp =
      outputDir /
      RelativeFile(
        "$1_group$2_examples.nim" % [d.filename.splitFile.name, $group.index]
      )
    group.code =
      "# autogenerated by docgen\n# source: $1\n# rdoccmd: $2\n$3" %
      [d.filename, group.rdoccmd, group.code]
    ...
    let cmd =
      "$nim $backend -r --lib:$libpath --warning:UnusedImport:off $pathArgs --nimcache:$nimcache $rdoccmd $docCmd $file" %
      [
        "nim",
        quoteShell(os.getAppFilename()),
        "backend",
        $d.conf.backend,
        "pathArgs",
        pathArgs,
    ...
    if d.conf.backend == backendJs and findNodeJs() == "":
      ...
      d.conf.quitOrRaise "[runnableExamples] failed: generated file: '$1' group: '$2' cmd: $3" %
        [outp.string, group[].prettyString, cmd]
    else:
...
proc prepareExample(
    d: PDoc, n: PNode, topLevel: bool
): tuple[rdoccmd: string, code: string] =
  ## returns `rdoccmd` and source code for this runnableExamples
  var rdoccmd = ""
  if n.len < 2 or n.len > 3:
    globalError(d.conf, n.info, "runnableExamples invalid")
  if n.len == 3:
    let n1 = n[1]
    # xxx this should be evaluated during sempass
    if n1.kind notin nkStrKinds:
      globalError(d.conf, n1.info, "string litteral expected")
    rdoccmd = n1.strVal
  ...
  let outp =
    ...
    RelativeFile(
      "$#_examples_$#.nim" %
        [d.filename.extractFilename.changeFileExt"", $d.exampleCounter]
    )

  if useRenderModule:
    var docComment = newTree(nkCommentStmt)
    docComment.comment = comment
    var runnableExamples = newTree(
      nkStmtList,
      docComment,
...
""" %
  ...
  if rdoccmd notin d.exampleGroups:
    d.exampleGroups[rdoccmd] =
      ExampleGroup(rdoccmd: rdoccmd, docCmd: d.conf.docCmd, index: d.exampleGroups.len)
...
proc getAllRunnableExamplesImpl(
    d: PDoc, n: PNode, dest: var ItemPre, state: RunnableState, topLevel: bool
): RunnableState =
  ##[
  ...
  of nkCommentStmt:
    if state in {rsStart, rsRunnable}:
      ...
      return rsComment
  of nkCallKinds:
    if isRunnableExamples(n[0]) and n.len >= 2 and n.lastSon.kind == nkStmtList:
      if state in {rsStart, rsComment, rsRunnable}:
        ...
        localError(
          d.conf, n.info, errUser,
          "runnableExamples must appear before the first non-comment statement",
        )
  else:
    discard
  return
...
=>
  ...
  if result.kind == nkAsgn and result[0].kind == nkSym and n.len > bodyPos + 1 and
      n[bodyPos + 1].kind == nkSym:
...
proc getName(n: PNode): string =
  ...
  of nkPragmaExpr:
    result = getName(n[0])
  of nkSym:
    result = n.sym.renderDefinitionName
  of nkIdent:
    result = n.ident.s
  of nkAccQuoted:
    result = "`"
...
proc getNameIdent(cache: IdentCache, n: PNode): PIdent =
  ...
  of nkAccQuoted:
    var r = ""
    for i in 0 ..< n.len:
      r.add(getNameIdent(cache, n[i]).s)
...
proc getRstName(n: PNode): PRstNode =
  case n.kind
  of nkPostfix:
    result = getRstName(n[1])
  of nkPragmaExpr:
    result = getRstName(n[0])
  of nkSym:
    result = newRstLeaf(n.sym.renderDefinitionName)
  of nkIdent:
    result = newRstLeaf(n.ident.s)
  of nkAccQuoted:
    result = getRstName(n[0])
    for i in 1 ..< n.len:
      result.text.add(getRstName(n[i]).text)
  of nkOpenSymChoice, nkClosedSymChoice:
    result = getRstName(n[0])
  else:
    result = nil
...
proc genDeprecationMsg(d: PDoc, n: PNode): string =
  ...
  of 0: # Deprecated w/o any message
    result =
      getConfigVar(d.conf, "doc.deprecationmsg") % [
...
proc symbolPriority(k: TSymKind): int =
  result =
    ...
    # including skProc which have higher priority
...
proc toLangSymbol(k: TSymKind, n: PNode, baseName: string): LangSymbol =
  ...
  if k in {skProc, skFunc, skType, skIterator}:
    ...
    if genNode != nil:
      ...
      initTokRender(
        ...
        genNode,
        {
          renderNoBody, renderNoComments, renderNoPragmas, renderNoProcDefs,
          renderExpandUsing,
        },
      )
      var kind = tkEof
  ...
  if k == skType:
    result.symTypeKind = getTypeKind(n)
...
proc genItem(
  ...
  initTokRender(
    ...
    n,
    {
      renderNoBody, renderNoComments, renderDocComments, renderNoPragmas,
      renderNoProcDefs, renderExpandUsing,
    },
  )
  while true:
    getNextTok(r, kind, literal)
  ...
  if pragmaNode != nil:
    pragmaNode = findPragma(pragmaNode, wDeprecated)

  inc(d.id)
  let
    plainNameEsc = esc(d.target, plainName.strip)
    typeDescr =
      if k == skType and getTypeKind(n) != "":
        getTypeKind(n)
      else:
        k.toHumanStr
    detailedName = typeDescr & " " & (if k in routineKinds: plainName else: name)
  ...
  let renderFlags =
    if nonExports:
      {
        renderNoBody, renderNoComments, renderDocComments, renderSyms,
        renderExpandUsing, renderNonExportedFields,
      }
    else:
      {renderNoBody, renderNoComments, renderDocComments, renderSyms, renderExpandUsing}
  ...
  let external =
    d.destFile.AbsoluteFile.relativeTo(d.conf.outDir, '/').changeFileExt(HtmlExt).string
  ...
  elif k == skType and nameNode.kind == nkSym and
      nameNode.sym.typ.kind in {tyEnum, tyBool}:
    let etyp = nameNode.sym.typ
    for e in etyp.n:
      if e.sym.kind != skEnumField:
        continue
      ...
      setIndexTerm(
        ...
        id = symbolOrId,
        term = plain,
        linkTitle = nameNode.sym.name.s & '.' & plain,
  ...
  d.tocSimple[k].add TocItem(
    sortName: sortName,
    content:
      getConfigVar(d.conf, "doc.item.toc") %
      ["name", name, "header_plain", plainNameEsc, "itemSymOrIDEnc", symbolOrIdEnc],
  )
...
proc genJsonItem(
    d: PDoc, n, nameNode: PNode, k: TSymKind, nonExports = false
): JsonItem =
  if not isVisible(d, nameNode):
    return
  var
  ...
  initTokRender(r, n, renderFlags)
  result.json =
    %{"name": %name, "type": %($k), "line": %n.info.line.int, "col": %n.info.col}
  if comm != nil:
    result.rst = comm
    ...
    for paramIdx in 1 ..< n[paramsPos].len:
      for identIdx in 0 ..< n[paramsPos][paramIdx].len - 2:
        ...
        if n[paramsPos][paramIdx][^1].kind != nkEmpty:
          ...
          result.json["signature"]["arguments"].add %{
            "name": %paramName, "type": %paramType, "default": %paramDefault
          }
        else:
          result.json["signature"]["arguments"].add %{
...
proc setDoctype(d: PDoc, n: PNode) =
  ...
  if n == nil:
    return
  if n.len != 2:
    localError(d.conf, n.info, errUser, "doctype pragma takes exactly 1 argument")
    return
  var dt = ""
  case n[1].kind
  of nkStrLit:
...
proc traceDeps(d: PDoc, it: PNode) =
  ...
  if it.kind == nkInfix and it.len == 3 and it[2].kind == nkBracket:
    ...
    if d.section[k].finalMarkup != "":
      d.section[k].finalMarkup.add(", ")
    dispA(
      d.conf,
      d.section[k].finalMarkup,
      "<a class=\"reference external\" href=\"$2\">$1</a>",
      "$1",
      [esc(d.target, external.prettyLink), changeFileExt(external, "html")],
    )
  ...
  if s.kind == skModule and belongsToProjectPackage(d.conf, s):
    ...
    if belongsToProjectPackage(d.conf, module):
      ...
      dispA(
        ...
        "$1",
        [esc(d.target, s.name.s), changeFileExt(external, "html"), symbolOrId],
      )
...
proc documentEffect(
    cache: IdentCache, n, x: PNode, effectType: TSpecialWord, idx: int
): PNode =
  ...
  if isNil(spec):
    ...
    result = newTreeI(
      nkExprColonExpr,
      n.info,
      newIdentNode(getIdent(cache, $effectType), n.info),
      effects,
    )

proc documentWriteEffect(
    cache: IdentCache, n: PNode, flag: TSymFlag, pragmaName: string
): PNode =
  let s = n[namePos].sym
  let params = s.typ.n
  ...
  if effects.len > 0:
    result = newTreeI(
      nkExprColonExpr,
      n.info,
      newIdentNode(getIdent(cache, pragmaName), n.info),
      effects,
    )
...
proc generateDoc*(
  ...
  of nkExportExceptStmt:
    discard "transformed into nkExportStmt by semExportExcept"
  ...
  if useMetaTitle and d.meta[metaTitle].len != 0:
    ...
    linkTitle =
      if d.standaloneDoc:
        ...
        canonicalImport(d.conf, AbsoluteFile d.filename)
  if not d.standaloneDoc:
    linkTitle = "module " & linkTitle
  setIndexTerm(
    d[], titleKind, htmlFile = external, id = "", term = term, linkTitle = linkTitle
  )
...
proc finishGenerateDoc*(d: var PDoc) =
  ...
  let importdoc =
    optGenIndexOnly notin d.conf.globalOptions and
    ...
    if k in routineKinds:
      for plainName, overloadChoices in d.section[k].secItems:
        if overloadChoices.len > 1:
          ...
          let tooltip =
            "$1 ($2 overloads)" % [k.toHumanStr & " " & plainName, $overloadChoices.len]
          ...
          setIndexTerm(
            ...
            id = refn,
            term = name,
            linkTitle = k.toHumanStr,
            linkDesc = "",
            line = overloadChoices[0].info.line.int,
          )
          if optGenIndexOnly in d.conf.globalOptions:
            continue
          addAnchorNim(
            ...
            refn,
            tooltip,
            LangSymbol(symKind: k.toHumanStr, name: name, isGroup: true),
            priority = symbolPriority(k),
  ...
  proc cmp(x, y: Item): int =
    cmpDecimalsIgnoreCase(x.sortName, y.sortName)
      ...
      for item in overloadChoices:
        ...
        nameContent.add(
          getConfigVar(d.conf, "doc.item") % (
            item.substitutions &
            @["desc", itemDesc, "name", item.name, "itemSymOrID", item.anchor]
          )
        )
      if k in routineKinds:
        ...
        d.section[k].finalMarkup.add(
          getConfigVar(d.conf, "doc.item2") % (
            @[
              ...
              plainNameEsc1,
              "overloadGroupName",
              overloadGroupName(plainNameEsc2, k),
              "content",
              nameContent,
            ]
          )
...
proc generateJson*(d: PDoc, n: PNode, config: ConfigRef, includeComments: bool = true) =
  ...
  of nkCommentStmt:
    if includeComments:
      d.add JsonItem(
        rst: genComment(d, n), rstField: "comment", json: %Table[string, string]()
      )
    else:
      d.modDescPre.add(genComment(d, n))
  of nkProcDef, nkFuncDef:
    ...
    for i in 0 ..< n.len:
      if n[i].kind != nkCommentStmt:
        ...
        d.add genJsonItem(
          ...
          n[i][0],
          succ(skType, ord(n.kind) - ord(nkTypeSection)),
          optShowNonExportedFields in config.globalOptions,
        )
  of nkStmtList:
    for i in 0 ..< n.len:
  ...
  of nkTypeSection, nkVarSection, nkLetSection, nkConstSection:
    for i in 0 ..< n.len:
      if n[i].kind != nkCommentStmt:
        # order is always 'type var let const':
        r.add genTagsItem(
          d, n[i], n[i][0], succ(skType, ord(n.kind) - ord(nkTypeSection))
        )
  of nkStmtList:
    for i in 0 ..< n.len:
...
proc genSection(d: PDoc, kind: TSymKind, groupedToc = false) =
  const sectionNames: array[skModule .. skField, string] = [
    "Imports", "Types", "Vars", "Lets", "Consts", "Vars", "Procs", "Funcs", "Methods",
    "Iterators", "Converters", "Macros", "Templates", "Exports",
  ]
  if d.section[kind].finalMarkup == "":
    return
  var title = sectionNames[kind]
  d.section[kind].finalMarkup =
    getConfigVar(d.conf, "doc.section") % [
      "sectionid",
      $ord(kind),
      "sectionTitle",
      title,
  ...
  if d.toc2[kind] != "":
    ...
    d.toc[kind] =
      getConfigVar(d.conf, "doc.section.toc") %
      (sectionValues & @["content", d.toc2[kind]])
  else:
    # Just have the link
    d.toc[kind] = getConfigVar(d.conf, "doc.section.toc_item") % sectionValues

proc relLink(
    outDir: AbsoluteDir, destFile: AbsoluteFile, linkto: RelativeFile
): string =
...
proc genOutFile(d: PDoc, groupedToc = false): string =
  ...
  for i in TSymKind:
    code.add(d.section[i].finalMarkup)
  ...
  if d.meta[metaSubtitle] != "":
    dispA(
      ...
      "\\\\\\vspace{0.5em}\\large $1",
      [esc(d.target, d.meta[metaSubtitle])],
    )

  var groupsection = getConfigVar(d.conf, "doc.body_toc_groupsection")
  let bodyname =
    if d.hasToc and not d.standaloneDoc and not d.conf.isLatexCmd:
  ...
  content =
    getConfigVar(d.conf, bodyname) % [
      ...
      d.modDeprecationMsg,
      "theindexhref",
      relLink(d.conf.outDir, d.destFile.AbsoluteFile, theindexFname.RelativeFile),
  ...
  if optCompileOnly notin d.conf.globalOptions:
    ...
    code =
      getConfigVar(d.conf, "doc.file") % [
        ...
        d.meta[metaAuthor],
        "version",
        esc(d.target, d.meta[metaVersion]),
        "analytics",
        d.analytics,
        "deprecationMsg",
        d.modDeprecationMsg,
...
proc indexFile(d: PDoc): AbsoluteFile =
  ...
  result =
    dir / changeFileExt(presentationPath(d.conf, AbsoluteFile d.filename), IndexExt)
...
proc updateOutfile(d: PDoc, outfile: AbsoluteFile) =
  if d.module == nil or sfMainModule in d.module.flags:
    # nil for e.g. for commandRst2Html
...
proc writeOutput*(d: PDoc, useWarning = false, groupedToc = false) =
  ...
  if optStdout in d.conf.globalOptions:
    ...
    template outfile(): untyped =
      d.destFile.AbsoluteFile
    ...
    try:
      ...
      rawMessage(
        d.conf,
        if useWarning: warnCannotOpenFile else: errCannotOpenFile,
        outfile.string,
      )
  ...
  if optStdout in d.conf.globalOptions:
    ...
    if open(f, d.destFile, fmWrite):
      ...
      localError(
        ...
        warnUser,
        "unable to open file \"" & d.destFile & "\" for writing",
      )
...
proc commandRst2Html*(cache: IdentCache, conf: ConfigRef, preferMarkdown = false) =
  commandRstAux(cache, conf, conf.projectFull, HtmlExt, preferMarkdown)

proc commandRst2TeX*(cache: IdentCache, conf: ConfigRef, preferMarkdown = false) =
...
proc commandJson*(cache: IdentCache, conf: ConfigRef) =
  ...
  if ast == nil:
    return
  var d = newDocumentor(conf.projectFull, cache, conf, hasToc = true)
  d.onTestSnippet = proc(
      d: var RstGenerator, filename, cmd: string, status: int, content: string
  ) {.gcsafe.} =
    localError(
      conf,
      newLineInfo(conf, AbsoluteFile d.filename, -1, -1),
      warnUser,
...
proc commandBuildIndex*(conf: ConfigRef, dir: string, outFile = RelativeFile"") =
  ...
  var outFile = outFile
  if outFile.isEmpty:
    outFile = theindexFname.RelativeFile.changeFileExt("")
  let filename = getOutFile(conf, outFile, HtmlExt)

  let code =
    getConfigVar(conf, "doc.file") % [
      "nimdoccss",
