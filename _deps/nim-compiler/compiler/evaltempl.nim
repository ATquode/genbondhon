...
import strutils, options, ast, astalgo, msgs, renderer, lineinfos, idents, trees
...
type TemplCtx = object
  ...
  isDeclarative: bool
  mapping: TIdTable
    # every gensym'ed symbol needs to be mapped to some
    # new symbol
  config: ConfigRef
  ic: IdentCache
  instID: int
...
proc evalTemplateAux(templ, actual: PNode, c: var TemplCtx, result: PNode) =
  ...
  of nkSym:
    ...
    if (s.owner == nil and s.kind == skParam) or s.owner == c.owner:
      if s.kind == skParam and {sfGenSym, sfTemplateParam} * s.flags == {
        sfTemplateParam
      }:
        handleParam actual[s.position]
      elif (s.owner != nil) and (
        s.kind == skGenericParam or
  ...
  of nkCommentStmt:
    ...
    if templ.kind in {
      ...
      nkConverterDef, nkTypeSection, nkVarSection, nkLetSection, nkConstSection,
    } and not c.isDeclarative:
      c.isDeclarative = true
      isDeclarative = true
    if (not c.isDeclarative) and templ.kind in nkCallKinds and
        isRunnableExamples(templ[0]):
  ...
  var
    ...
    genericParams =
      if fromHlo:
        ...
        s.ast[genericParamsPos].len
    expectedRegularParams = s.typ.len - 1
    givenRegularParams = totalParams - genericParams
  if givenRegularParams < 0:
    givenRegularParams = 0

  if totalParams > expectedRegularParams + genericParams:
    globalError(conf, n.info, errWrongNumberOfArguments)

  if totalParams < genericParams:
    globalError(conf, n.info, errMissingGenericParamsForTemplate % n.renderTree)
...
proc evalTemplate*(
  ...
  if isAtom(body):
    ...
    if result.len == 1:
      ...
      localError(
        conf, result.info, "illformed AST: " & renderTree(result, {renderNoComments})
      )
  else:
