...
import
  std/
    [os, osproc, streams, sequtils, times, strtabs, json, jsonutils, sugar, parseutils]
...
type
  ...
  TInfoCC* =
    tuple
      ...
      pic: string
        # command for position independent code
...
template compiler(name, settings: untyped): untyped =
  proc name(): TInfoCC {.compileTime.} =
    settings

const gnuAsmListing = "-Wa,-acdl=$asmfile -g -fverbose-asm -masm=intel"
...
compiler gcc:
  result = (
    ...
    props:
      {hasSwitchRange, hasComputedGoto, hasCpp, hasGcGuard, hasGnuAsm, hasAttribute},
  )
...
proc isVSCompatible*(conf: ConfigRef): bool =
  return
    conf.cCompiler == ccVcc or conf.cCompiler == ccClangCl or
...
proc getConfigVar(conf: ConfigRef, c: TSystemCC, suffix: string): string =
  ...
  case conf.backend
  of backendCpp, backendJs, backendObjc:
    fullSuffix = "." & $conf.backend & suffix
  of backendC:
    discard
  of backendInvalid:
  ...
  if (
    conf.target.hostOS != conf.target.targetOS or
    conf.target.hostCPU != conf.target.targetCPU
  ) and optCompileOnly notin conf.globalOptions:
    let fullCCname =
      platform.CPU[conf.target.targetCPU].name & '.' &
      platform.OS[conf.target.targetOS].name & '.' & CC[c].name & fullSuffix
...
proc setCC*(conf: ConfigRef, ccname: string, info: TLineInfo) =
  ...
  if conf.cCompiler == ccNone:
    localError(
      conf,
      info,
      "unknown C compiler: '$1'. Available options are: $2" % [ccname, listCCnames()],
    )
...
proc completeCfilePath*(
    conf: ConfigRef, cfile: AbsoluteFile, createSubDir: bool = true
): AbsoluteFile =
...
proc execWithEcho(conf: ConfigRef, cmd: string, msg = hintExecuting): int =
  rawMessage(
    conf,
    msg,
    if msg == hintLinking and
        not (optListCmd in conf.globalOptions or conf.verbosity > 1): "" else: cmd,
  )
  result = execCmd(cmd)
...
proc execExternalProgram*(conf: ConfigRef, cmd: string, msg = hintExecuting) =
  if execWithEcho(conf, cmd, msg) != 0:
    rawMessage(
      conf, errGenerated, "execution of an external program failed: '$1'" % cmd
    )
...
proc cFileSpecificOptions(conf: ConfigRef, nimname, fullNimFile: string): string =
  ...
  if optCDebug in conf.globalOptions:
    ...
    if existsConfigVar(conf, key):
      addOpt(result, getConfigVar(conf, key))
    else:
      addOpt(result, getDebug(conf, conf.cCompiler))
  if optOptimizeSpeed in conf.options:
    let key = nimname & ".speed"
    if existsConfigVar(conf, key):
      addOpt(result, getConfigVar(conf, key))
    else:
      addOpt(result, getOptSpeed(conf, conf.cCompiler))
  elif optOptimizeSize in conf.options:
    let key = nimname & ".size"
    if existsConfigVar(conf, key):
      addOpt(result, getConfigVar(conf, key))
    else:
      addOpt(result, getOptSize(conf, conf.cCompiler))
...
proc needsExeExt(conf: ConfigRef): bool {.inline.} =
  result =
    (optGenScript in conf.globalOptions and conf.target.targetOS == osWindows) or
...
proc getCompilerExe(conf: ConfigRef, compiler: TSystemCC, isCpp: bool): string =
  ...
  if result.len == 0:
    rawMessage(
      conf,
      errGenerated,
      "Compiler '$1' doesn't support the requested target" % CC[compiler].name,
    )
...
proc ccHasSaneOverflow*(conf: ConfigRef): bool =
  if conf.cCompiler == ccGcc:
    ...
    if exe.len == 0:
      exe = CC[conf.cCompiler].compilerExe
    # NOTE: should we need the full version, use -dumpfullversion
    let (s, exitCode) =
      try:
        execCmdEx(exe & " -dumpversion")
...
proc getLinkerExe(conf: ConfigRef, compiler: TSystemCC): string =
  result =
    if CC[compiler].linkerExe.len > 0:
      ...
      getCompilerExe(
        conf, compiler, optMixedMode in conf.globalOptions or conf.backend == backendCpp
      )

proc getCompileCFileCmd*(
    conf: ConfigRef, cfile: Cfile, isMainFile = false, produceOutput = false
): string =
  let
    c = conf.cCompiler
  ...
  var options =
    cFileSpecificOptions(conf, cfile.nimname, cfile.cname.changeFileExt("").string)
  if isCpp:
  ...
  if exe.len == 0:
    exe = getCompilerExe(conf, c, isCpp)
    ...
    if CC[conf.cCompiler].produceAsm.len > 0:
      ...
      if produceOutput:
        rawMessage(
          conf,
          hintUserRaw,
          "Couldn't produce assembler listing " & "for the selected C compiler: " &
            CC[conf.cCompiler].name,
        )
  ...
  strutils.addf(
    ...
    [
      ...
      includeCmd,
      "nim",
      quoteShell(getPrefixDir(conf)),
      "lib",
      quoteShell(conf.libpath),
      "vccplatform",
      vccplatform(conf),
      "ccenvflags",
      envFlags(conf),
...
proc footprint(conf: ConfigRef, cfile: Cfile): SecureHash =
  result = secureHash(
    $secureHashFile(cfile.cname.string) & platform.OS[conf.target.targetOS].name &
      platform.CPU[conf.target.targetCPU].name & extccomp.CC[conf.cCompiler].name &
      getCompileCFileCmd(conf, cfile)
  )

proc externalFileChanged(conf: ConfigRef, cfile: Cfile): bool =
  if conf.backend == backendJs:
    return false # pre-existing behavior, but not sure it's good
...
proc addExternalFileToCompile*(conf: ConfigRef, filename: AbsoluteFile) =
  var c = Cfile(
    nimname: splitFile(filename).name,
    cname: filename,
...
proc getLinkCmd(
    ...
    objfiles: string,
    isDllBuild: bool,
    removeStaticFile: bool,
): string =
  if optGenStaticLib in conf.globalOptions:
    if removeStaticFile:
      removeFile output # fixes: bug #16947
    result =
      CC[conf.cCompiler].buildLib % [
        "libfile",
        quoteShell(output),
        "objfiles",
        objfiles,
    ...
    if needsExeExt(conf):
      linkerExe = addFileExt(linkerExe, "exe")
    if noAbsolutePaths(conf):
      result = linkerExe
    else:
      result = joinPath(conf.cCompilerPath, linkerExe)
    let buildgui =
      if optGenGuiApp in conf.globalOptions and conf.target.targetOS == osWindows:
        CC[conf.cCompiler].buildGui
      else:
        ""
    let builddll =
      if isDllBuild:
        CC[conf.cCompiler].buildDll
      else:
        ""
    ...
    let mapfile =
      quoteShell(getNimcacheDir(conf) / RelativeFile(splitFile(output).name & ".map"))

    let linkOptions =
      getLinkOptions(conf) & " " & getConfigVar(conf, conf.cCompiler, ".options.linker")
    ...
    if conf.hcrOn and isVSCompatible(conf):
      let t = now()
      let pdb =
        output.string & "." & format(t, "MMMM-yyyy-HH-mm-") & $t.nanosecond & ".pdb"
      result.add " /link /PDB:" & pdb
  if optCDebug in conf.globalOptions and conf.cCompiler == ccVcc:
    result.add " /Zi /FS /Od"

template getLinkCmd(
    conf: ConfigRef, output: AbsoluteFile, objfiles: string, removeStaticFile = false
): string =
  getLinkCmd(
    conf, output, objfiles, optGenDynLib in conf.globalOptions, removeStaticFile
  )

template tryExceptOSErrorMessage(
    conf: ConfigRef, errorPrefix: string = "", body: untyped
) =
  try:
    body
  except OSError:
    ...
    if errorPrefix.len > 0:
      ...
      rawMessage(
        conf,
        errGenerated,
        "execution of an external program failed: '$1'" %
...
proc execCmdsInParallel(conf: ConfigRef, cmds: seq[string], prettyCb: proc(idx: int)) =
  let runCb = proc(idx: int, p: Process) =
    ...
    if exitCode != 0:
      rawMessage(
        conf,
        errGenerated,
        "execution of an external compiler program '" & cmds[idx] &
  ...
  if conf.numberOfProcessors <= 1:
    for i in 0 .. high(cmds):
      ...
      if res != 0:
        rawMessage(
          conf, errGenerated, "execution of an external program failed: '$1'" % cmds[i]
        )
  else:
    tryExceptOSErrorMessage(conf, "invocation of external compiler program failed."):
      res = execProcesses(
  ...
  if res != 0:
    if conf.numberOfProcessors <= 1:
      rawMessage(
        conf,
        errGenerated,
        "execution of an external program failed: '$1'" % cmds.join(),
      )
...
proc getObjFilePath(conf: ConfigRef, f: Cfile): string =
  if noAbsolutePaths(conf): f.obj.extractFilename else: f.obj.string
...
proc hcrLinkTargetName(conf: ConfigRef, objFile: string, isMain = false): AbsoluteFile =
  ...
  let targetName =
    if isMain:
      ...
      platform.OS[conf.target.targetOS].dllFrmt % basename
...
proc displayProgressCC(conf: ConfigRef, path, compileCmd: string): string =
  if conf.hasHint(hintCC):
    if optListCmd in conf.globalOptions or conf.verbosity > 1:
      result =
        MsgKindToStr[hintCC] %
        (demangleModuleName(path.splitFile.name) & ": " & compileCmd)
    else:
...
proc callCCompiler*(conf: ConfigRef) =
  ...
  if conf.globalOptions * {optCompileOnly, optGenScript} == {optCompileOnly}:
    return
      # speed up that call if only compiling and no script shall be
      # generated
  #var c = cCompiler
  var script: Rope = ""
  var cmds: TStringSeq
  ...
  for idx, it in conf.toCompile:
    ...
    if CfileFlag.Cached in it.flags:
      continue
    let compileCmd =
      getCompileCFileCmd(conf, it, idx == conf.toCompile.len - 1, produceOutput = true)
  ...
  if optNoLinking notin conf.globalOptions:
    ...
    for it in conf.externalToLink:
      ...
      objfiles.add(' ')
      objfiles.add(quoteShell(addFileExt(objFile, CC[conf.cCompiler].objExt)))
    ...
    if conf.hcrOn: # lets assume that optCompileOnly isn't on
      ...
      for idx, x in conf.toCompile:
        ...
        if CfileFlag.Cached in x.flags:
          continue
        ...
        cmds.add(
          getLinkCmd(
            conf,
            linkTarget,
            objfiles & " " & quoteShell(objFile),
            buildDll,
            removeStaticFile = true,
          )
        )
      ...
      let mainOutput =
        if optGenScript notin conf.globalOptions:
          conf.prepareToWriteOutput
        else:
          AbsoluteFile(conf.outFile)
...
template hashNimExe(): string =
  $secureHashFile(os.getAppFilename())
...
proc writeJsonBuildInstructions*(conf: ConfigRef, deps: StringTableRef) =
  var linkFiles = collect(
    for it in conf.externalToLink:
      var it = it
      if conf.noAbsolutePaths:
        it = it.extractFilename
      it.addFileExt(CC[conf.cCompiler].objExt)
  )
  for it in conf.toCompile:
    linkFiles.add it.obj.string
  var bcache = BuildCache(
    cacheVersion: cacheVersion,
    outputFile: conf.absOutFile.string,
    compile: collect(
      for i, it in conf.toCompile:
        if CfileFlag.Cached notin it.flags:
          (it.cname.string, getCompileCFileCmd(conf, it))
    ),
    link: linkFiles,
...
proc changeDetectedViaJsonBuildInstructions*(
    conf: ConfigRef, jsonFile: AbsoluteFile
): bool =
  if not fileExists(jsonFile) or not fileExists(conf.absOutFile):
    return true
  var bcache: BuildCache
  try:
  ...
  bcache.configFiles != conf.configFiles.mapIt(it.string) or
    ...
    return true
  if bcache.stdinInput or conf.projectIsStdin:
    return true # xxx optimize by returning false if stdin input was the same
  for (file, hash) in bcache.depfiles:
    if $secureHashFile(file) != hash:
      return true
...
proc writeMapping*(conf: ConfigRef, symbolMapping: Rope) =
  if optGenMapping notin conf.globalOptions:
    return
