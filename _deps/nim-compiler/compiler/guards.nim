...
import
  ast, astalgo, msgs, magicsys, nimsets, trees, types, renderer, idents, saturate,
  modulegraphs, options, lineinfos, int128
...
const
  someEq =
    {mEqI, mEqF64, mEqEnum, mEqCh, mEqB, mEqRef, mEqProc, mEqStr, mEqSet, mEqCString}

  # set excluded here as the semantics are vastly different:
  someLe = {mLeI, mLeF64, mLeU, mLeEnum, mLeCh, mLeB, mLePtr, mLeStr}
  someLt = {mLtI, mLtF64, mLtU, mLtEnum, mLtCh, mLtB, mLtPtr, mLtStr}
...
proc isValue(n: PNode): bool =
  n.kind in {nkCharLit .. nkNilLit}

proc isLocation(n: PNode): bool =
  not n.isValue
...
proc isLet(n: PNode): bool =
  if n.kind == nkSym:
    if n.sym.kind in {skLet, skTemp, skForVar}:
      result = true
    elif n.sym.kind == skParam and skipTypes(n.sym.typ, abstractInst).kind notin {tyVar}:
      result = true
...
proc `| div |`(a, b: PNode): PNode =
  result = copyNode(a)
  if a.kind in {nkCharLit .. nkUInt64Lit}:
    result.intVal = a.intVal div b.intVal
  else:
    result.floatVal = a.floatVal / b.floatVal
...
proc zero(): PNode =
  nkIntLit.newIntNode(0)

proc one(): PNode =
  nkIntLit.newIntNode(1)
...
proc highBound*(conf: ConfigRef, x: PNode, o: Operators): PNode =
  ...
  result =
    ...
    elif typ.kind == tySequence and x.kind == nkSym and x.sym.kind == skConst:
...
proc reassociation(n: PNode, o: Operators): PNode =
  ...
  of someAdd:
    if result[2].isValue and result[1].getMagic in someAdd and result[1][2].isValue:
      result = o.opAdd.buildCall(result[1][1], result[1][2] |+| result[2])
      if result[2].intVal == 0:
        result = result[1]
  of someMul:
    if result[2].isValue and result[1].getMagic in someMul and result[1][2].isValue:
...
proc canon*(n: PNode, o: Operators): PNode =
  if n.safeLen >= 1:
    ...
    for i in 0 ..< n.len:
      ...
      n.sym.astdef.getMagic in (
        someEq + someAdd + someMul + someMin + someMax + someHigh + someSub + someLen +
        someDiv
      ):
  ...
  of someLt - {mLtF64}:
    ...
    let minus =
      if p != y:
        ...
        o.opAdd.buildCall(y, minusOne()).canon(o)
  ...
  of someLe:
    ...
    if x.kind in nkCallKinds and x.len == 3 and x[2].isValue and
      ...
      of someSub:
        result =
          buildCall(result[0].sym, x[1], reassociation(o.opAdd.buildCall(y, x[2]), o))
      of someAdd:
        # Rule A:
        let plus = negate(y, x[2], nil, o).reassociation(o)
        if plus != nil:
          result = buildCall(result[0].sym, x[1], plus)
      else:
        discard
      ...
      of someSub:
        result =
          buildCall(result[0].sym, y[1], reassociation(o.opAdd.buildCall(x, y[2]), o))
      of someAdd:
...
proc usefulFact(n: PNode, o: Operators): PNode =
  case n.getMagic
  of someEq:
    if skipConv(n[2]).kind == nkNilLit and (isLetLocation(n[1], false) or isVar(n[1])):
...
proc sameOpr(a, b: PSym): bool =
  case a.magic
  of someEq:
    result = b.magic in someEq
  of someLe:
    result = b.magic in someLe
  of someLt:
    result = b.magic in someLt
  of someLen:
    result = b.magic in someLen
  of someAdd:
    result = b.magic in someAdd
  of someSub:
    result = b.magic in someSub
  of someMul:
    result = b.magic in someMul
  of someDiv:
    result = b.magic in someDiv
  else:
    result = a == b
...
proc invalidateFacts*(s: var seq[PNode], n: PNode) =
  ...
  for i in 0 .. high(s):
    if s[i] != nil and s[i].hasSubTree(n):
      s[i] = nil
...
proc impliesEq(fact, eq: PNode): TImplication =
  ...
  of someEq:
    if sameTree(fact[1], eq[loc]):
      ...
      if sameTree(fact[2], eq[val]):
        result = impYes
      elif valuesUnequal(fact[2], eq[val]):
        result = impNo
    elif sameTree(fact[2], eq[loc]):
      if sameTree(fact[1], eq[val]):
        result = impYes
      elif valuesUnequal(fact[1], eq[val]):
        result = impNo
  of mInSet:
    # remember: mInSet is 'contains' so the set comes first!
    if sameTree(fact[2], eq[loc]) and isValue(eq[val]):
      if inSet(fact[1], eq[val]):
        result = impYes
      else:
        result = impNo
  of mNot, mOr, mAnd:
...
proc compareSets(a, b: PNode): TImplication =
  if equalSets(nil, a, b):
    result = impYes
  elif intersectSets(nil, a, b).len == 0:
    result = impNo
...
proc impliesIn(fact, loc, aSet: PNode): TImplication =
  ...
  of someEq:
    if sameTree(fact[1], loc):
      if inSet(aSet, fact[2]):
        result = impYes
      else:
        result = impNo
    elif sameTree(fact[2], loc):
      if inSet(aSet, fact[1]):
        result = impYes
      else:
        result = impNo
  of mInSet:
...
proc valueIsNil(n: PNode): TImplication =
  if n.kind == nkNilLit:
    impYes
  elif n.kind in {nkStrLit .. nkTripleStrLit, nkBracket, nkObjConstr}:
    impNo
  else:
    impUnknown
...
proc factImplies(fact, prop: PNode): TImplication =
  ...
  of mNot:
    result = ~fact.factImplies(prop[1])
  of mIsNil:
    result = impliesIsNil(fact, prop)
  of someEq:
    result = impliesEq(fact, prop)
  of someLe:
    result = impliesLe(fact, prop[1], prop[2])
  of someLt:
    result = impliesLt(fact, prop[1], prop[2])
  of mInSet:
    result = impliesIn(fact, prop[2], prop[1])
  else:
    result = impUnknown
...
template isMul(x): untyped =
  x.getMagic in someMul

template isDiv(x): untyped =
  x.getMagic in someDiv

template isAdd(x): untyped =
  x.getMagic in someAdd

template isSub(x): untyped =
  x.getMagic in someSub

template isVal(x): untyped =
  x.kind in {nkCharLit .. nkUInt64Lit}

template isIntVal(x, y): untyped =
  x.intVal == y
...
macro `=~`(x: PNode, pat: untyped): bool =
  proc m(x, pat, conds: NimNode) =
    ...
    of nnkInfix:
      case $pat[0]
      of "*":
        conds.add getAst(isMul(x))
      of "/":
        conds.add getAst(isDiv(x))
      of "+":
        conds.add getAst(isAdd(x))
      of "-":
        conds.add getAst(isSub(x))
      else:
...
proc ple(m: TModel, a, b: PNode): TImplication =
  ...
  template `>=?`(a, b): untyped =
    ple(m, nkIntLit.newIntNode(b), a) == impYes
  ...
  if b.isValue and a.typ != nil and a.typ.isOrdinalType:
    if lastOrd(nil, a.typ) <= b.intVal:
      return impYes
  # 3 <= x   iff  low(x) <= 3
  if a.isValue and b.typ != nil and b.typ.isOrdinalType:
    if a.intVal <= firstOrd(nil, b.typ):
      return impYes
  ...
  if b.getMagic in someAdd:
    if zero() <=? b[2] and a <=? b[1]:
      return impYes
    # x <= y-c  if x+c <= y
    if b[2] <=? zero() and
        (canon(m.g.operators.opSub.buildCall(a, b[2]), m.g.operators) <=? b[1]):
      return impYes

  #   x+c <= y  if c <= 0 and x <= y
  if a.getMagic in someAdd and a[2] <=? zero() and a[1] <=? b:
    return impYes

  #   x <= y*c  if  1 <= c and x <= y  and 0 <= y
  if b.getMagic in someMul:
    if a <=? b[1] and one() <=? b[2] and zero() <=? b[1]:
      return impYes

  if a.getMagic in someMul and a[2].isValue and a[1].getMagic in someDiv and
      a[1][2].isValue:
    # simplify   (x div 4) * 2 <= y   to  x div (c div d)  <= y
    if ple(m, buildCall(m.g.operators.opDiv, a[1][1], `| div |`(a[1][2], a[2])), b) ==
        impYes:
      return impYes
...
proc applyReplacements(n: PNode, rep: TReplacements): PNode =
  result = n
  for x in rep:
    result = result.replaceSubTree(x.a, x.b)
...
proc proveLe*(m: TModel, a, b: PNode): TImplication =
  ...
  if result == impUnknown:
    ...
    let y = canon(
      m.g.operators.opLe.buildCall(m.g.operators.opAdd.buildCall(b, one()), a),
      m.g.operators,
    )
...
proc checkFieldAccess*(m: TModel, n: PNode, conf: ConfigRef, produceError: bool) =
  for i in 1 ..< n.len:
    ...
    if check != nil and m.doesImply(check) != impYes:
      if produceError:
        localError(
          conf, n.info, "field access outside of valid case branch: " & renderTree(n[0])
        )
      else:
