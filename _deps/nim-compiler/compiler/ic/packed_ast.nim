...
const nilItemId* = PackedItemId(module: LitId(0), item: -1.int32)
...
type
  ...
  PackedNode* = object # 28 bytes
    ...
    flags*: TNodeFlags
    operand*: int32
      # for kind in {nkSym, nkSymDef}: SymId
...
when false:
  proc declareSym*(
      tree: var PackedTree, kind: TSymKind, name: LitId, info: PackedLineInfo
  ): SymId =
    result = SymId(tree.sh.syms.len)
    tree.sh.syms.add PackedSym(
      kind: kind, name: name, flags: {}, magic: mNone, info: info
    )
  ...
  proc add*(
      tree: var PackedTree, kind: TNodeKind, token: string, info: PackedLineInfo
  ) =
    tree.nodes.add PackedNode(
      kind: kind, info: info, operand: int32 getOrIncl(tree.sh.strings, token)
    )
...
proc prepare*(
  ...
  result = PatchPos tree.nodes.len
  tree.nodes.add PackedNode(
    kind: kind, flags: flags, operand: 0, info: info, typeId: typeId
  )
...
proc len*(tree: PackedTree): int {.inline.} =
  tree.nodes.len
...
template symId*(n: NodePos): SymId =
  SymId tree.nodes[n.int].operand

proc firstSon*(n: NodePos): NodePos {.inline.} =
  NodePos(n.int + 1)
...
const
  externIntLit* = {
    nkCharLit, nkIntLit, nkInt8Lit, nkInt16Lit, nkInt64Lit, nkUIntLit, nkUInt8Lit,
    nkUInt16Lit, nkUInt32Lit, nkUInt64Lit,
...
when false:
  ...
  template identId*(n: NodePos): LitId =
    identIdImpl(tree, n)
...
proc getNodeId*(tree: PackedTree): NodeId {.inline.} =
  NodeId tree.nodes.len
