...
import ast, types, renderer, intsets
...
proc canAlias(arg, ret: PType, marker: var IntSet): bool =
  ...
  of tyTuple:
    for i in 0 ..< ret.len:
      ...
      if result:
        break
  of tyArray, tySequence, tyDistinct, tyGenericInst, tyAlias, tyInferred, tySink,
      tyLent, tyOwned, tyRef:
...
proc isValueOnlyType(t: PType): bool =
  ...
  proc wrap(t: PType): bool {.nimcall.} =
    t.kind in {tyRef, tyPtr, tyVar, tyLent}
...
proc containsDangerousRefAux(t: PType, marker: var IntSet): SearchResult =
  ...
  of tyObject:
    ...
    if result == NotFound:
      result = containsDangerousRefAux(t.n, marker)
...
const SomeVar = {skForVar, skParam, skVar, skLet, skConst, skResult, skTemp}
    ...
    of nkCallKinds:
      ...
      for i in 1 ..< n.len:
        if checkIsolate(n[i]):
          ...
          let argType = n[i].typ
          if argType != nil and not isCompileTimeOnly(argType) and
              containsDangerousRef(argType):
