...
const Letters* = {'a' .. 'z', 'A' .. 'Z', '0' .. '9', '\x80' .. '\xFF', '_'}
...
proc beautifyName(s: string, k: TSymKind): string =
  ...
  if allUpper and k in {skConst, skEnumField, skType}:
    return s
  ...
  of skType, skGenericParam:
    ...
    if s =~ [
      "int", "uint", "cint", "cuint", "clong", "cstring", "string", "char", "byte",
      "bool", "openArray", "seq", "array", "void", "pointer", "float", "csize",
      "csize_t", "cdouble", "cchar", "cschar", "cshort", "cu", "nil", "typedesc",
      "auto", "any", "range", "openarray", "varargs", "set", "cfloat", "ref", "ptr",
  ...
  of skConst, skEnumField:
    ...
    if s.len == 1 and s[0] == 'L':
      result.add 'L'
    elif '_' in s:
      result.add(s[i])
    else:
      result.add toLowerAscii(s[0])
  inc i
  while i < s.len:
    if s[i] == '_':
      if i + 1 >= s.len:
...
proc differ*(line: string, a, b: int, x: string): string =
  proc substrEq(s: string, pos, last: int, substr: string): bool =
    ...
    for i in 0 ..< substr.len:
      if pos + i > last or s[pos + i] != substr[i]:
        return false
...
template styleCheckDef*(ctx: PContext, info: TLineInfo, sym: PSym, k: TSymKind) =
  ...
  {optStyleHint, optStyleError} * ctx.config.globalOptions != {} and
    # check only if hint/error is enabled
  hintName in ctx.config.notes and # ignore if name checks are not requested
  ctx.config.belongsToProjectPackage(sym) and # ignore foreign packages
  optStyleUsages notin ctx.config.globalOptions and
    # ignore if requested to only check name usage
...
proc differs(conf: ConfigRef, info: TLineInfo, newName: string): string =
  ...
  while first > 0 and line[first - 1] in Letters:
    dec first
  if first < 0:
    return
  if first + 1 < line.len and line[first] == '`':
    inc first
...
template styleCheckUse*(ctx: PContext, info: TLineInfo, sym: PSym) =
  ...
  if {optStyleHint, optStyleError} * ctx.config.globalOptions != {} and
    # ignore if styleChecks are off
  hintName in ctx.config.notes and # ignore if name checks are not requested
  ctx.config.belongsToProjectPackage(sym) and # ignore foreign packages
  sym.kind != skTemp and # ignore temporary variables created by the compiler
  sym.name.s[0] in Letters and # ignore operators TODO: what about unicode symbols???
...
proc checkPragmaUseImpl(
    conf: ConfigRef, info: TLineInfo, w: TSpecialWord, pragmaName: string
) =
  let wanted = $w
  if pragmaName != wanted:
    lintReport(conf, info, wanted, pragmaName)

template checkPragmaUse*(
    ctx: PContext, info: TLineInfo, w: TSpecialWord, pragmaName: string, sym: PSym
) =
