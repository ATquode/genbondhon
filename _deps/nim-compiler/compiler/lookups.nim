...
import
  intsets, ast, astalgo, idents, semdata, types, msgs, options, renderer, lineinfos,
  modulegraphs, astmsgs, sets, wordrecg
...
proc openScope*(c: PContext): PScope {.discardable.} =
  result =
    PScope(parent: c.currentScope, symbols: newStrTable(), depthLevel: c.scopeDepth + 1)
...
proc searchInScopesAllCandidatesFilterBy*(
    c: PContext, s: PIdent, filter: TSymKinds
): seq[PSym] =
  result = @[]
  for scope in allScopes(c.currentScope):
    var ti: TIdentIter
...
proc openShadowScope*(c: PContext) =
  ...
  c.currentScope =
    PScope(parent: c.currentScope, symbols: newStrTable(), depthLevel: c.scopeDepth)
...
proc errorUseQualifier(
    c: PContext, info: TLineInfo, candidates: seq[PSym], prefix = "use one of"
) =
  var err = "ambiguous identifier: '" & candidates[0].name.s & "'"
  var i = 0
  for candidate in candidates:
    if i == 0:
      err.add " -- $1 the following:\n" % prefix
    else:
      err.add "\n"
    err.add "  " & candidate.owner.name.s & "." & candidate.name.s
    err.add ": " & typeToString(candidate.typ)
