...
const genPrefix* = ":tmp" # prefix for generated names

import ast, astalgo, types, idents, magicsys, msgs, options, modulegraphs, lineinfos
...
proc newTupleAccess*(g: ModuleGraph, tup: PNode, i: int): PNode =
  if tup.kind == nkHiddenAddr:
    result = newNodeIT(
      nkHiddenAddr, tup.info, tup.typ.skipTypes(abstractInst + {tyPtr, tyVar, tyLent})
    )
    result.add newNodeIT(
      nkBracketExpr,
      tup.info,
      tup.typ.skipTypes(abstractInst + {tyPtr, tyVar, tyLent})[i],
    )
    result[0].add tup[0]
    ...
    result = newNodeIT(nkBracketExpr, tup.info, tup.typ.skipTypes(abstractInst)[i])
...
proc lowerTupleUnpacking*(
    g: ModuleGraph, n: PNode, idgen: IdGenerator, owner: PSym
): PNode =
  ...
  if avoidTemp:
    ...
    var temp = newSym(
      skTemp, getIdent(g.cache, genPrefix), idgen, owner, value.info, g.config.options
    )
  ...
  for i in 0 ..< n.len - 2:
    ...
    if n[i].kind == nkSym:
      ...
      result.add newAsgnStmt(n[i], val)
...
proc lowerTupleUnpackingForAsgn*(
    g: ModuleGraph, n: PNode, idgen: IdGenerator, owner: PSym
): PNode =
  let value = n.lastSon
  result = newNodeI(nkStmtList, n.info)
  ...
  var temp =
    newSym(skTemp, getIdent(g.cache, "_"), idgen, owner, value.info, owner.options)
  var v = newNodeI(nkLetSection, value.info)
  let tempAsNode = newSymNode(temp)
    #newIdentNode(getIdent(genPrefix & $temp.id), value.info)
...
proc lowerSwap*(g: ModuleGraph, n: PNode, idgen: IdGenerator, owner: PSym): PNode =
  ...
  var temp =
    newSym(skVar, getIdent(g.cache, genPrefix), idgen, owner, n.info, owner.options)
  temp.typ = n[1].typ
  incl(temp.flags, sfFromGeneric)
...
proc createObj*(
    g: ModuleGraph, idgen: IdGenerator, owner: PSym, info: TLineInfo, final = true
): PType =
  ...
  let s = newSym(
    skType,
    getIdent(g.cache, "Env_" & toFilename(g.config, info) & "_" & $owner.name.s),
    idgen,
    owner,
    info,
    owner.options,
  ...
  of nkRecCase:
    ...
    for i in 1 ..< n.len:
      ...
      of nkOfBranch, nkElse:
        ...
        discard
  of nkSym:
    if n.sym.itemId.module == id.module and n.sym.itemId.item == -abs(id.item):
      result = n.sym
  else:
    discard
...
proc addField*(obj: PType, s: PSym, cache: IdentCache, idgen: IdGenerator): PSym =
  ...
  var field = newSym(
    skField, getIdent(cache, s.name.s & $obj.n.len), idgen, s.owner, s.info, s.options
  )
...
proc addUniqueField*(
    obj: PType, s: PSym, cache: IdentCache, idgen: IdGenerator
): PSym {.discardable.} =
...
proc genDeref*(n: PNode, k = nkHiddenDeref): PNode =
  result = newNodeIT(k, n.info, n.typ.skipTypes(abstractInst)[0])
  result.add n

proc callCodegenProc*(
    g: ModuleGraph,
    name: string,
...
proc genHigh*(g: ModuleGraph, n: PNode): PNode =
  if skipTypes(n.typ, abstractVar).kind == tyArray:
    result =
      newIntLit(g, n.info, toInt64(lastOrd(g.config, skipTypes(n.typ, abstractVar))))
  else:
...
proc genLen*(g: ModuleGraph, n: PNode): PNode =
  if skipTypes(n.typ, abstractVar).kind == tyArray:
    result = newIntLit(
      g, n.info, toInt64(lastOrd(g.config, skipTypes(n.typ, abstractVar)) + 1)
    )
  else:
    result = newNodeI(nkCall, n.info, 2)
    result.typ = getSysType(g, n.info, tyInt)
