...
import
  llstream, commands, os, strutils, msgs, lexer, ast, options, idents, wordrecg,
  strtabs, lineinfos, pathutils, scriptconfig
...
proc ppGetTok(L: var Lexer, tok: var Token) =
  # simple filter
  rawGetTok(L, tok)
  while tok.tokType in {tkComment}:
    rawGetTok(L, tok)
...
proc evalppIf(L: var Lexer, tok: var Token, config: ConfigRef): bool =
  ...
  if tok.tokType == tkColon:
    ...
    lexMessage(L, errGenerated, "expected ':'")

#var condStack: seq[bool] = @[]

proc doEnd(L: var Lexer, tok: var Token, condStack: var seq[bool]) =
  if high(condStack) < 0:
    lexMessage(L, errGenerated, "expected @if")
...
proc parseDirective(
    L: var Lexer, tok: var Token, config: ConfigRef, condStack: var seq[bool]
) =
  ppGetTok(L, tok) # skip @
  case whichKeyword(tok.ident)
  of wIf:
  ...
  of wWrite:
    ppGetTok(L, tok)
    msgs.msgWriteln(
      config, strtabs.`%`($tok, config.configVars, {useEnvironment, useKey})
    )
    ppGetTok(L, tok)
  else:
...
proc parseAssignment(
  ...
  if percent:
    processSwitch(
      s,
      strtabs.`%`(val, config.configVars, {useEnvironment, useEmpty}),
      passPP,
      info,
      config,
    )
...
proc readConfigFile*(
    filename: AbsoluteFile, cache: IdentCache, config: ConfigRef
): bool =
  var
    L: Lexer
    tok: Token
  ...
  if stream != nil:
    ...
    while tok.tokType != tkEof:
      parseAssignment(L, tok, config, condStack)
    if condStack.len > 0:
      lexMessage(L, errGenerated, "expected @end")
    closeLexer(L)
    return true
...
proc getSystemConfigPath*(conf: ConfigRef, filename: RelativeFile): AbsoluteFile =
  ...
  when defined(unix):
    if not fileExists(result):
      result = p / RelativeDir"etc/nim" / filename
    if not fileExists(result):
      result = AbsoluteDir"/etc/nim" / filename

proc loadConfigs*(
    cfg: RelativeFile, cache: IdentCache, conf: ConfigRef, idgen: IdGenerator
) =
  ...
  let pd =
    if not conf.projectPath.isEmpty:
      conf.projectPath
    else:
