...
import ast, astalgo, lineinfos, bitsets, types, options
...
proc inSet*(s: PNode, elem: PNode): bool =
  ...
  for i in 0 ..< s.len:
    if s[i].kind == nkRange:
      if leValue(s[i][0], elem) and leValue(elem, s[i][1]):
        return true
    else:
...
proc overlap*(a, b: PNode): bool =
  if a.kind == nkRange:
    if b.kind == nkRange:
      ...
      result = leValue(a[0], b[1]) and leValue(b[0], a[1])
    else:
...
proc toTreeSet*(conf: ConfigRef, s: TBitSet, settype: PType, info: TLineInfo): PNode =
  ...
  while e < s.len * ElemSize:
    if bitSetIn(s, e):
      ...
      while true:
        inc(b)
        if (b >= s.len * ElemSize) or not bitSetIn(s, b):
          break
      dec(b)
