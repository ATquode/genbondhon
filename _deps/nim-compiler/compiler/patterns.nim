...
import ast, types, semdata, sigmatch, idents, aliases, parampatterns, trees
...
proc matchNested(c: PPatternContext, p, n: PNode, rpn: bool): bool =
  ...
  proc matchStarAux(c: PPatternContext, op, n, arglist: PNode, rpn: bool): bool =
    result = true
    if n.kind in nkCallKinds and matches(c, op[1], n[0]):
      for i in 1 ..< n.len:
        if not matchStarAux(c, op, n[i], arglist, rpn):
          return false
      if rpn:
        arglist.add(n[0])
...
proc matches(c: PPatternContext, p, n: PNode): bool =
  ...
  if nfNoRewrite in n.flags:
    ...
    case opr
    of "|":
      result = matchChoice(c, p, n)
    of "*":
      result = matchNested(c, p, n, rpn = false)
    of "**":
      result = matchNested(c, p, n, rpn = true)
    of "~":
      result = not matches(c, p[1], n)
    else:
    ...
    of nkSym:
      result = p.sym == n.sym
    of nkIdent:
      result = p.ident.id == n.ident.id
    of nkCharLit .. nkInt64Lit:
      result = p.intVal == n.intVal
    of nkFloatLit .. nkFloat64Lit:
      result = p.floatVal == n.floatVal
    of nkStrLit .. nkTripleStrLit:
      result = p.strVal == n.strVal
