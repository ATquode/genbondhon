...
type TSystemOS* = enum
  # Also add OS in initialization section and alias
...
const OS*: array[succ(low(TSystemOS)) .. high(TSystemOS), TInfoOS] = [
  (
    name: "DOS",
    parDir: "..",
    dllFrmt: "$1.dll",
...
type TSystemCPU* = enum
  # Also add CPU for in initialization section and
...
type TInfoCPU* =
  tuple[name: string, intSize: int, endian: Endianness, floatSize, bit: int]
...
const
  ...
  CPU*: array[succ(low(TSystemCPU)) .. high(TSystemCPU), TInfoCPU] = [
    ...
    (name: "amd64", intSize: 64, endian: littleEndian, floatSize: 64, bit: 64),
      # a.k.a. x86_64, covers both amd and intel
