import
  intsets, ast, idents, algorithm, renderer, strutils, msgs, modulegraphs, syntaxes,
  options, modulepaths, lineinfos
...
proc hasForbiddenPragma(n: PNode): bool =
  ...
  for a in n:
    if a.kind == nkPragma and a[0].kind == nkIdent and a[0].ident.s == "push":
      return true
