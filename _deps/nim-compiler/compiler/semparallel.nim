...
import
  ast, astalgo, idents, lowerings, magicsys, guards, msgs, renderer, types,
  modulegraphs, options, spawn, lineinfos
...
type
  ...
  MonotonicVar = object
    v, alias: PSym
      # to support the ordinary 'countup' iterator
  ...
  AnalysisCtx = object
    ...
    spawns: int
      # we can check that at last 1 spawn is used in
...
proc lookupSlot(c: AnalysisCtx, s: PSym): int =
  for i in 0 ..< c.locals.len:
    if c.locals[i].v == s or c.locals[i].alias == s:
      return i
  return -1
...
proc isSingleAssignable(n: PNode): bool =
  n.kind == nkSym and (
    let s = n.sym
    s.kind in {skTemp, skForVar, skLet} and {sfAddrTaken, sfGlobal} * s.flags == {}
  )
...
proc checkLe(c: AnalysisCtx, a, b: PNode) =
  ...
  of impUnknown:
    message(
      c.graph.config, a.info, warnStaticIndexCheck, "cannot prove: " & ?a & " <= " & ?b
    )
  of impYes:
    discard
  of impNo:
    message(
      c.graph.config, a.info, warnStaticIndexCheck, "can prove: " & ?a & " > " & ?b
    )
...
proc checkSlicesAreDisjoint(c: var AnalysisCtx) =
  ...
  for x, a, b, id, inLoop in items(c.slices):
    if inLoop:
      overlap(c.guards, c.graph.config, a, b, c.subStride(a), c.subStride(b))
    ...
    for j in i + 1 .. high(c.slices):
      ...
      if x.spawnId != y.spawnId and guards.sameTree(x.x, y.x):
        if not x.inLoop or not y.inLoop:
          ...
          let k = simpleSlice(x.a, x.b)
          let m = simpleSlice(y.a, y.b)
          k >= 0 and m >= 0
        ):
...
proc analyse(c: var AnalysisCtx, n: PNode) =
  ...
  of nkTypeSection, nkProcDef, nkConverterDef, nkMethodDef, nkIteratorDef, nkMacroDef,
      nkTemplateDef, nkConstSection, nkPragma, nkFuncDef, nkMixinStmt, nkBindStmt,
      nkExportStmt:
    discard
  else:
    analyseSons(c, n)
...
proc transformSpawn(
  g: ModuleGraph, idgen: IdGenerator, owner: PSym, n, barrier: PNode
): PNode

proc transformSpawnSons(
    g: ModuleGraph, idgen: IdGenerator, owner: PSym, n, barrier: PNode
): PNode =
  result = shallowCopy(n)
  for i in 0 ..< n.len:
...
proc transformSpawn(
  ...
  of nkVarSection, nkLetSection:
    ...
    for it in n:
      ...
      if getMagic(b) == mSpawn:
        if it.len != 3:
          localError(g.config, it.info, "invalid context for 'spawn'")
  ...
  of nkAsgn, nkFastAsgn, nkSinkAsgn:
    let b = n[1]
    if getMagic(b) == mSpawn and
        (let t = b[1][0].typ[0]; spawnResult(t, true) == srByVar):
      let m = transformSlices(g, idgen, b)
