...
const
  ...
  errSetTooBig =
    "set is too large; use `std/sets` for ordinal types with more than 2^16 elements"
  ...
  for i in 1 ..< n.len:
    ...
    of nkEnumFieldDef:
          ...
          if skipTypes(strVal.typ, abstractInst).kind in {tyString, tyCstring}:
            if not isOrdinalType(v[0].typ, allowEnumWithHoles = true):
              localError(
                c.config,
                v[0].info,
                errOrdinalTypeExpected % typeToString(v[0].typ, preferDesc),
              )
      ...
      of tyString, tyCstring:
        ...
        if not isOrdinalType(v.typ, allowEnumWithHoles = true):
          localError(
            c.config, v.info, errOrdinalTypeExpected % typeToString(v.typ, preferDesc)
          )
    ...
    if counter == high(typeof(counter)):
      if i > 1 and result.n[i - 2].sym.position == high(int):
        localError(
          c.config, n[i].info, errOverflowInEnumX % [e.name.s, $high(typeof(counter))]
        )
    else:
      inc(counter)
...
proc semSet(c: PContext, n: PNode, prev: PType): PType =
  ...
  if n.len == 2 and n[1].kind != nkEmpty:
    ...
    if base.kind in {tyGenericInst, tyAlias, tySink}:
      base = lastSon(base)
    if base.kind notin {tyGenericParam, tyGenericInvocation}:
      if base.kind == tyForward:
        ...
        localError(
          c.config, n.info, errOrdinalTypeExpected % typeToString(base, preferDesc)
        )
...
proc semContainer(
    c: PContext, n: PNode, kind: TTypeKind, kindStr: string, prev: PType
): PType =
...
proc addSonSkipIntLitChecked(
    c: PContext, father, son: PType, it: PNode, id: IdGenerator
) =
...
proc semDistinct(c: PContext, n: PNode, prev: PType): PType =
  if n.len == 0:
    return newConstraint(c, tyDistinct)
...
proc semRangeAux(c: PContext, n: PNode, prev: PType): PType =
  ...
  if not hasUnknownTypes:
    if not sameType(rangeT[0].skipTypes({tyRange}), rangeT[1].skipTypes({tyRange})):
      ...
      localError(
        c.config,
        n.info,
        "ordinal or float type expected, but got " & typeToString(rangeT[0]),
      )
  ...
  for i in 0 .. 1:
    ...
    if result.n[i].kind in {nkFloatLit .. nkFloat64Lit} and result.n[i].floatVal.isNaN:
      localError(
        c.config, n.info, "NaN is not a valid range " & (if i == 0: "start" else: "end")
      )
    ...
    if isRange(n[1]):
      ...
      if not isDefined(c.config, "nimPreviewRangeDefault"):
        ...
        if n[0].kind in {nkCharLit .. nkUInt64Lit} and n[0].intVal > 0:
          ...
          incl(result.flags, tfRequiresInit)
        elif n[0].kind in {nkFloatLit .. nkFloat64Lit} and n[0].floatVal > 0.0:
          incl(result.flags, tfRequiresInit)
        elif n[1].kind in {nkFloatLit .. nkFloat64Lit} and n[1].floatVal < 0.0:
          incl(result.flags, tfRequiresInit)
    else:
...
proc semArrayIndex(c: PContext, n: PNode): PType =
  if isRange(n):
    ...
    if e.typ.kind == tyFromExpr:
      ...
      if e.intVal < 0:
        localError(
          c.config, n.info, "Array length can't be negative, but was " & $e.intVal
        )
      ...
      if e.typ.lastSon.kind != tyGenericParam and not isOrdinalType(e.typ.lastSon):
        let info =
          if n.safeLen > 1:
            ...
            n.info
        localError(
          c.config, info, errOrdinalTypeExpected % typeToString(e.typ, preferDesc)
        )
      ...
      if not isOrdinalType(e.typ.skipTypes({tyStatic, tyAlias, tyGenericInst, tySink})):
        localError(
          c.config, n[1].info, errOrdinalTypeExpected % typeToString(e.typ, preferDesc)
        )
      ...
      if x.kind in {nkIntLit .. nkUInt64Lit}:
        result =
          makeRangeType(c, 0, x.intVal - 1, n.info, x.typ.skipTypes({tyTypeDesc}))
      else:
...
proc semArray(c: PContext, n: PNode, prev: PType): PType =
  ...
  if n.len == 3:
    ...
    var indxB = indx
    if indxB.kind in {tyGenericInst, tyAlias, tySink}:
      indxB = lastSon(indxB)
    if indxB.kind notin {tyGenericParam, tyStatic, tyFromExpr} and
      ...
      if indxB.skipTypes({tyRange}).kind in {tyUInt, tyUInt64}:
        ...
        localError(
          c.config, n[1].info, errOrdinalTypeExpected % typeToString(indxB, preferDesc)
        )
      elif enumHasHoles(indxB):
        localError(
          c.config,
          n[1].info,
          "enum '$1' has holes" % typeToString(indxB.skipTypes({tyRange})),
        )
  ...
  if n.len == 2:
    ...
    if base.kind != tyGenericParam:
      if not isOrdinalType(base):
        localError(
          c.config, n[1].info, errOrdinalTypeExpected % typeToString(base, preferDesc)
        )
    addSonSkipIntLit(result, base, c.idgen)
  else:
  ...
  for i in ord(n.kind == nkBracketExpr) ..< n.len:
    ...
    if hasDefaultField:
      ...
      if c.graph.config.isDefined("nimPreviewRangeDefault") and
          typ.skipTypes(abstractInst).kind == tyRange:
...
proc semIdentWithPragma(
    c: PContext, kind: TSymKind, n: PNode, allowed: TSymFlags
): PSym =
  if n.kind == nkPragmaExpr:
    ...
    of skType:
      ...
      discard
    of skField:
      pragma(c, result, n[1], fieldPragmas)
    of skVar:
      pragma(c, result, n[1], varPragmas)
    of skLet:
      pragma(c, result, n[1], letPragmas)
    of skConst:
      pragma(c, result, n[1], constPragmas)
    else:
      discard
  else:
...
proc checkForOverlap(c: PContext, t: PNode, currentEx, branchIndex: int) =
  ...
  for i in 1 .. branchIndex:
    for j in 0 ..< t[i].len - 1:
      if i == branchIndex and j == currentEx:
        break
...
proc semBranchRange(c: PContext, t, a, b: PNode, covered: var Int128): PNode =
  ...
  result.add(bt)
  if emptyRange(ac, bc):
    localError(c.config, b.info, "range is empty")
  else:
    covered = covered + getOrdValue(bc) + 1 - getOrdValue(ac)

proc semCaseBranchRange(c: PContext, t, b: PNode, covered: var Int128): PNode =
  checkSonsLen(b, 3, c.config)
  result = semBranchRange(c, t, b[1], b[2], covered)

proc semCaseBranchSetElem(c: PContext, t, b: PNode, covered: var Int128): PNode =
  if isRange(b):
    checkSonsLen(b, 3, c.config)
    result = semBranchRange(c, t, b[1], b[2], covered)
  elif b.kind == nkRange:
...
proc semCaseBranch(
    c: PContext, t, branch: PNode, branchIndex: int, covered: var Int128
) =
...
proc semRecordCase(
  ...
  const shouldChckCovered =
    {tyInt .. tyInt64, tyChar, tyEnum, tyUInt .. tyUInt32, tyBool}
  case typ.kind
  ...
  if firstOrd(c.config, typ) != 0:
    localError(
      c.config, n.info, "low(" & $a[0].sym.name.s & ") must be 0 for discriminant"
    )
  ...
  if chckCovered and covered != toCover(c, a[0].typ):
    if a[0].typ.skipTypes(abstractRange).kind == tyEnum:
      localError(
        c.config,
        a.info,
        "not all cases are covered; missing: $1" % formatMissingEnums(c, a),
      )
    else:
...
proc semRecordNodeAux(
    ...
    for i in 0 ..< a.len:
      ...
      of nkElifBranch:
        ...
        if c.inGenericContext == 0:
          ...
          if e.kind != nkIntLit:
            discard "don't report followup error"
          elif e.intVal != 0 and branch == nil:
            branch = it[1]
        else:
          # XXX this is still a hard compilation in a generic context, this can
          # result in unresolved generic parameters being treated like real types
  ...
  of nkIdentDefs:
    ...
    var a: PNode
    if father.kind != nkRecList and n.len >= 4:
      a = newNodeI(nkRecList, n.info)
    else:
      a = newNodeI(nkEmpty, n.info)
    var typ: PType
    ...
    if hasDefaultField:
      ...
      if c.graph.config.isDefined("nimPreviewRangeDefault") and
        ...
        hasDefaultField = true
      propagateToOwner(rectype, typ)
    var fieldOwner = if c.inGenericContext > 0: c.getCurrOwner else: rectype.sym
    for i in 0 ..< n.len - 2:
      ...
      if fieldOwner != nil and {sfImportc, sfExportc} * fieldOwner.flags != {} and
...
proc addInheritedFieldsAux(c: PContext, check: var IntSet, pos: var int, n: PNode) =
  case n.kind
  of nkRecCase:
    if (n[0].kind != nkSym):
...
proc skipGenericInvocation(t: PType): PType {.inline.} =
  ...
  if result.kind == tyGenericInvocation:
    result = result[0]
  while result.kind in
      {tyGenericInst, tyGenericBody, tyRef, tyPtr, tyAlias, tySink, tyOwned}:
    result = lastSon(result)

proc tryAddInheritedFields(
    c: PContext,
    check: var IntSet,
    pos: var int,
    obj: PType,
  ...
  if (
    (not isPartial) and
    (obj.kind notin {tyObject, tyGenericParam} or tfFinal in obj.flags)
  ) or (innerObj != nil and obj.sym.id == innerObj.sym.id):
    ...
    if (obj.len > 0) and (obj[0] != nil):
      result =
        result and
        tryAddInheritedFields(
          c, check, pos, obj[0].skipGenericInvocation, n, false, obj
        )
...
proc semObjectNode(c: PContext, n: PNode, prev: PType, flags: TTypeFlags): PType =
  ...
  if n[1].kind != nkEmpty:
    ...
    if base.isNil:
      localError(
        c.config, n.info, "cannot inherit from a type that is not an object type"
      )
    else:
      var concreteBase = skipGenericInvocation(base)
      if concreteBase.kind in {tyObject, tyGenericParam, tyGenericInvocation} and
          tfFinal notin concreteBase.flags:
        ...
        if concreteBase.kind != tyError:
          localError(
            c.config,
            n[1].info,
            "inheritance only works with non-final objects; " & "for " &
              typeToString(realBase) & " to be inheritable it must be " &
              "'object of RootObj' instead of 'object'",
          )
        base = nil
        realBase = nil
  if n.kind != nkObjectTy:
    internalError(c.config, n.info, "semObjectNode")
  result = newOrPrevType(tyObject, prev, c)
  rawAddSon(result, realBase)
  if realBase == nil and tfInheritable in flags:
    result.flags.incl tfInheritable
  if tfAcyclic in flags:
    result.flags.incl tfAcyclic
  if result.n.isNil:
    result.n = newNodeI(nkRecList, n.info)
  else:
...
proc semAnyRef(c: PContext, n: PNode, kind: TTypeKind, prev: PType): PType =
  if n.len < 1:
    ...
    let body = n.lastSon
    var t =
      if prev != nil and prev.kind != tyGenericBody and body.kind == nkObjectTy:
    ...
    for i in isCall ..< n.len - 1:
      ...
      if ni.kind == nkNilLit:
        ...
        if region.kind in {tyOwned, tySink}:
          ...
          message(
            c.config, n.info, warnDeprecated, "region for pointer types is deprecated"
          )
          addSonSkipIntLit(result, region, c.idgen)
    addSonSkipIntLit(result, t, c.idgen)
    if tfPartial in result.flags:
      if result.lastSon.kind == tyObject:
        incl(result.lastSon.flags, tfPartial)
...
proc addImplicitGeneric(
  ...
  let finalTypId =
    if typId != nil:
      ...
      getIdent(c.cache, paramName & ":type")
  ...
  let owner =
    if typeClass.sym != nil:
      typeClass.sym
    else:
      getCurrOwner(c)
  var s = newSym(skType, finalTypId, c.idgen, owner, info)
  if sfExplain in owner.flags:
    s.flags.incl sfExplain
  if typId == nil:
    s.flags.incl(sfAnon)
  s.linkTo(typeClass)
...
proc liftParamType(
  ...
  if paramType == nil:
    return
    # (e.g. proc return type)

  template recurse(typ: PType, anonFlag = false): untyped =
    liftParamType(c, procKind, genericParams, typ, paramName, info, anonFlag)

  var paramTypId = if not anon and paramType.sym != nil: paramType.sym.name else: nil

  case paramType.kind
  of tyAnything:
    result = addImplicitGeneric(
      c, newTypeS(tyGenericParam, c), nil, info, genericParams, paramName
    )
  of tyStatic:
    if paramType.base.kind != tyNone and paramType.n != nil:
      # this is a concrete static value
      return
    if tfUnresolved in paramType.flags:
      return
      # already lifted
    ...
    result = addImplicitGeneric(
      c,
      c.newTypeWithSons(tyStatic, @[base]),
      paramTypId,
      info,
      genericParams,
      paramName,
  ...
  of tySequence, tySet, tyArray, tyOpenArray, tyVar, tyLent, tyPtr, tyRef, tyProc:
    ...
    if paramType.kind == tySequence and paramType.lastSon.kind == tyNone:
      let typ = c.newTypeWithSons(tyBuiltInTypeClass, @[newTypeS(paramType.kind, c)])
      result = addImplicitGeneric(c, typ, paramTypId, info, genericParams, paramName)
    else:
      for i in 0 ..< paramType.len:
        if paramType[i] == paramType:
          globalError(
            c.config, info, errIllegalRecursionInTypeX % typeToString(paramType)
          )
  ...
  of tyGenericBody:
    ...
    let x = instGenericContainer(c, paramType.sym.info, result, allowMetaTypes = true)
  ...
  of tyGenericInvocation:
    ...
    if body.lastSon.kind == tyUserTypeClass:
      let expanded = instGenericContainer(c, info, paramType, allowMetaTypes = true)
      result = recurse(expanded, true)
  of tyUserTypeClasses, tyBuiltInTypeClass, tyCompositeTypeClass, tyAnd, tyOr, tyNot,
      tyConcept:
    result = addImplicitGeneric(
      c,
      copyType(paramType, nextTypeId c.idgen, getCurrOwner(c)),
      paramTypId,
      info,
      genericParams,
      paramName,
...
proc semProcTypeNode(
    c: PContext, n, genericParams: PNode, prev: PType, kind: TSymKind, isType = false
): PType =
  ...
  for i in 1 ..< n.len:
    ...
    if hasDefault:
      ...
      if typ == nil:
        ...
        if isEmptyContainer(typ):
          localError(
            c.config, a.info, "cannot infer the type of parameter '" & $a[0] & "'"
          )
    ...
    if not hasType and not hasDefault:
      if isType:
        localError(c.config, a.info, "':' expected")
    ...
    for j in 0 ..< a.len - 2:
      ...
      let lifted = liftParamType(c, kind, genericParams, typ, nameForLift, arg.info)
      let finalType =
        if lifted != nil:
          lifted
        else:
          typ.skipIntLit(c.idgen)
      arg.typ = finalType
  ...
  if r != nil and kind in {skMacro, skTemplate} and r.kind == tyTyped:
    ...
    let info = n[0].info
    const msg =
      "`typed` will change its meaning in future versions of Nim. " &
      "`void` or no return type declaration at all has the same " &
      "meaning as the current meaning of `typed` as return type " & "declaration."
    ...
    if skipTypes(r, {tyGenericInst, tyAlias, tySink}).kind != tyVoid:
      if kind notin {skMacro, skTemplate} and r.kind in {tyTyped, tyUntyped}:
        localError(
          c.config,
          n[0].info,
          "return type '" & typeToString(r) & "' is only valid for macros and templates",
        )
      # 'auto' as a return type does not imply a generic:
        ...
        if r.sym == nil or sfAnon notin r.sym.flags:
          let lifted = liftParamType(c, kind, genericParams, r, "result", n[0].info)
          if lifted != nil:
            r = lifted
...
proc semGeneric(c: PContext, n: PNode, s: PSym, prev: PType): PType =
  if s.typ == nil:
    localError(
      c.config, n.info, "cannot instantiate the '$1' $2" % [s.name.s, s.kind.toHumanStr]
    )
  ...
  if t.kind == tyForward:
    ...
    if m.state != csMatch:
      ...
      err.add "\ngot: <$1>\nbut expected: <$2>" %
        [describeArgs(c, n), describeArgs(c, t.n, 0)]
    ...
    if isConcrete:
      if s.ast == nil and s.typ.kind != tyCompositeTypeClass:
        ...
        result = instGenericContainer(c, n.info, result, allowMetaTypes = false)
  ...
  if tx.isNil or isTupleRecursive(tx):
    localError(
      c.config, n.info, "illegal recursion in type '$1'" % typeToString(result[0])
    )
    return errorType(c)
...
proc maybeAliasType(c: PContext, typeExpr, prev: PType): PType =
  if typeExpr.kind in {tyObject, tyEnum, tyDistinct, tyForward, tyGenericBody} and
      prev != nil:
...
proc semTypeClass(c: PContext, n: PNode, prev: PType): PType =
  ...
  for param in n[0]:
    ...
    # see bug #8230
    if dummyName.kind == nkEmpty:
      continue

    internalAssert c.config, dummyName.kind == nkIdent
    var dummyParam = newSym(
      if modifier == tyTypeDesc: skType else: skVar,
      dummyName.ident,
      c.idgen,
      owner,
      param.info,
      ...
      if strTableGet(c.userPragmas, ident) != nil:
        ...
        if sym != nil and sfCustomPragma in sym.flags:
          ...
          if r != nil:
            ...
            case m.kind
            of skMacro:
              return semMacroExpr(c, r, r, m, {efNoSemCheck})
            of skTemplate:
              return semTemplateExpr(c, r, m, {efNoSemCheck})
            else:
              doAssert(false, "cannot happen")

proc semProcTypeWithScope(c: PContext, n: PNode, prev: PType, kind: TSymKind): PType =
  checkSonsLen(n, 2, c.config)
  ...
  if n[1].kind != nkEmpty and n[1].len > 0:
    ...
    when useEffectSystem:
      setEffectsForProcType(c.graph, result, s.ast[pragmasPos])
  closeScope(c)
...
proc semTypeOf2(c: PContext, n: PNode, prev: PType): PType =
  ...
  if n.len == 3:
    ...
    if mode.kind != nkIntLit:
      localError(
        c.config, n.info, "typeof: cannot evaluate 'mode' parameter at compile-time"
      )
    else:
      m = mode.intVal
  let t = semExprWithType(
    c,
    n[1],
    if m == 1:
      {efInTypeof}
...
proc semTypeNode(c: PContext, n: PNode, prev: PType): PType =
  ...
  of nkCallKinds:
    ...
    if ident != nil and ident.s == "[]":
      ...
      result = semTypeNode(c, n[1], prev)
      if result.skipTypes({tyGenericInst, tyAlias, tySink, tyOwned}).kind in
          NilableTypes + GenericTypes:
  ...
  of nkBracketExpr:
    ...
    var head = n[0]
    var s =
      if head.kind notin nkCallKinds:
        semTypeIdent(c, head)
      else:
        symFromExpectedTypeNode(c, semExpr(c, head))
    case s.magic
    of mArray:
      result = semArray(c, n, prev)
    of mOpenArray:
      result = semContainer(c, n, tyOpenArray, "openarray", prev)
    of mUncheckedArray:
      result = semContainer(c, n, tyUncheckedArray, "UncheckedArray", prev)
    of mRange:
      result = semRange(c, n, prev)
    of mSet:
      result = semSet(c, n, prev)
    of mOrdinal:
      result = semOrdinal(c, n, prev)
    of mIterableType:
      result = semIterableType(c, n, prev)
    of mSeq:
      result = semContainer(c, n, tySequence, "seq", prev)
      if optSeqDestructors in c.config.globalOptions:
        incl result.flags, tfHasAsgn
    of mVarargs:
      result = semVarargs(c, n, prev)
  ...
  of nkDotExpr:
    ...
    if typeExpr.typ.isNil:
      localError(
        c.config,
        n.info,
        "object constructor needs an object type;" &
      ...
      if result.isMetaType and result.kind != tyUserTypeClass:
  ...
  of nkSym:
    ...
    if s.typ != nil and (s.kind == skType or s.typ.kind == tyTypeDesc):
      ...
      if s.kind != skError:
        if s.typ == nil:
          localError(
            c.config, n.info, "type expected, but symbol '$1' has no type." % [s.name.s]
          )
        else:
          localError(
            c.config,
            n.info,
            "type expected, but got symbol '$1' of kind '$2'" %
  ...
  of nkTupleTy:
    result = semTuple(c, n, prev)
  of nkTupleClassTy:
    result = newConstraint(c, tyTuple)
  of nkTypeClassTy:
    result = semTypeClass(c, n, prev)
  of nkRefTy:
    result = semAnyRef(c, n, tyRef, prev)
  of nkPtrTy:
    result = semAnyRef(c, n, tyPtr, prev)
  of nkVarTy:
    result = semVarOutType(c, n, prev, {})
  of nkOutTy:
    result = semVarOutType(c, n, prev, {tfIsOutParam})
  of nkDistinctTy:
    result = semDistinct(c, n, prev)
  of nkStaticTy:
    result = semStaticType(c, n[0], prev)
...
proc processMagicType(c: PContext, m: PSym) =
  case m.magic
  of mInt:
    setMagicIntegral(c.config, m, tyInt, c.config.target.intSize)
  of mInt8:
    setMagicIntegral(c.config, m, tyInt8, 1)
  of mInt16:
    setMagicIntegral(c.config, m, tyInt16, 2)
  of mInt32:
    setMagicIntegral(c.config, m, tyInt32, 4)
  of mInt64:
    setMagicIntegral(c.config, m, tyInt64, 8)
  of mUInt:
    setMagicIntegral(c.config, m, tyUInt, c.config.target.intSize)
  of mUInt8:
    setMagicIntegral(c.config, m, tyUInt8, 1)
  of mUInt16:
    setMagicIntegral(c.config, m, tyUInt16, 2)
  of mUInt32:
    setMagicIntegral(c.config, m, tyUInt32, 4)
  of mUInt64:
    setMagicIntegral(c.config, m, tyUInt64, 8)
  of mFloat:
    setMagicIntegral(c.config, m, tyFloat, c.config.target.floatSize)
  of mFloat32:
    setMagicIntegral(c.config, m, tyFloat32, 4)
  of mFloat64:
    setMagicIntegral(c.config, m, tyFloat64, 8)
  of mFloat128:
    setMagicIntegral(c.config, m, tyFloat128, 16)
  of mBool:
    setMagicIntegral(c.config, m, tyBool, 1)
  of mChar:
    setMagicIntegral(c.config, m, tyChar, 1)
  of mString:
  ...
  of mCstring:
    ...
    rawAddSon(m.typ, getSysType(c.graph, m.info, tyChar))
  of mPointer:
    setMagicIntegral(c.config, m, tyPointer, c.config.target.ptrSize)
  of mNil:
    setMagicType(c.config, m, tyNil, c.config.target.ptrSize)
  of mExpr:
    if m.name.s == "auto":
  ...
  of mBuiltinType:
    case m.name.s
    of "lent":
      setMagicType(c.config, m, tyLent, c.config.target.ptrSize)
    of "sink":
      setMagicType(c.config, m, tySink, szUncomputedSize)
    of "owned":
      setMagicType(c.config, m, tyOwned, c.config.target.ptrSize)
      incl m.typ.flags, tfHasOwned
    else:
      localError(c.config, m.info, errTypeExpected)
  else:
    localError(c.config, m.info, errTypeExpected)
...
proc semGenericParamList(c: PContext, n: PNode, father: PType = nil): PNode =
  template addSym(result: PNode, s: PSym): untyped =
    if father != nil:
      addSonSkipIntLit(father, s.typ, c.idgen)
  ...
  for i in 0 ..< n.len:
    ...
    of nkIdentDefs:
      ...
      for j in 0 ..< a.len - 2:
        ...
        if paramName.safeLen == 2:
          if not nimEnableCovariance or paramName[0].ident.s == "in":
            if father == nil or sfImportc notin father.sym.flags:
              localError(
                c.config, paramName.info, errInOutFlagNotExtern % $paramName[0]
              )
          covarianceFlag =
            if paramName[0].ident.s == "in": tfContravariant else: tfCovariant
          if father != nil:
            father.flags.incl tfCovariant
        ...
        if covarianceFlag != tfUnresolved:
          s.typ.flags.incl(covarianceFlag)
        if def.kind != nkEmpty:
          s.ast = def
