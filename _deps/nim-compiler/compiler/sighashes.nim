...
proc `&=`(c: var MD5Context, s: string) =
  md5Update(c, s, s.len)
...
proc hashTypeSym(c: var MD5Context, s: PSym, conf: ConfigRef) =
  if sfAnon in s.flags or s.kind == skGenericParam:
    ...
    while it != nil:
      if sfFromGeneric in it.flags and it.kind in routineKinds and it.typ != nil:
...
proc hashType(c: var MD5Context, t: PType, flags: set[ConsiderFlag], conf: ConfigRef) =
  ...
  of tyObject, tyEnum:
    ...
    if t.sym != nil:
      ...
      if hasFlag(t.sym) or (
        t.kind == tyObject and t.owner.kind == skType and t.owner.typ.kind == tyRef and
        hasFlag(t.owner)
      ):
  ...
  of tyArray:
    ...
    for i in 0 ..< t.len:
      c.hashType(t[i], flags, conf)
  if tfNotNil in t.flags and CoType notin flags:
    c &= "not nil"
...
when defined(debugSigHashes):
  ...
  let db = open(
    connection = "sighashes.db", user = "araq", password = "", database = "sighashes"
  )
...
proc hashType*(
    t: PType, conf: ConfigRef, flags: set[ConsiderFlag] = {CoType}
): SigHash =
  var c: MD5Context
  md5Init c
  ...
  when defined(debugSigHashes):
    db.exec(
      sql"INSERT OR IGNORE INTO sighashes(type, hash) VALUES (?, ?)",
      typeToString(t),
      $result,
    )
...
proc hashVarSymBody(graph: ModuleGraph, c: var MD5Context, s: PSym) =
  assert:
    s.kind in {skParam, skResult, skVar, skLet, skConst, skForVar}
...
proc idOrSig*(
    ...
    sigCollisions: var CountTable[SigHash],
    conf: ConfigRef,
): Rope =
  if s.kind in routineKinds and s.typ != nil:
