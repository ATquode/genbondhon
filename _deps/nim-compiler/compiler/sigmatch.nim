...
import
  intsets, ast, astalgo, semdata, types, msgs, renderer, lookups, semtypinst, magicsys,
  idents, lexer, options, parampatterns, strutils, trees, linter, lineinfos, lowerings,
  modulegraphs, concepts
...
type
  ...
  MismatchInfo* = object
    ...
    formal*: PSym
      # parameter that mismatches against provided argument
  ...
  TCandidate* = object
    ...
    baseTypeMatch: bool
      # needed for conversions from T to openarray[T]
      # for example
    fauxMatch*: TTypeKind
      # the match was successful only due to the use
      # of error or wildcard (unknown) types.
      # this is used to prevent instantiations.
    genericConverter*: bool
      # true if a generic converter needs to
      # be instantiated
    coerceDistincts*: bool
      # this is an explicit coercion that can strip away
      # a distrinct type
    typedescMatched*: bool
    isNoCall*: bool # misused for generic type instantiations C[T]
    inferredTypes: seq[PType]
      # inferred types during the current signature
...
const isNilConversion = isConvertible # maybe 'isIntConv' fits better?
...
proc initCandidateAux(ctx: PContext, c: var TCandidate, callee: PType) {.inline.} =
  c.c = ctx
  c.exactMatches = 0
  c.subtypeMatches = 0
  c.convMatches = 0
...
proc put(c: var TCandidate, key, val: PType) {.inline.} =
  ...
  when false:
    ...
    if old != nil:
      echo "Putting ",
        typeToString(key), " ", typeToString(val), " and old is ", typeToString(old)
...
proc newCandidate*(
    ctx: PContext, callee: PSym, binding: PNode, calleeScope = -1
): TCandidate =
...
proc typeRel*(
  c: var TCandidate, f, aOrig: PType, flags: TTypeRelFlags = {}
): TTypeRelation
...
proc sumGeneric(t: PType): int =
  ...
  while true:
    case t.kind
    of tyGenericInst, tyArray, tyRef, tyPtr, tyDistinct, tyUncheckedArray, tyOpenArray,
        tyVarargs, tySet, tyRange, tySequence, tyGenericBody, tyLent, tyOwned:
      t = t.lastSon
      inc result
    of tyOr:
      var maxBranch = 0
    ...
    of tyGenericParam, tyUntyped, tyTyped:
      break
    of tyAlias, tySink:
      t = t.lastSon
    of tyBool,
...
template describeArgImpl(
    c: PContext, n: PNode, i: int, startIdx = 1, prefer = preferName
) =
  var arg = n[i]
...
proc concreteType(c: TCandidate, t: PType, f: PType = nil): PType =
  ...
  of tyOwned:
    ...
    if f != nil and f.len > 0 and
        f[0].skipTypes({tyBuiltInTypeClass, tyOr}).kind == tyProc:
      result = t.lastSon
    else:
      result = t
  else:
...
proc handleRange(c: PContext, f, a: PType, min, max: TTypeKind): TTypeRelation =
  if a.kind == f.kind:
    ...
    if k == f.kind:
      result = isSubrange
    elif k == tyInt and f.kind in {tyRange, tyInt .. tyInt64, tyUInt .. tyUInt64} and
    ...
    (
      ...
      f.kind in {tyUInt8 .. tyUInt32} and a[0].kind in {tyUInt8 .. tyUInt32}
    ) and a.n[0].intVal >= firstOrd(nil, f) and a.n[1].intVal <= lastOrd(nil, f):
      # passing 'nil' to firstOrd/lastOrd here as type checking rules should
      # not depend on the target integer size configurations!
      result = isConvertible
    else:
...
proc isConvertibleToRange(c: PContext, f, a: PType): bool =
  if f.kind in {tyInt .. tyInt64, tyUInt .. tyUInt64} and
    ...
    case f.kind
    of tyInt8:
      result = isIntLit(a) or a.kind in {tyInt8}
    of tyInt16:
      result = isIntLit(a) or a.kind in {tyInt8, tyInt16}
    of tyInt32:
      result = isIntLit(a) or a.kind in {tyInt8, tyInt16, tyInt32}
    ...
    of tyInt64:
      result = isIntLit(a) or a.kind in {tyInt8, tyInt16, tyInt32, tyInt, tyInt64}
    of tyUInt8:
      result = isIntLit(a) or a.kind in {tyUInt8}
    of tyUInt16:
      result = isIntLit(a) or a.kind in {tyUInt8, tyUInt16}
    of tyUInt32:
      result = isIntLit(a) or a.kind in {tyUInt8, tyUInt16, tyUInt32}
    # of tyUInt: result = isIntLit(a) or a.kind in {tyUInt8 .. c.config.targetSizeUnsignedToKind}
    of tyUInt64:
      result = isIntLit(a) or a.kind in {tyUInt8, tyUInt16, tyUInt32, tyUInt64}
    else:
      result = false
...
proc skipToObject(t: PType, skipped: var SkippedPtr): PType =
  ...
  while r != nil:
    ...
    of tyGenericBody, tyGenericInst, tyAlias, tySink, tyOwned:
      ...
      break
  if r.kind == tyObject and ptrs <= 1:
    result = r

proc isGenericSubtype(
    c: var TCandidate, a, f: PType, d: var int, fGenericOrigin: PType
): bool =
...
proc recordRel(c: var TCandidate, f, a: PType): TTypeRelation =
  ...
  if sameType(f, a):
    ...
    result = isEqual
    let firstField = if f.kind == tyTuple: 0 else: 1
...
proc procTypeRel(c: var TCandidate, f, a: PType): TTypeRelation =
  ...
  of tyProc:
    if f.len != a.len:
      return
    result = isEqual
      # start with maximum; also correct for no
      # params at all
    ...
    when useEffectSystem:
      if compatibleEffects(f, a) != efCompat:
        return isNone
    when defined(drnim):
      if not c.c.graph.compatibleProps(c.c.graph, f, a):
        return isNone
  of tyNil:
    result = f.allowsNil
  else:
...
proc matchUserTypeClass*(m: var TCandidate, ff, a: PType): PType =
  ...
  var
    ...
    errorPrefix: string
    flags: TExprFlags = {}
    collectDiagnostics = m.diagnosticsEnabled or sfExplain in typeClass.sym.flags
...
proc shouldSkipDistinct(m: TCandidate, rules: PNode, callIdent: PIdent): bool =
  ...
  if rules.kind == nkWith:
    for r in rules:
      if considerQuotedIdent(m.c, r) == callIdent:
        return true
    return false
  else:
    for r in rules:
...
proc tryResolvingStaticExpr(
    c: var TCandidate, n: PNode, allowUnresolved = false
): PNode =
  ...
  let instantiated =
    replaceTypesInBody(c.c, c.bindings, n, nil, allowMetaTypes = allowUnresolved)
...
proc failureToInferStaticParam(conf: ConfigRef, n: PNode) =
  ...
  let name = if staticParam != nil: staticParam.sym.name.s else: "unknown"
  localError(conf, n.info, "cannot infer the value of the static param '" & name & "'")

proc inferStaticsInRange(c: var TCandidate, inferred, concrete: PType): TTypeRelation =
  let lowerBound = tryResolvingStaticExpr(c, inferred.n[0], allowUnresolved = true)
  let upperBound = tryResolvingStaticExpr(c, inferred.n[1], allowUnresolved = true)
...
template subtypeCheck() =
  if result <= isSubrange and
      f.lastSon.skipTypes(abstractInst).kind in {tyRef, tyPtr, tyVar, tyLent, tyOwned}:
    result = isNone
...
proc isCovariantPtr(c: var TCandidate, f, a: PType): bool =
  ...
  of tyGenericInst:
    let body = f.base
    return
      body == a.base and a.len == 3 and tfWeakCovariant notin body[0].flags and
...
when false:
  proc maxNumericType(prev, candidate: PType): PType =
    ...
    of tyInt:
      greater({tyInt64})
    of tyInt8:
      greater({tyInt, tyInt16, tyInt32, tyInt64})
    of tyInt16:
      greater({tyInt, tyInt32, tyInt64})
    of tyInt32:
      greater({tyInt64})
    of tyUInt:
      greater({tyUInt64})
    of tyUInt8:
      greater({tyUInt, tyUInt16, tyUInt32, tyUInt64})
    of tyUInt16:
      greater({tyUInt, tyUInt32, tyUInt64})
    of tyUInt32:
      greater({tyUInt64})
    of tyFloat32:
      greater({tyFloat64, tyFloat128})
    of tyFloat64:
      greater({tyFloat128})
    else:
      discard
...
template skipOwned(a) =
  if a.kind == tyOwned:
    a = a.skipTypes({tyOwned, tyGenericInst})
...
proc typeRel(
  ...
  var
    useTypeLoweringRuleInTypeClass =
      c.c.matchedConcept != nil and not c.isNoCall and f.kind != tyTypeDesc and
      tfExplicit notin aOrig.flags and tfConceptMatchedTypeSym notin aOrig.flags
  ...
  template doBind(): bool =
    trDontBind notin flags

  # var, sink and static arguments match regular modifier-free types
  var a = maybeSkipDistinct(
    c, aOrig.skipTypes({tyStatic, tyVar, tyLent, tySink}), c.calleeSym
  )
  ...
  if a.kind == tyGenericInst and
      ...
      {tyGenericBody, tyGenericInvocation, tyGenericInst, tyGenericParam} + tyTypeClasses:
  ...
  of tyNot:
    ...
    of tyNot:
      ...
      return if f.kind == tyAnything: isGeneric else: isNone
  of tyAnything:
    if f.kind == tyAnything:
      return isGeneric
    else:
      return isNone
  ...
  of tyInt:
    result = handleRange(c.c, f, a, tyInt8, c.c.config.targetSizeSignedToKind)
  of tyInt8:
    result = handleRange(c.c, f, a, tyInt8, tyInt8)
  of tyInt16:
    result = handleRange(c.c, f, a, tyInt8, tyInt16)
  of tyInt32:
    result = handleRange(c.c, f, a, tyInt8, tyInt32)
  of tyInt64:
    result = handleRange(c.c, f, a, tyInt, tyInt64)
  of tyUInt:
    result = handleRange(c.c, f, a, tyUInt8, c.c.config.targetSizeUnsignedToKind)
  of tyUInt8:
    result = handleRange(c.c, f, a, tyUInt8, tyUInt8)
  of tyUInt16:
    result = handleRange(c.c, f, a, tyUInt8, tyUInt16)
  of tyUInt32:
    result = handleRange(c.c, f, a, tyUInt8, tyUInt32)
  of tyUInt64:
    result = handleRange(c.c, f, a, tyUInt, tyUInt64)
  of tyFloat:
  ...
  of tyArray:
    ...
    of tyArray:
      ...
      if result < isGeneric:
        if nimEnableCovariance and trNoCovariance notin flags and ff.kind == aa.kind and
  ...
  of tyOpenArray, tyVarargs:
    ...
    template matchArrayOrSeq(aBase: PType) =
      ...
      if baseRel >= isGeneric:
        result = isConvertible
      elif nimEnableCovariance and trNoCovariance notin flags and ff.kind == aa.kind and
          isCovariantPtr(c, ff, aa):
        result = isConvertible
  ...
  of tyGenericInvocation:
    ...
    if x.kind in {tyGenericInst, tyArray} and c.calleeSym != nil and
    ...
    if x.kind == tyGenericInvocation:
      if f[0] == x[0]:
        for i in 1 ..< f.len:
          ...
          if tr <= isSubtype:
            return
        result = isGeneric
    elif x.kind == tyGenericInst and f[0] == x[0] and x.len - 1 == f.len:
      for i in 1 ..< f.len:
      ...
      result =
        if concepts.conceptMatch(c.c, concpt, x, c.bindings, f): isGeneric else: isNone
    else:
      let genericBody = f[0]
      var askip = skippedNone
      var fskip = skippedNone
      ...
      if result == isNone:
        ...
        if not (genericSubtype and aobj.sym.id != fobj.sym.id) and
            aOrig.kind != tyGenericBody:
          depth = -1
  ...
  of tyBuiltInTypeClass:
    considerPreviousT:
      ...
      let effectiveArgType = a.skipTypes(
        {tyRange, tyGenericInst, tyBuiltInTypeClass, tyAlias, tySink, tyOwned}
      )
  ...
  of tyConcept:
    result =
      if concepts.conceptMatch(c.c, f, a, c.bindings, nil): isGeneric else: isNone
      ...
      if c.callee.kind == tyGenericBody and not c.typedescMatched:
        ...
        if f.len > 0 and f[0].kind != tyNone:
          ...
          if result in {isEqual, isSubtype}:
            ...
            c.inheritancePenalty =
              oldInheritancePenalty - c.inheritancePenalty - 100 * ord(
                result == isEqual
              )
            result = isGeneric
        elif a.kind == tyTypeDesc:
  ...
  of tyTypeDesc:
    ...
    if prev == nil:
      ...
      if c.c.inGenericContext > 0 and a.skipTypes({tyTypeDesc}).kind == tyGenericParam:
        # generic type bodies can sometimes compile call expressions
  ...
  of tyFromExpr:
    ...
    if f.n == nil or f.n.kind == nkEmpty:
      return isGeneric
...
proc getInstantiatedType(c: PContext, arg: PNode, m: TCandidate, f: PType): PType =
...
proc implicitConv(
    kind: TNodeKind, f: PType, arg: PNode, m: TCandidate, c: PContext
): PNode =
...
proc isLValue(c: PContext, n: PNode, isOutParam = false): bool {.inline.} =
  ...
  of arAddressableConst:
    let sym = getRoot(n)
    result =
      strictDefs in c.features and sym != nil and sym.kind == skLet and isOutParam
  else:
    result = false

proc userConvMatch(c: PContext, m: var TCandidate, f, a: PType, arg: PNode): PNode =
  result = nil
...
proc localConvMatch(c: PContext, m: var TCandidate, f, a: PType, arg: PNode): PNode =
...
proc paramTypesMatchAux(
    m: var TCandidate, f, a: PType, argSemantized, argOrig: PNode
): PNode =
  var
  ...
  if tfHasStatic in fMaybeStatic.flags:
    ...
    if a.kind == tyStatic:
      if m.callee.kind == tyGenericBody and a.n == nil and
  ...
  if r != isNone and m.calleeSym != nil and m.calleeSym.kind in {skMacro, skTemplate}:
  ...
  while r == isBothMetaConvertible and lastBindingsLength != m.bindings.counter and
      bothMetaCounter < 100:
  ...
  of isEqual:
    ...
    if ff.kind == tyTuple or
        (
          arg.typ != nil and
          skipTypes(arg.typ, abstractVar - {tyTypeDesc}).kind == tyTuple
        ):
      result = implicitConv(nkHiddenSubConv, f, arg, m, c)
  of isNone:
...
proc paramTypesMatch*(m: var TCandidate, f, a: PType, arg, argOrig: PNode): PNode =
  if arg == nil or arg.kind notin nkSymChoices:
    ...
    var best = -1
    for i in 0 ..< arg.len:
      if arg[i].sym.kind in {
        skProc, skFunc, skMethod, skConverter, skIterator, skMacro, skTemplate,
        skEnumField,
      }:
        copyCandidate(z, m)
        z.callee = arg[i].typ
    ...
    if x.state == csEmpty:
      ...
      # anyway:
      if f.kind in {tyUntyped, tyTyped}:
        result = arg
      else:
        result = nil
    else:
  ...
  when false:
    if m.calleeSym != nil and m.calleeSym.name.s == "[]":
      echo m.c.config $ arg.info,
        " for ", m.calleeSym.name.s, " ", m.c.config $ m.calleeSym.info
      writeMatches(m)
...
proc prepareOperand(c: PContext, formal: PType, a: PNode): PNode =
  if formal.kind == tyUntyped and formal.len != 1:
    ...
    if result.kind != nkHiddenDeref and result.typ.kind in {tyVar, tyLent} and
...
proc arrayConstr(c: PContext, n: PNode): PType =
  ...
  addSonSkipIntLit(
    result, skipTypes(n.typ, {tyGenericInst, tyVar, tyLent, tyOrdinal}), c.idgen
  )
...
proc matchesAux(c: PContext, n, nOrig: PNode, m: var TCandidate, marker: var IntSet) =
  ...
  var
    ...
    f = if m.callee.kind != tyGenericBody: 1 else: 0 # iterates over formal parameters
  ...
  while a < n.len:
    ...
    if a >= formalLen - 1 and f < formalLen and m.callee.n[f].typ.isVarargsUntyped:
      ...
      if n[a].kind == nkHiddenStdConv:
        doAssert n[a][0].kind == nkEmpty and n[a][1].kind in {nkBracket, nkArgList}
      ...
      if containsOrIncl(marker, formal.position):
        ...
        when false:
          localError(n[a].info, errCannotBindXTwice, formal.name.s)
        noMatch()
...
proc instTypeBoundOp*(
    c: PContext, dc: PSym, t: PType, info: TLineInfo, op: TTypeAttachedOp, col: int
): PSym =
