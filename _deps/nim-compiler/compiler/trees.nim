...
proc exprStructuralEquivalent*(a, b: PNode, strictSymEquality = false): bool =
  if a == b:
    ...
    of nkFloatLit .. nkFloat64Lit:
      result = sameFloatIgnoreNan(a.floatVal, b.floatVal)
    of nkStrLit .. nkTripleStrLit:
      result = a.strVal == b.strVal
    of nkCommentStmt:
      result = a.comment == b.comment
    of nkEmpty, nkNilLit, nkType:
      result = true
    else:
      if a.len == b.len:
        for i in 0 ..< a.len:
          if not exprStructuralEquivalent(a[i], b[i], strictSymEquality):
            return
        result = true
...
proc isConstExpr*(n: PNode): bool =
  const atomKinds = {nkCharLit .. nkNilLit}
    # Char, Int, UInt, Str, Float and Nil literals
...
proc isDeepConstExpr*(n: PNode, preventInheritance = false): bool =
  ...
  of nkCurly, nkBracket, nkPar, nkTupleConstr, nkObjConstr, nkClosure, nkRange:
    for i in ord(n.kind == nkObjConstr) ..< n.len:
      if not isDeepConstExpr(n[i], preventInheritance):
        return false
    if n.typ.isNil:
      result = true
    else:
...
proc whichPragma*(n: PNode): TSpecialWord =
  let key =
    if n.kind in nkPragmaCallKinds and n.len > 0:
      n[0]
    else:
      n
  case key.kind
  ...
  of nkSym:
    result = whichKeyword(key.sym.name)
  of nkCast:
    result = wCast
...
proc getRoot*(n: PNode): PSym =
  ...
  of nkDotExpr, nkBracketExpr, nkHiddenDeref, nkDerefExpr, nkObjUpConv, nkObjDownConv,
      nkCheckedFieldExpr, nkHiddenAddr, nkAddr:
    result = getRoot(n[0])
...
proc isRunnableExamples*(n: PNode): bool =
  ...
  result =
    n.kind == nkSym and n.sym.magic == mRunnableExamples or
