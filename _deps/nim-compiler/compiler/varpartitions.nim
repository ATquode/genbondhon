...
type AbstractTime = distinct int
...
template isConstParam(a): bool =
  a.kind == skParam and a.typ.kind notin {tyVar, tySink}
...
proc registerResult(c: var Partitions, n: PNode) =
  if n.kind == nkSym:
    c.s.add VarIndex(
      con: Connection(kind: isEmptyRoot),
      sym: n.sym,
      reassignedTo: 0,
      aliveStart: MaxTime,
      aliveEnd: c.abstractTime,
    )
...
proc registerParam(c: var Partitions, n: PNode) =
  ...
  if isConstParam(n.sym):
    c.s.add VarIndex(
      con: Connection(kind: isRootOf, graphIndex: c.graphs.len),
      sym: n.sym,
      reassignedTo: 0,
      aliveStart: c.abstractTime,
      aliveEnd: c.abstractTime,
    )
    c.graphs.add MutationInfo(
      param: n.sym,
      mutatedHere: unknownLineInfo,
      connectedVia: unknownLineInfo,
      flags: {connectsConstParam},
      maxMutation: MinTime,
      minConnection: MaxTime,
      mutations: @[],
    )
  else:
    c.s.add VarIndex(
...
proc potentialMutation(v: var Partitions, s: PSym, level: int, info: TLineInfo) =
  ...
  if id >= 0:
    ...
    of isEmptyRoot:
      ...
      v.graphs.add MutationInfo(
        param: if isConstParam(s): s else: nil,
        mutatedHere: info,
        connectedVia: unknownLineInfo,
        flags: flags,
        maxMutation: v.abstractTime,
        minConnection: MaxTime,
...
proc borrowFromConstExpr(n: PNode): bool =
  ...
  of nkCurly, nkBracket, nkPar, nkTupleConstr, nkObjConstr, nkClosure, nkRange:
    ...
    for i in ord(n.kind == nkObjConstr) ..< n.len:
      if not borrowFromConstExpr(n[i]):
        return false
  of nkCallKinds:
    ...
    of nkSym:
      ...
      of skParam, skTemp, skResult, skForVar:
        if n.sym.owner == owner:
          result = n
      of skVar:
        if n.sym.owner == owner or sfThread in n.sym.flags:
          result = n
      of skLet, skConst:
        ...
        discard
      break
    of nkDotExpr, nkDerefExpr, nkBracketExpr, nkHiddenDeref, nkCheckedFieldExpr, nkAddr,
        nkHiddenAddr:
      n = n[0]
    of nkHiddenStdConv, nkHiddenSubConv, nkConv, nkCast, nkObjUpConv, nkObjDownConv:
      n = n.lastSon
...
const RootEscapes = 1000
  # in 'p(r)' we don't know what p does to our poor root.
...
proc allRoots(n: PNode, result: var seq[(PSym, int)], level: int) =
  ...
  of nkExprEqExpr, nkExprColonExpr, nkHiddenStdConv, nkHiddenSubConv, nkConv,
      nkStmtList, nkStmtListExpr, nkBlockStmt, nkBlockExpr, nkCast, nkObjUpConv,
      nkObjDownConv:
    if n.len > 0:
  ...
  of nkSym:
    if n.sym.kind in {skVar, skResult, skTemp, skLet, skForVar, skParam}:
      if n.sym.flags * {sfThread, sfGlobal} != {}:
        ...
        if c.inConstructor > 0:
          dest.flags.incl ownsData
  of nkDotExpr, nkBracketExpr, nkHiddenDeref, nkDerefExpr, nkObjUpConv, nkObjDownConv,
  ...
  of nkCallKinds:
    if n.typ != nil:
      if hasDestructor(n.typ):
        ...
        # also reflect 'dest':
        if magic in {
          mNone, mMove, mSlice, mAppendStrCh, mAppendStrStr, mAppendSeqElem, mArrToSeq,
          mOpenArrayToSeq,
        }:
          for i in 1 ..< n.len:
            ...
            # the root's symbol.
            if hasDestructor(
              n[i].typ.skipTypes({tyVar, tySink, tyLent, tyGenericInst, tyAlias})
            ):
              destMightOwn(c, dest, n[i])
  else:
...
proc borrowFrom(c: var Partitions, dest: PSym, src: PNode) =
  const url =
    "see https://nim-lang.github.io/Nim/manual_experimental.html#view-types-algorithm-path-expressions for details"
  ...
  if s == nil:
    localError(
      c.g.config,
      src.info,
      "cannot borrow from " & src.toString & ", it is not a path expression; " & url,
    )
  elif s.kind == nkSym:
    if dest.kind == skResult:
      if s.sym.kind != skParam or s.sym.position != 0:
        localError(
          c.g.config, src.info, "'result' must borrow from the first parameter"
        )
...
proc borrowingAsgn(c: var Partitions, dest, src: PNode) =
  ...
  if dest.kind == nkSym:
    ...
    if viewOrigin != nil and viewOrigin.kind == nkSym:
      ...
      let directView = directViewType(dest[0].typ)
        # check something like result[first] = toOpenArray(s, first, last-1)
        # so we don't need to iterate the original type
      let originSymbolView = directViewType(viewSym.typ)
        # find the original symbol which preserves the view type
          ...
          if s == d.sym:
            ...
            if srcid >= 0:
              if s.kind notin {skResult, skParam} and
                ...
                when explainCursors:
                  echo "B not a cursor ",
                    d.sym, " ", c.s[srcid].aliveEnd, " ", c.s[vid].aliveEnd
                c.s[vid].flags.incl preventCursor
...
proc potentialMutationViaArg(c: var Partitions, n: PNode, callee: PType) =
  if constParameters in c.goals and tfNoSideEffect in callee.flags:
    ...
    for r in roots:
      potentialMutation(c, r[0], r[1], n.info)
  ...
  of nkCallKinds:
    ...
    for i in 1 ..< n.len:
      ...
      if i < L:
        ...
        if not paramType.isCompileTimeOnly and paramType.kind in {
          tyVar, tySink, tyOwned
        }:
          var roots: seq[(PSym, int)]
          ...
          if paramType.kind == tyVar:
            ...
            if borrowChecking in c.goals:
              ...
              if directViewType(paramType[0]) == noView and
                  classifyViewType(paramType[0]) != noView:
...
proc computeLiveRanges(c: var Partitions, n: PNode) =
  ...
  of nkAsgn, nkFastAsgn, nkSinkAsgn:
    ...
    if n[0].kind == nkSym:
      ...
      if vid >= 0:
        if n[1].kind == nkSym and
            (c.s[vid].reassignedTo == 0 or c.s[vid].reassignedTo == n[1].sym.id):
...
proc computeGraphPartitions*(
    s: PSym, n: PNode, g: ModuleGraph, goals: set[Goal]
): Partitions =
...
proc cannotBorrow(config: ConfigRef, s: PSym, g: MutationInfo) =
  var m = "cannot borrow " & s.name.s & "; what it borrows from is potentially mutated"
          ...
          if sid >= 0:
            ...
            if par.s[sid].sym.kind != skParam and
              ...
              localError(
                config,
                v.info,
                "'" & v.name.s & "' borrows from location '" & par.s[sid].sym.name.s &
            ...
            if viewDoesMutate in par.s[rid].flags and isConstSym(par.s[sid].sym):
              localError(
                config,
                v.info,
                "'" & v.name.s & "' borrows from the immutable location '" &
                  par.s[sid].sym.name.s & "' and attempts to mutate it",
              )
              constViolation = true
        if {viewDoesMutate, viewBorrowsFromConst} * par.s[rid].flags ==
            {viewDoesMutate, viewBorrowsFromConst} and not constViolation:
...
proc computeCursors*(s: PSym, n: PNode, g: ModuleGraph) =
  ...
  for i in 0 ..< par.s.len:
    ...
    if v.flags * {ownsData, preventCursor, isConditionallyReassigned} == {} and
        v.sym.kind notin {skParam, skResult} and v.sym.flags * {sfThread, sfGlobal} == {} and
        hasDestructor(v.sym.typ) and
