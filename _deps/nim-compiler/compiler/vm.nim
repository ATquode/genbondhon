...
const traceCode = defined(nimVMDebug)
...
proc bailOut(c: PCtx, tos: PStackFrame) =
  stackTrace(
    ...
    tos,
    c.exceptionInstr,
    "unhandled exception: " & c.currentExceptionA[3].skipColon.strVal & " [" &
...
proc derefPtrToReg(
    address: BiggestInt, typ: PType, r: var TFullReg, isAssign: bool
): bool =
  ...
  case typ.kind
  of tyChar:
    fun(intVal, char, rkInt)
  of tyInt:
    fun(intVal, int, rkInt)
  of tyInt8:
    fun(intVal, int8, rkInt)
  of tyInt16:
    fun(intVal, int16, rkInt)
  of tyInt32:
    fun(intVal, int32, rkInt)
  of tyInt64:
    fun(intVal, int64, rkInt)
  of tyUInt:
    fun(intVal, uint, rkInt)
  of tyUInt8:
    fun(intVal, uint8, rkInt)
  of tyUInt16:
    fun(intVal, uint16, rkInt)
  of tyUInt32:
    fun(intVal, uint32, rkInt)
  of tyUInt64:
    fun(intVal, uint64, rkInt)
  # note: differs from typeinfo.getBiggestInt
  of tyFloat:
    fun(floatVal, float, rkFloat)
  of tyFloat32:
    fun(floatVal, float32, rkFloat)
  of tyFloat64:
    fun(floatVal, float64, rkFloat)
  else:
    return false
...
proc createStrKeepNode(x: var TFullReg, keepNode = true) =
  if x.node.isNil or not keepNode:
    ...
    when defined(useNodeIds):
      x.node.id = id
  elif x.node.kind notin {nkStrLit .. nkTripleStrLit} or nfAllConst in x.node.flags:
...
type ExceptionGoto = enum
  ExceptionGotoHandler
  ExceptionGotoFinally
  ExceptionGotoUnhandled

proc findExceptionHandler(
    c: PCtx, f: PStackFrame, exc: PNode
  ...
  while f.safePoints.len > 0:
    ...
    while c.code[pc].opcode == opcExcept:
      ...
      while c.code[pc].opcode == opcExcept:
        ...
        let exceptType =
          if excIndex > 0:
            c.types[excIndex].skipTypes(abstractPtrs)
          else:
            nil
...
proc opConv(c: PCtx, dest: var TFullReg, src: TFullReg, desttyp, srctyp: PType): bool =
  if desttyp.kind == tyString:
    ...
    of tyInt .. tyInt64:
      ...
      if toInt128(dest.intVal) < firstOrd(c.config, desttyp) or
          toInt128(dest.intVal) > lastOrd(c.config, desttyp):
        return true
    of tyUInt .. tyUInt64:
      dest.ensureKind(rkInt)
      let styp = srctyp.skipTypes(abstractRange)
        # skip distinct types(dest type could do this too if needed)
      case styp.kind
      of tyFloat .. tyFloat64:
        dest.intVal = int(src.floatVal)
      else:
...
template handleJmpBack() {.dirty.} =
  if c.loopIterations <= 0:
    if allowInfiniteLoops in c.features:
      ...
      stackTraceAux(c, tos, pc)
      globalError(
        c.config, c.debug[pc], errTooManyIterations % $c.config.maxLoopIterationsVM
      )
...
template maybeHandlePtr(node2: PNode, reg: TFullReg, isAssign2: bool): bool =
  ...
  if nfIsPtr in node.flags or (typ != nil and typ.kind == tyPtr):
    ...
    if not derefPtrToReg(node.intVal, typ2, reg, isAssign = isAssign2):
      ...
      stackTrace(
        c, tos, pc, "deref unsupported ptr type: " & $(typeToString(typ), typ.kind)
      )
    true
  else:
    false
  ...
  while true:
    ...
    when traceCode:
      ...
      echo "PC:$pc $opcode $ra $rb $rc" % [
        ...
        $pc,
        "opcode",
        alignLeft($c.code[pc].opcode, 15),
        "ra",
        regDescr("ra", ra),
        "rb",
        regDescr("rb", instr.regB),
        "rc",
        regDescr("rc", instr.regC),
      ]
    ...
    of opcCastIntToPtr:
      ...
      of rkNode:
        if regs[rb].node.typ.kind notin PtrLikeKinds:
          stackTrace(
            c, tos, pc, "opcCastIntToPtr: regs[rb].node.typ: " & $regs[rb].node.typ.kind
          )
        node2.intVal = regs[rb].node.intVal
      else:
        stackTrace(c, tos, pc, "opcCastIntToPtr: regs[rb].kind: " & $regs[rb].kind)
      regs[ra].node = node2
    of opcAsgnComplex:
    ...
    of opcLdObj:
      ...
      of nkTupleConstr:
        let n =
          if src.typ != nil and tfTriggersCompileTime in src.typ.flags:
            src[rc]
          else:
            src[rc].skipColon
        regs[ra].node = n
    ...
    of opcAddrNode:
      ...
      of rkNodeAddr: # bug #14339
        ...
        stackTrace(
          c, tos, pc, "limited VM support for 'addr', got kind: " & $regs[rb].kind
        )
    of opcLdDeref:
      # a = b[]
      let ra = instr.regA
    ...
    of opcWrDeref:
      ...
      of rkNodeAddr:
        ...
        if nAddr[] == nil:
          stackTrace(c, tos, pc, "opcWrDeref internal error")
          # refs bug #16613
        if (nfIsRef notin nAddr[].flags and nfIsRef notin n.flags):
          nAddr[][] = n[]
        else:
          nAddr[] = n
      of rkRegisterAddr:
        regs[ra].regAddr[] = regs[rc]
    ...
    of opcDivInt:
      decodeBC(rkInt)
      if regs[rc].intVal == 0:
        stackTrace(c, tos, pc, errConstantDivisionByZero)
      else:
    ...
    of opcEqRef:
      ...
      if rbReg.kind == rkNodeAddr:
        ...
        if nb.kind != nc.kind:
          discard
        elif (nb == nc) or (nb.kind == nkNilLit):
          ret = true
          # intentional
        elif nb.kind in {nkSym, nkTupleConstr, nkClosure} and nb.typ != nil and
            nb.typ.kind == tyProc and sameConstant(nb, nc):
          ret = true
          # this also takes care of procvar's, represented as nkTupleConstr, e.g. (nil, nil)
        elif nb.kind == nkIntLit and nc.kind == nkIntLit and nb.intVal == nc.intVal:
          # TODO: nkPtrLit
          let tb = nb.getTyp
          let tc = nc.getTyp
    ...
    of opcEqNimNode:
      decodeBC(rkInt)
      regs[ra].intVal = ord(
        exprStructuralEquivalent(regs[rb].node, regs[rc].node, strictSymEquality = true)
      )
    of opcSameNodeType:
      decodeBC(rkInt)
      regs[ra].intVal = ord(
        regs[rb].node.typ.sameTypeOrNil(
          regs[rc].node.typ, {ExactTypeDescValues, ExactGenericParams}
        )
      )
    ...
    of opcLtSet:
      ...
      let b = regs[rc].node
      regs[ra].intVal =
        ord(containsSets(c.config, a, b) and not equalSets(c.config, a, b))
    of opcMulSet:
      decodeBC(rkNode)
      createSet(regs[ra])
      move(
        ...
        regs[ra].node =
          if a.sym.ast.isNil:
            ...
            for i in 0 ..< a.sym.ast.len:
              ast[i] = a.sym.ast[i]
            ast[bodyPos] =
              transformBody(c.graph, c.idgen, a.sym, useCache, force = true)
            ast.copyTree()
    of opcSymOwner:
      decodeB(rkNode)
      let a = regs[rb].node
      if a.kind == nkSym:
        regs[ra].node =
          if a.sym.owner.isNil:
            newNode(nkNilLit)
          else:
            newSymNode(a.sym.skipGenericOwner)
        regs[ra].node.flags.incl nfIsRef
      else:
        stackTrace(c, tos, pc, "node is not a symbol")
    of opcSymIsInstantiationOf:
      ...
      let a = regs[rb].node
      let b = regs[rc].node
      if a.kind == nkSym and a.sym.kind in skProcKinds and b.kind == nkSym and
          b.sym.kind in skProcKinds:
        regs[ra].intVal =
          if sfFromGeneric in a.sym.flags and a.sym.owner == b.sym: 1 else: 0
      else:
        stackTrace(c, tos, pc, "node is not a proc symbol")
    of opcEcho:
      let rb = instr.regB
      template fn(s) =
        msgWriteln(c.config, s, {msgStdout, msgNoUnitSep})
    ...
    of opcParseFloat:
      ...
      of nkTupleConstr:
        ...
        case data.kind
        of nkStrKinds:
          regs[ra].intVal = parseBiggestFloat(
            data.strVal.toOpenArray(int left, int right), rcAddr.floatVal
          )
        of nkBracket:
          ...
          internalError(
            c.config, c.debug[pc], "opcParseFloat: Incorrectly created openarray"
          )
      else:
    ...
    of opcRangeChck:
      ...
      let rc = instr.regC
      if not (
        leValueConv(regs[rb].regToNode, regs[ra].regToNode) and
    ...
    of opcIndCall, opcIndCallAsgn:
      ...
      if prc.offset < -1:
        ...
        if compiletimeFFI notin c.config.features:
          globalError(
            c.config, c.debug[pc], "VM not allowed to do FFI, see `compiletimeFFI`"
          )
        ...
        when hasFFI:
          if prc.position - 1 < 0:
            globalError(
              c.config, c.debug[pc], "VM call invalid: prc.position: " & $prc.position
            )
          ...
          let newValue = callForeignFunction(
            c.config, prcValue, prc.typ, slots2, rb + 1, rc - 1, c.debug[pc]
          )
        ...
        let genSymOwner =
          if tos.next != nil and tos.next.prc != nil: tos.next.prc else: c.module
        ...
        for i in 1 .. rc - 1:
          ...
          macroCall.add(node)
        var a = evalTemplate(
          macroCall, prc, genSymOwner, c.config, c.cache, c.templInstCounter, c.idgen
        )
        if a.kind == nkStmtList and a.len == 1:
          a = a[0]
        a.recSetFlagIsRef
        ensureKind(rkNode)
        regs[ra].node = a
    ...
    of opcRepr:
      decodeB(rkNode)
      createStr regs[ra]
      regs[ra].node.strVal = renderTree(
        regs[rb].regToNode,
        {renderNoComments, renderDocComments, renderNonExportedFields},
      )
    of opcQuit:
    ...
    of opcSetLenSeq:
      ...
      if regs[ra].node.isNil:
        ...
        c.setLenSeq(regs[ra].node, newLen, c.debug[pc])
    of opcNarrowS:
      decodeB(rkInt)
    ...
    of opcNSetChild:
      ...
      if nfSem in dest.flags and
          allowSemcheckedAstModification notin c.config.legacyFeatures:
    ...
    of opcNGetType:
      ...
      of 0:
        ...
        if regs[rb].kind == rkNode and regs[rb].node.typ != nil:
          regs[ra].node =
            opMapTypeToAst(c.cache, regs[rb].node.typ, c.debug[pc], c.idgen)
        elif regs[rb].kind == rkNode and regs[rb].node.kind == nkSym and
            regs[rb].node.sym.typ != nil:
          regs[ra].node =
            opMapTypeToAst(c.cache, regs[rb].node.sym.typ, c.debug[pc], c.idgen)
        else:
      ...
      of 2:
        ...
        if regs[rb].kind == rkNode and regs[rb].node.typ != nil:
          regs[ra].node =
            opMapTypeInstToAst(c.cache, regs[rb].node.typ, c.debug[pc], c.idgen)
        elif regs[rb].kind == rkNode and regs[rb].node.kind == nkSym and
            regs[rb].node.sym.typ != nil:
          regs[ra].node =
            opMapTypeInstToAst(c.cache, regs[rb].node.sym.typ, c.debug[pc], c.idgen)
        else:
          stackTrace(c, tos, pc, "node has no type")
      else:
        # getTypeImpl opcode:
        ...
        if regs[rb].kind == rkNode and regs[rb].node.typ != nil:
          regs[ra].node =
            opMapTypeImplToAst(c.cache, regs[rb].node.typ, c.debug[pc], c.idgen)
        elif regs[rb].kind == rkNode and regs[rb].node.kind == nkSym and
            regs[rb].node.sym.typ != nil:
          regs[ra].node =
            opMapTypeImplToAst(c.cache, regs[rb].node.sym.typ, c.debug[pc], c.idgen)
        else:
          stackTrace(c, tos, pc, "node has no type")
    of opcNGetSize:
      decodeBImm(rkInt)
    ...
    of opcSlurp:
      ...
      createStr regs[ra]
      regs[ra].node.strVal =
        opSlurp(regs[rb].node.strVal, c.debug[pc], c.module, c.config)
    of opcGorge:
      decodeBC(rkNode)
      inc pc
    ...
    of opcCallSite:
      ...
      if c.callsite != nil:
        ...
        stackTrace(c, tos, pc, errFieldXNotFound & "callsite")
    of opcNGetLineInfo:
      decodeBImm(rkNode)
      let n = regs[rb].node
      case imm
    ...
    of opcConv:
      ...
      if opConv(c, regs[ra], regs[rb], desttyp, srctyp):
        stackTrace(
          ...
          tos,
          pc,
          errIllegalConvFromXtoY % [typeToString(srctyp), typeToString(desttyp)],
        )
    of opcCast:
      let rb = instr.regB
      inc pc
    ...
    of opcNSetIntVal:
      decodeB(rkNode)
      var dest = regs[ra].node
      if dest.kind in {nkCharLit .. nkUInt64Lit} and regs[rb].kind in {rkInt}:
    ...
    of opcNSetFloatVal:
      decodeB(rkNode)
      var dest = regs[ra].node
      if dest.kind in {nkFloatLit .. nkFloat64Lit} and regs[rb].kind in {rkFloat}:
    ...
    of opcNSetStrVal:
      decodeB(rkNode)
      var dest = regs[ra].node
      if dest.kind in {nkStrLit .. nkTripleStrLit} and regs[rb].kind in {rkNode}:
    ...
    of opcNNewNimNode:
      ...
      if k < 0 or k > ord(high(TNodeKind)):
        internalError(
          c.config, c.debug[pc], "request to create a NimNode of invalid kind"
        )
      let cc = regs[rc].node
    ...
    of opcGenSym:
      ...
      var sym = newSym(
        k.TSymKind, getIdent(c.cache, name), c.idgen, c.module.owner, c.debug[pc]
      )
    ...
    of opcNctNext:
      ...
      if contains(g.cacheTables, destKey):
        ...
        regs[ra].node = newTree(
          nkTupleConstr, newStrNode(k, c.debug[pc]), v, newIntNode(nkIntLit, nextIndex)
        )
      else:
        stackTrace(c, tos, pc, "key does not exist: " & destKey)
    of opcTypeTrait:
      ...
      while typ.kind == tyTypeDesc and typ.len > 0:
        typ = typ[0]
      createStr regs[ra]
  ...
  if sym.kind in routineKinds:
    if sym.typ.len - 1 != args.len:
      localError(
        c.config,
        sym.info,
        "NimScript: expected $# arguments, but got $#" % [$(sym.typ.len - 1), $args.len],
      )
    else:
    ...
    localError(
      c.config, sym.info, "NimScript: attempt to call non-routine: " & sym.name.s
    )
...
proc evalStaticExpr*(
    module: PSym, idgen: IdGenerator, g: ModuleGraph, e: PNode, prc: PSym
): PNode =
...
proc prepareVMValue(arg: PNode): PNode =
  ...
  if arg.kind == nkExprColonExpr and arg[0].typ != nil and arg[0].typ.sym != nil and
      arg[0].typ.sym.magic == mPNimrodNode:
...
proc setupMacroParam(x: PNode, typ: PType): TFullReg =
  ...
  of tyStatic:
    ...
    var n = x
    if n.kind in {nkHiddenSubConv, nkHiddenStdConv}:
      n = n[1]
...
proc evalMacroCall*(
  ...
  if sym.typ.len > n.safeLen and sym.typ.len > 1:
    globalError(
      g.config,
      n.info,
      "in call '$#' got $#, but expected $# argument(s)" %
  ...
  result = rawExecute(c, start, tos).regToNode
  if result.info.line < 0:
    result.info = n.info
  if cyclicTree(result):
    globalError(c.config, n.info, "macro produced a cyclic tree")
  dec(g.config.evalMacroCounter)
  c.callsite = nil
  c.mode = oldMode
