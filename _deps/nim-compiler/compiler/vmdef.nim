...
type
  ...
  TEvalMode* = enum ## reason for evaluation
    ...
    emOptimize
      ## evaluate for optimization purposes (same as
      ## emConst?)
    emStaticExpr
      ## evaluate for enforced compile time eval
      ## ('static' context)
  ...
  TSlotKind* = enum
    # We try to re-use slots in a smart way to
  ...
  TFullReg* = object
    # with a custom mark proc, we could use the same
  ...
  TCtx* = object of TPassContext # code gen context
    code*: seq[TInstr]
    debug*: seq[TLineInfo]
      # line info for every instruction; kept separate
  ...
  TStackFrame* {.acyclic.} = object
    ...
    slots*: seq[TFullReg]
      # parameters passed to the proc + locals;
...
proc newCtx*(
    module: PSym, cache: IdentCache, g: ModuleGraph, idgen: IdGenerator
): PCtx =
  PCtx(
    code: @[],
    debug: @[],
    ...
    callbacks: @[],
    callbackIndex: initTable[string, int](),
    errorFlag: "",
    cache: cache,
    config: g.config,
    graph: g,
...
proc registerCallback*(
    c: PCtx, name: string, callback: VmCallback
): int {.discardable.} =
...
template opcode*(x: TInstr): TOpcode =
  TOpcode(x.TInstrType shr regOShift and regOMask)

template regA*(x: TInstr): TRegister =
  TRegister(x.TInstrType shr regAShift and regAMask)

template regB*(x: TInstr): TRegister =
  TRegister(x.TInstrType shr regBShift and regBMask)

template regC*(x: TInstr): TRegister =
  TRegister(x.TInstrType shr regCShift and regCMask)

template regBx*(x: TInstr): int =
  (x.TInstrType shr regBxShift and regBxMask).int
