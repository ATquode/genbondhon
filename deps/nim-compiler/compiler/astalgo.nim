...
import ast, hashes, intsets, options, lineinfos, ropes, idents, rodutils, msgs
...
proc symToYamlAux(
    conf: ConfigRef, n: PSym, marker: var IntSet, indent: int, maxRecDepth: int
): Rope =
  if n == nil:
    result = rope("null")
...
proc typeToYamlAux(
    conf: ConfigRef, n: PType, marker: var IntSet, indent: int, maxRecDepth: int
): Rope =
  var sonsRope: Rope
  if n == nil:
    sonsRope = rope("null")
    ...
    sonsRope =
      "\"$1 @$2\"" % [rope($n.kind), rope(strutils.toHex(cast[int](n), sizeof(n) * 2))]
  else:
    if n.len > 0:
      sonsRope = rope("[")
      for i in 0 ..< n.len:
...
proc treeToYamlAux(
    conf: ConfigRef, n: PNode, marker: var IntSet, indent: int, maxRecDepth: int
): Rope =
  if n == nil:
    result = rope("null")
  else:
...
proc isAddrNode*(n: PNode): bool =
  ...
  of nkAddr, nkHiddenAddr:
    true
  of nkCallKinds:
    if n[0].kind == nkSym and n[0].sym.magic == mAddr: true else: false
  else:
    false
