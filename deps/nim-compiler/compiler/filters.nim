...
import llstream, idents, strutils, ast, msgs, options, renderer, pathutils

proc invalidPragma(conf: ConfigRef, n: PNode) =
  localError(conf, n.info, "'$1' not allowed here" % renderTree(n, {renderNoComments}))

proc getArg(conf: ConfigRef, n: PNode, name: string, pos: int): PNode =
  result = nil
  if n.kind in {nkEmpty .. nkNilLit}:
    return
  for i in 1 ..< n.len:
    if n[i].kind == nkExprEqExpr:
      if n[i][0].kind != nkIdent:
...
proc charArg*(conf: ConfigRef, n: PNode, name: string, pos: int, default: char): char =
  ...
  if x == nil:
    result = default
  elif x.kind == nkCharLit:
    result = chr(int(x.intVal))
  else:
    invalidPragma(conf, n)

proc strArg*(
    conf: ConfigRef, n: PNode, name: string, pos: int, default: string
): string =
  var x = getArg(conf, n, name, pos)
  if x == nil:
    result = default
  elif x.kind in {nkStrLit .. nkTripleStrLit}:
    result = x.strVal
  else:
    invalidPragma(conf, n)
...
proc boolArg*(conf: ConfigRef, n: PNode, name: string, pos: int, default: bool): bool =
  ...
  if x == nil:
    result = default
  elif x.kind == nkIdent and cmpIgnoreStyle(x.ident.s, "true") == 0:
    result = true
  elif x.kind == nkIdent and cmpIgnoreStyle(x.ident.s, "false") == 0:
    result = false
  else:
    invalidPragma(conf, n)

proc filterStrip*(
    conf: ConfigRef, stdin: PLLStream, filename: AbsoluteFile, call: PNode
): PLLStream =
