...
import
  ".."/[
    ast, options, lineinfos, modulegraphs, cgendata, cgen, pathutils, extccomp, msgs,
    modulepaths,
  ]

import packed_ast, ic, dce, rodfiles

proc unpackTree(g: ModuleGraph, thisModule: int, tree: PackedTree, n: NodePos): PNode =
...
proc addFileToLink(config: ConfigRef, m: PSym) =
  ...
  let ext =
    if config.backend == backendCpp:
      ".nim.cpp"
    elif config.backend == backendObjc:
      ".nim.m"
    else:
      ".nim.c"
  let cfile = changeFileExt(
    completeCfilePath(config, mangleModuleName(config, filename).AbsoluteFile), ext
  )
  let objFile = completeCfilePath(config, toObjFile(config, cfile))
  if fileExists(objFile):
    var cf =
      Cfile(nimname: m.name.s, cname: cfile, obj: objFile, flags: {CfileFlag.Cached})
...
template prepare() {.dirty.} =
  let asymFile =
    toRodFile(config, AbsoluteFile toFullPath(config, position.FileIndex), ".alivesyms")
  var s = newSeqOfCap[int32](alive[position].len)
  for a in items(alive[position]):
    s.add int32(a)
  sort(s)
...
proc aliveSymsChanged(config: ConfigRef, position: int, alive: AliveSyms): bool =
  ...
  if f2.err == ok and oldData == s:
    ...
    when defined(debugDce):
      ...
      echo "set of live symbols changed ",
        asymFile.changeFileExt("rod"), " ", position, " ", f2.err
      echo "in old but not in new ",
        oldAsSet.difference(newAsSet), " number of entries in old ", oldAsSet.len
      echo "in new but not in old ",
        newAsSet.difference(oldAsSet), " number of entries in new ", newAsSet.len
  ...
  of loaded:
    if g.packed[i].loadedButAliveSetChanged:
      ...
      if g.backend == nil:
        g.backend = cgendata.newModuleList(g)
      registerInitProcs(
        BModuleList(g.backend), g.packed[i].module, g.packed[i].fromDisk.backendFlags
      )
...
proc generateCode*(g: ModuleGraph) =
  ...
  when false:
    for i in 0 .. high(g.packed):
      echo i,
        " is of status ", g.packed[i].status, " ", toFullPath(g.config, FileIndex(i))
