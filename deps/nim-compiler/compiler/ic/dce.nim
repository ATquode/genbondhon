...
proc isExportedToC(c: var AliveContext, g: PackedModuleGraph, symId: int32): bool =
  ...
  if sfCompileTime notin flags:
    if ({sfExportc, sfCompilerProc} * flags != {}) or (symPtr.kind == skMethod):
      result = true
      # XXX: This used to be a condition to:
      #  (sfExportc in prc.flags and lfExportLib in prc.loc.flags) or
    if sfCompilerProc in flags:
      c.compilerProcs[g[c.thisModule].fromDisk.strings[symPtr.name]] =
        (c.thisModule, symId)

template isNotGeneric(n: NodePos): bool =
  ithSon(tree, n, genericParamsPos).kind == nkEmpty
    ...
    when false:
      ...
      if nid != LitId(0):
        ...
        if name in ["nimFrame", "callDepthLimitReached"]:
          echo "I was called! ",
            name, " body exists: ", body != emptyNodeId, " ", module, " ", item
...
proc loadTypeKind(
    t: PackedItemId, c: AliveContext, g: PackedModuleGraph, toSkip: set[TTypeKind]
): TTypeKind =
  ...
  while result in toSkip:
    t2 = translateId(
      g[t2.module].fromDisk.types[t2.item].types[^1], g, t2.module, c.decoder.config
    )
    result = t2.kind

proc rangeCheckAnalysis(
    c: var AliveContext, g: PackedModuleGraph, tree: PackedTree, n: NodePos
) =
...
proc aliveCode(
  ...
  of nkMacroDef, nkTemplateDef, nkTypeSection, nkTypeOfExpr, nkCommentStmt,
      nkIncludeStmt, nkImportStmt, nkImportExceptStmt, nkExportStmt, nkExportExceptStmt,
...
proc computeAliveSyms*(g: PackedModuleGraph, conf: ConfigRef): AliveSyms =
  ...
  var c = AliveContext(
    stack: @[],
    decoder: PackedDecoder(config: conf),
    thisModule: -1,
    alive: newSeq[IntSet](g.len),
