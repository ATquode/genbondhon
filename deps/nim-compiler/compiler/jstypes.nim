...
proc genObjectFields(p: PProc, typ: PType, n: PNode): Rope =
  ...
  of nkRecCase:
    if (n[0].kind != nkSym):
      internalError(p.config, n.info, "genObjectFields")
    field = n[0].sym
    s = genTypeInfo(p, field.typ)
    for i in 1 ..< n.len:
      b = n[i] # branch
      ...
      of nkOfBranch:
        ...
        for j in 0 ..< b.len - 1:
          if u != "":
            u.add(", ")
          if b[j].kind == nkRange:
            u.addf("[$1, $2]", [rope(getOrdValue(b[j][0])), rope(getOrdValue(b[j][1]))])
          else:
      ...
      if result != "":
        result.add(", \L")
      result.addf("[setConstr($1), $2]", [u, genObjectFields(p, typ, lastSon(b))])
    result =
...
proc genObjectInfo(p: PProc, typ: PType, name: Rope) =
  ...
  p.g.typeInfo.addf("var NNI$1 = $2;$n", [rope(typ.id), genObjectFields(p, typ, typ.n)])
  p.g.typeInfo.addf("$1.node = NNI$2;$n", [name, rope(typ.id)])
  if (typ.kind == tyObject) and (typ[0] != nil):
    p.g.typeInfo.addf(
      "$1.base = $2;$n", [name, genTypeInfo(p, typ[0].skipTypes(skipPtrs))]
    )
...
proc genTupleInfo(p: PProc, typ: PType, name: Rope) =
  ...
  prepend(p.g.typeInfo, s)
  p.g.typeInfo.addf("var NNI$1 = $2;$n", [rope(typ.id), genTupleFields(p, typ)])
  p.g.typeInfo.addf("$1.node = NNI$2;$n", [name, rope(typ.id)])

proc genEnumInfo(p: PProc, typ: PType, name: Rope) =
  var s: Rope = ""
  for i in 0 ..< typ.n.len:
    if (typ.n[i].kind != nkSym):
      internalError(p.config, typ.n.info, "genEnumInfo")
    let field = typ.n[i].sym
    if i > 0:
      s.add(", \L")
    let extName = if field.ast == nil: field.name.s else: field.ast.strVal
    s.addf(
      "\"$1\": {kind: 1, offset: $1, typ: $2, name: $3, len: 0, sons: null}",
      [rope(field.position), name, makeJSString(extName)],
    )
  var n =
    (
      "var NNI$1 = {kind: 2, offset: 0, typ: null, " &
  ...
  p.g.typeInfo.addf("$1.node = NNI$2;$n", [name, rope(typ.id)])
  if typ[0] != nil:
    p.g.typeInfo.addf("$1.base = $2;$n", [name, genTypeInfo(p, typ[0])])
...
proc genTypeInfo(p: PProc, typ: PType): Rope =
  ...
  of tyVar, tyLent, tyRef, tyPtr, tySequence, tyRange, tySet, tyOpenArray:
    var s =
      ...
      [result, rope(ord(t.kind))]
    prepend(p.g.typeInfo, s)
    p.g.typeInfo.addf("$1.base = $2;$n", [result, genTypeInfo(p, t.lastSon)])
  of tyArray:
    var s =
      "var $1 = {size: 0, kind: $2, base: null, node: null, finalizer: null};$n" %
      [result, rope(ord(t.kind))]
    prepend(p.g.typeInfo, s)
    p.g.typeInfo.addf("$1.base = $2;$n", [result, genTypeInfo(p, t[1])])
  of tyEnum:
  ...
  of tyStatic:
    if t.n != nil:
      ...
      internalError(p.config, "genTypeInfo(" & $t.kind & ')')
  else:
    internalError(p.config, "genTypeInfo(" & $t.kind & ')')
