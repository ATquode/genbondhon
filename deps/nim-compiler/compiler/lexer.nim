...
import
  hashes, options, msgs, strutils, platform, idents, nimlexbase, llstream, wordrecg,
  lineinfos, pathutils, parseutils
...
type
  TokType* = enum
    ...
    tkOpr
    tkComment
    tkAccent = "`" # these are fake tokens used by renderer.nim
    tkSpaces
    tkInfixOpr
    tkPrefixOpr
    tkPostfixOpr
...
const
  weakTokens = {
    tkComma, tkSemiColon, tkColon, tkParRi, tkParDotRi, tkBracketRi, tkBracketDotRi,
    tkCurlyRi,
  }
    # \
...
template tokenEnd(tok, pos) {.dirty.} =
  when defined(nimsuggest):
    ...
    if L.fileIdx == L.config.m.trackPos.fileIndex and
        L.config.m.trackPos.col in colA .. colB and
        L.lineNumber == L.config.m.trackPos.line.int and
        L.config.ideCmd in {ideSug, ideCon}:
  ...
  if L.buf[postPos] in {'\'', 'f', 'F', 'd', 'D', 'i', 'I', 'u', 'U'}:
    ...
    if L.buf[postPos] in SymChars:
      ...
      while true:
        ...
        inc postPos
        if L.buf[postPos] notin SymChars + {'_'}:
          break
  ...
  if result.tokType != tkCustomLit:
    ...
    try:
      if (L.buf[pos] == '0') and (L.buf[pos + 1] in baseCodeChars):
        ...
        of tkInt8Lit:
          setNumber result.iNumber, ashr(xi shl 56, 56)
        of tkInt16Lit:
          setNumber result.iNumber, ashr(xi shl 48, 48)
        of tkInt32Lit:
          setNumber result.iNumber, ashr(xi shl 32, 32)
        of tkUIntLit, tkUInt64Lit:
          setNumber result.iNumber, xi
        of tkUInt8Lit:
          setNumber result.iNumber, xi and 0xff
        of tkUInt16Lit:
          setNumber result.iNumber, xi and 0xffff
        of tkUInt32Lit:
          setNumber result.iNumber, xi and 0xffffffff
        of tkFloat32Lit:
...
proc getString(L: var Lexer, tok: var Token, mode: StringMode) =
  ...
  if L.buf[pos] == '\"' and L.buf[pos + 1] == '\"':
    ...
    while true:
      case L.buf[pos]
      of '\"':
        if L.buf[pos + 1] == '\"' and L.buf[pos + 2] == '\"' and L.buf[pos + 3] != '\"':
  ...
  of '\226':
    if buf[pos + 1] == '\136':
      ...
      if buf[pos + 2] == '\147':
        ...
        result = 3.m
        # ⊡
    elif buf[pos + 1] == '\152' and buf[pos + 2] == '\133':
      result = 3.m
      # ★
  of '\194':
    if buf[pos + 1] == '\177':
...
proc endOperator(L: var Lexer, tok: var Token, pos: int, hash: Hash) {.inline.} =
  ...
  if (tok.ident.id < oprLow) or (tok.ident.id > oprHigh):
    ...
    tok.tokType = TokType(tok.ident.id - oprLow + ord(tkColon))
  L.bufpos = pos
...
proc getPrecedence*(tok: Token): int =
  ...
  of tkDotDot:
    result = 6
  of tkIn, tkNotin, tkIs, tkIsnot, tkOf, tkAs, tkFrom:
    result = 5
  of tkAnd:
    result = 4
...
proc skipMultiLineComment(L: var Lexer, tok: var Token, start: int, isDoc: bool) =
  var pos = start
  var toStrip = 0
  tokenBegin(tok, pos)
  ...
  if c in SymStartChars - {'r', 'R'} - UnicodeOperatorStartChars:
    ...
    of '(':
      ...
      if L.buf[L.bufpos] == '.' and L.buf[L.bufpos + 1] != '.':
        ...
        when defined(nimsuggest):
          if L.fileIdx == L.config.m.trackPos.fileIndex and
              tok.col < L.config.m.trackPos.col and
    ...
    of '.':
      when defined(nimsuggest):
        if L.fileIdx == L.config.m.trackPos.fileIndex and
            tok.col + 1 == L.config.m.trackPos.col and
    ...
    of '0' .. '9':
      ...
      if c in SymChars + {'_'}:
        if c in UnicodeOperatorStartChars and unicodeOprLen(L.buf, L.bufpos)[0] != 0:
          discard
        else:
          lexMessage(
            L, errGenerated,
