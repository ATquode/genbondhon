...
import strutils, options, ast, msgs, idents, renderer, types, lowerings, lineinfos
...
proc interestingVar(s: PSym): bool {.inline.} =
  result =
    s.kind in {skVar, skLet, skTemp, skForVar, skResult} and sfGlobal notin s.flags
...
proc liftLocals(n: PNode, i: int, c: var Ctx) =
  ...
  of procDefs, nkTypeSection, nkMixinStmt, nkBindStmt:
    discard
  else:
...
proc lookupParam(params, dest: PNode): PSym =
  if dest.kind != nkIdent:
    return nil
  for i in 1 ..< params.len:
    if params[i].kind == nkSym and params[i].sym.name.id == dest.ident.id:
      return params[i].sym

proc liftLocalsIfRequested*(
    prc: PSym, n: PNode, cache: IdentCache, conf: ConfigRef, idgen: IdGenerator
): PNode =
  ...
  if partialParam.isNil:
    localError(
      conf, liftDest.info, "'$1' is not a parameter of '$2'" % [$liftDest, prc.name.s]
    )
    return n
  ...
  if objType.kind != tyObject or tfPartial notin objType.flags:
    localError(
      conf,
      liftDest.info,
      "parameter '$1' is not a pointer to a partial object" % $liftDest,
    )
    return n
