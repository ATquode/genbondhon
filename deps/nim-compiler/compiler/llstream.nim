...
const hasRstdin =
  (defined(nimUseLinenoise) or defined(useLinenoise) or defined(useGnuReadline)) and
  not defined(windows)
...
proc llStreamOpen*(filename: AbsoluteFile, mode: FileMode): PLLStream =
  new(result)
  result.kind = llsFile
  if not open(result.f, filename.string, mode):
    result = nil
...
proc llStreamOpenStdIn*(
    r: TLLRepl = llReadFromStdin, onPrompt: OnPrompt = nil
): PLLStream =
  new(result)
...
proc llReadFromStdin(s: PLLStream, buf: pointer, bufLen: int): int =
  ...
  while readLineFromStdin(if s.s.len == 0: ">>> " else: "... ", line):
    ...
    if not continueLine(line, (triples and 1) == 1):
      break
  inc(s.lineOffset)
