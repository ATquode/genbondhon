...
import
  ast, magicsys, msgs, options, idents, lexer, syntaxes, modulegraphs, lineinfos,
  pathutils
...
proc partialInitModule*(
    result: PSym, graph: ModuleGraph, fileIdx: FileIndex, filename: AbsoluteFile
) =
...
proc newModule*(graph: ModuleGraph, fileIdx: FileIndex): PSym =
  ...
  if not isNimIdentifier(result.name.s):
    rawMessage(
      graph.config,
      errGenerated,
      "invalid module name: '" & result.name.s &
