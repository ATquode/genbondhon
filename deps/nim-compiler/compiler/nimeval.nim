...
type Interpreter* = ref object ## Use Nim as an interpreter with this object
  mainModule: PSym
  graph: ModuleGraph
  scriptName: string
  idgen: IdGenerator
...
proc selectUniqueSymbol*(
    i: Interpreter, name: string, symKinds: set[TSymKind] = {skLet, skVar}
): PSym =
...
proc selectRoutine*(i: Interpreter, name: string): PSym =
  ...
  result = selectUniqueSymbol(
    i, name, {skTemplate, skMacro, skFunc, skMethod, skProc, skConverter}
  )
...
proc implementRoutine*(
    i: Interpreter, pkg, module, name: string, impl: proc(a: VmArgs) {.closure, gcsafe.}
) =
  assert i != nil
...
proc evalScript*(i: Interpreter, scriptStream: PLLStream = nil) =
  ...
  let s =
    if scriptStream != nil:
      ...
      llStreamOpen(findFile(i.graph.config, i.scriptName), fmRead)
...
proc createInterpreter*(
  ...
  result =
    Interpreter(mainModule: m, graph: graph, scriptName: scriptName, idgen: idgen)
...
proc registerErrorHook*(
    i: Interpreter,
    hook: proc(config: ConfigRef, info: TLineInfo, msg: string, severity: Severity) {.
      gcsafe
    .},
) =
  i.graph.config.structuredErrorHook = hook

proc runRepl*(r: TLLRepl, searchPaths: openArray[string], supportNimscript: bool) =
