...
import llstream, lexer, idents, strutils, ast, msgs, options, lineinfos, pathutils
...
proc simpleStmt(p: var Parser): PNode =
  ...
  of tkComment:
    result = newCommentStmt(p)
  else:
    if isExprStart(p):
      result = parseExprStmt(p)
    else:
      result = p.emptyNode
  if result.kind notin {nkEmpty, nkCommentStmt}:
    skipComment(p, result)
...
proc complexOrSimpleStmt(p: var Parser): PNode =
  ...
  of tkUsing:
    result = parseSection(p, nkUsingStmt, parseVariable)
  else:
    result = simpleStmt(p)
