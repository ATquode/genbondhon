...
when defined(nimPreviewSlimSystem):
  import std/[formatfloat, assertions]
...
proc isArrayConstr(n: PNode): bool {.inline.} =
  result = n.kind == nkBracket and n.typ.skipTypes(abstractInst).kind == tyArray
...
proc fitNodePostMatch(c: PContext, formal: PType, arg: PNode): PNode =
  ...
  if (x.kind == nkCurly and formal.kind == tySet and formal.base.kind != tyGenericParam) or
      (x.kind in {nkPar, nkTupleConstr}) and
...
proc fitNode(c: PContext, formal: PType, arg: PNode, info: TLineInfo): PNode =
  if arg.typ.isNil:
    localError(
      c.config,
      arg.info,
      "expression has no type: " & renderTree(arg, {renderNoComments}),
    )
    # error correction:
    result = copyTree(arg)
    result.typ = formal
...
proc fitNodeConsiderViewType(
    c: PContext, formal: PType, arg: PNode, info: TLineInfo
): PNode =
...
proc inferWithMetatype(
  c: PContext, formal: PType, arg: PNode, coerceDistincts = false
): PNode

template commonTypeBegin*(): PType =
  PType(kind: tyUntyped)
...
proc commonType*(c: PContext, x, y: PType): PType =
  ...
  result = x
  if a.kind in {tyUntyped, tyNil}:
    result = y
  elif b.kind in {tyUntyped, tyNil}:
    result = x
  elif a.kind == tyTyped:
    result = a
  elif b.kind == tyTyped:
  ...
  elif b.kind in {tyArray, tySet, tySequence} and a.kind == b.kind:
    # check for seq[empty] vs. seq[int]
    let idx = ord(b.kind == tyArray)
    if a[idx].kind == tyEmpty:
      return y
...
proc endsInNoReturn(n: PNode): bool =
  ...
  of nkTryStmt:
    ...
    result =
      it.kind in nkLastBlockStmts or
  ...
  if n.kind == nkSym:
    ...
    if result.kind notin {kind, skTemp}:
      localError(
        c.config,
        n.info,
        "cannot use symbol of kind '$1' as a '$2'" %
...
proc semIdentWithPragma(c: PContext, kind: TSymKind, n: PNode, allowed: TSymFlags): PSym

proc typeAllowedCheck(
    c: PContext,
    info: TLineInfo,
    typ: PType,
    kind: TSymKind,
  ...
  if t != nil:
    ...
    if t == typ:
      ...
      err =
        "invalid type: '$1' in this context: '$2' for $3" %
...
proc fixupTypeAfterEval(c: PContext, evaluated, eOrig: PNode): PNode =
  ...
  when true:
    if eOrig.typ.kind in {tyUntyped, tyTyped, tyTypeDesc}:
      ...
      if hasCycle(result):
        result = localErrorNode(
          c, eOrig, "the resulting AST is cyclic and cannot be processed further"
        )
      else:
    ...
    if result == nil:
      ...
      if eOrig.typ.skipTypes(abstractInst).kind == tySequence and isArrayConstr(arg):
        arg.typ = eOrig.typ
...
const errConstExprExpected = "constant expression expected"
...
proc semAfterMacroCall(
    c: PContext,
    call, macroResult: PNode,
    s: PSym,
    flags: TExprFlags,
    expectedType: PType = nil,
): PNode =
  ...
  if s.typ[0] == nil:
    ...
    var retType = s.typ[0]
    if retType.kind == tyTypeDesc and tfUnresolved in retType.flags and retType.len == 1:
    ...
    of tyTypeDesc:
      ...
      if s.ast[genericParamsPos] != nil and retType.isMetaType:
        ...
        retType = generateTypeInstance(c, paramTypes, macroResult.info, retType)
...
proc defaultFieldsForTheUninitialized(
  c: PContext, recNode: PNode, checkDefault: bool
): seq[PNode]
...
proc defaultFieldsForTuple(
    c: PContext, recNode: PNode, hasDefault: var bool, checkDefault: bool
): seq[PNode] =
  case recNode.kind
  of nkRecList:
...
proc defaultFieldsForTheUninitialized(
  ...
  of nkRecCase:
    ...
    if defaultValue == nil:
      ...
      if checkDefault:
        # don't add defaults when checking whether a case branch has default fields
        return
      defaultValue = newIntNode(
        nkIntLit, #[c.graph]#
        0,
      )
    ...
    result.add defaultFieldsForTheUninitialized(
      c, recNode[selectedBranch][^1], checkDefault
    )
  of nkSym:
    let field = recNode.sym
    let recType = recNode.typ.skipTypes(defaultFieldsSkipTypes)
  ...
  if aTypSkip.kind == tyObject:
    ...
    if child != nil:
      ...
      result = semExpr(
        ...
        newTree(
          nkCall,
          newSymNode(getSysSym(c.graph, a.info, "arrayWith"), a.info),
...
proc preparePContext*(graph: ModuleGraph, module: PSym, idgen: IdGenerator): PContext =
  ...
  if result.p != nil:
    internalError(graph.config, module.info, "sem.preparePContext")
...
proc recoverContext(c: PContext) =
  ...
  while getCurrOwner(c).kind != skModule:
    popOwner(c)
  while c.p != nil and c.p.owner.kind != skModule:
    c.p = c.p.next
...
proc reportUnusedModules(c: PContext) =
  for i in 0 .. high(c.unusedImports):
    if sfUsed notin c.unusedImports[i][0].flags:
      message(
        c.config, c.unusedImports[i][1], warnUnusedImportX, c.unusedImports[i][0].name.s
      )
