...
proc initCandidateSymbols(
  ...
  if result.len > 0:
    initCandidate(c, best, result[0].s, initialBinding, result[0].scope, diagnostics)
    initCandidate(c, alt, result[0].s, initialBinding, result[0].scope, diagnostics)
    best.state = csNoMatch

proc pickBestCandidate(
    c: PContext,
    headSymbol: PNode,
    n, orig: PNode,
    initialBinding: PNode,
  ...
  var syms = initCandidateSymbols(
    c, headSymbol, initialBinding, filter, best, alt, o, diagnosticsFlag
  )
  if len(syms) == 0:
    return
    ...
    if c.currentScope.symbols.counter == symCount:
      ...
      if z.state == csMatch:
        ...
        of csMatch:
          ...
          if cmp < 0:
            best = z
            # x is better than the best so far
          elif cmp == 0:
            alt = z
            # x is as good as the best so far
      elif errorsEnabled or z.diagnosticsEnabled:
        errors.add(
          CandidateError(
            sym: sym, firstMismatch: z.firstMismatch, diagnostics: z.diagnostics
          )
        )
    else:
...
proc presentFailedCandidates(
    c: PContext, n: PNode, errors: CandidateErrors
  ...
  if optShowAllMismatches notin c.config.globalOptions and
      verboseTypeMismatch in c.config.legacyFeatures:
    for err in errors:
  ...
  for err in errors:
    ...
    if err.sym.kind in routineKinds and err.sym.ast != nil:
      candidates.add(
        renderTree(err.sym.ast, {renderNoBody, renderNoComments, renderNoPragmas})
      )
    else:
    ...
    let nArg =
      if err.firstMismatch.arg < n.len:
        ...
        nil
    let nameParam =
      if err.firstMismatch.formal != nil: err.firstMismatch.formal.name.s else: ""
    if n.len > 1:
      if verboseTypeMismatch notin c.config.legacyFeatures:
  ...
  if skipped > 0:
    candidates.add(
      $skipped & " other mismatching symbols have been " &
...
proc describeParamList(
    c: PContext, n: PNode, startIdx = 1, prefer = preferName
): string =
  result = "Expression: " & $n
...
proc bracketNotFoundError(c: PContext, n: PNode) =
  ...
  while symx != nil:
    if symx.kind in routineKinds:
      errors.add(
        CandidateError(
          sym: symx, firstMismatch: MismatchInfo(), diagnostics: @[], enabled: false
        )
      )
    symx = nextOverloadIter(o, c, headSymbol)
  if errors.len == 0:
...
proc getMsgDiagnostic(c: PContext, flags: TExprFlags, n, f: PNode): string =
  ...
  if nfExplicitCall notin n.flags and {nfDotField, nfDotSetter} * n.flags != {}:
    ...
    if result.len == 0:
      result = errUndeclaredRoutine % ident
    else:
      result = errBadRoutine % [ident, result]

proc resolveOverloads(
    c: PContext,
    n, orig: PNode,
    filter: TSymKinds,
    flags: TExprFlags,
    ...
    if overloadsState == csEmpty and result.state == csEmpty:
      ...
      if nfExprCall in n.flags:
        localError(
          c.config,
          n.info,
          "expression '$1' cannot be called" % renderTree(n, {renderNoComments}),
        )
      else:
...
proc inferWithMetatype(
  ...
  if result != nil:
    ...
    result.typ = generateTypeInstance(
      c, m.bindings, arg.info, formal.skipTypes({tyCompositeTypeClass})
    )
  else:
...
proc updateDefaultParams(call: PNode) =
  ...
  for i in 1 ..< call.len:
    if nfDefaultParam in call[i].flags:
      ...
      if nfDefaultRefsParam in def.flags:
        call.flags.incl nfDefaultRefsParam
      call[i] = def
...
proc semResolvedCall(c: PContext, x: TCandidate, n: PNode, flags: TExprFlags): PNode =
...
proc semOverloadedCall(
    c: PContext, n, nOrig: PNode, filter: TSymKinds, flags: TExprFlags
): PNode =
  ...
  if a.kind == nkSym:
    ...
    if s.ast[genericParamsPos].safeLen != n.len - 1:
      ...
      localError(
        ...
        getCallLineInfo(n),
        errGenerated,
        "cannot instantiate: '" & renderTree(n) & "'; got " & $(n.len - 1) &
          " typeof(s) but expected " & $expected,
      )
      return n
    ...
    for i in 0 ..< a.len:
      ...
      if candidate.kind in {skProc, skMethod, skConverter, skFunc, skIterator}:
...
proc searchForBorrowProc(c: PContext, startScope: PScope, fn: PSym): PSym =
  ...
  for i in 1 ..< fn.typ.n.len:
    ...
    if t.kind == tyDistinct or param.typ.kind == tyDistinct:
      hasDistinct = true
    var x: PType
