...
import
  strutils, options, ast, trees, nimsets, platform, math, msgs, idents, renderer, types,
...
proc ordinalValToString*(a: PNode, g: ModuleGraph): string =
  ...
  of tyEnum:
    ...
    for i in 0 ..< n.len:
      if n[i].kind != nkSym:
        internalError(g.config, a.info, "ordinalValToString")
      var field = n[i].sym
      if field.position == x:
        if field.ast == nil:
          return field.name.s
    ...
    localError(
      g.config,
      a.info,
      "Cannot convert int literal to $1. The value is invalid." % [typeToString(t)],
    )
  else:
    result = $x
...
proc isIntRange(t: PType): bool {.inline.} =
  result = t.kind == tyRange and t[0].kind in {tyInt .. tyInt64, tyUInt8 .. tyUInt32}
...
proc evalOp(m: TMagic, n, a, b, c: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  case m
  of mOrd:
    result = newIntNodeT(getOrdValue(a), n, idgen, g)
  of mChr:
    result = newIntNodeT(getInt(a), n, idgen, g)
  of mUnaryMinusI, mUnaryMinusI64:
    result = foldUnarySub(getInt(a), n, idgen, g)
  of mUnaryMinusF64:
    result = newFloatNodeT(-getFloat(a), n, g)
  of mNot:
    result = newIntNodeT(One - getInt(a), n, idgen, g)
  of mCard:
    result = newIntNodeT(toInt128(nimsets.cardSet(g.config, a)), n, idgen, g)
  of mBitnotI:
    if n.typ.isUnsigned:
      result = newIntNodeT(
        bitnot(getInt(a)).maskBytes(int(getSize(g.config, n.typ))), n, idgen, g
      )
    else:
      result = newIntNodeT(bitnot(getInt(a)), n, idgen, g)
  of mLengthArray:
    result = newIntNodeT(lengthOrd(g.config, a.typ), n, idgen, g)
  ...
  of mAbsI:
    result = foldAbs(getInt(a), n, idgen, g)
  of mSucc:
    result = foldAdd(getOrdValue(a), getInt(b), n, idgen, g)
  of mPred:
    result = foldSub(getOrdValue(a), getInt(b), n, idgen, g)
  of mAddI:
    result = foldAdd(getInt(a), getInt(b), n, idgen, g)
  of mSubI:
    result = foldSub(getInt(a), getInt(b), n, idgen, g)
  of mMulI:
    result = foldMul(getInt(a), getInt(b), n, idgen, g)
  of mMinI:
    let argA = getInt(a)
    let argB = getInt(b)
  ...
  of mShlI:
    ...
    of tyInt8:
      result =
        newIntNodeT(toInt128(toInt8(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyInt16:
      result =
        newIntNodeT(toInt128(toInt16(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyInt32:
      result =
        newIntNodeT(toInt128(toInt32(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyInt64:
      result =
        newIntNodeT(toInt128(toInt64(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyInt:
    ...
    of tyUInt8:
      result =
        newIntNodeT(toInt128(toUInt8(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyUInt16:
      result =
        newIntNodeT(toInt128(toUInt16(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyUInt32:
      result =
        newIntNodeT(toInt128(toUInt32(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyUInt64:
      result =
        newIntNodeT(toInt128(toUInt64(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
    of tyUInt:
      if g.config.target.intSize == 4:
        result =
          newIntNodeT(toInt128(toUInt32(getInt(a)) shl toInt64(getInt(b))), n, idgen, g)
      ...
      internalError(g.config, n.info, "constant folding for shl")
  of mShrI:
    ...
    # representation.
    if 0'u64 < b:
      # do not cut off the sign extension, when there is
  ...
  of mAshrI:
    ...
    of tyInt8:
      result =
        newIntNodeT(toInt128(ashr(toInt8(getInt(a)), toInt8(getInt(b)))), n, idgen, g)
    of tyInt16:
      result =
        newIntNodeT(toInt128(ashr(toInt16(getInt(a)), toInt16(getInt(b)))), n, idgen, g)
    of tyInt32:
      result =
        newIntNodeT(toInt128(ashr(toInt32(getInt(a)), toInt32(getInt(b)))), n, idgen, g)
    of tyInt64, tyInt:
      result =
        newIntNodeT(toInt128(ashr(toInt64(getInt(a)), toInt64(getInt(b)))), n, idgen, g)
    else:
  ...
  of mDivF64:
    result = newFloatNodeT(getFloat(a) / getFloat(b), n, g)
  of mIsNil:
    result = newIntNodeT(toInt128(ord(a.kind == nkNilLit)), n, idgen, g)
  ...
  of mEqI, mEqB, mEqEnum, mEqCh:
    result = newIntNodeT(toInt128(ord(getOrdValue(a) == getOrdValue(b))), n, idgen, g)
  of mLtF64:
    result = newIntNodeT(toInt128(ord(getFloat(a) < getFloat(b))), n, idgen, g)
  of mLeF64:
    result = newIntNodeT(toInt128(ord(getFloat(a) <= getFloat(b))), n, idgen, g)
  of mEqF64:
    result = newIntNodeT(toInt128(ord(getFloat(a) == getFloat(b))), n, idgen, g)
  of mLtStr:
    result = newIntNodeT(toInt128(ord(getStr(a) < getStr(b))), n, idgen, g)
  of mLeStr:
    result = newIntNodeT(toInt128(ord(getStr(a) <= getStr(b))), n, idgen, g)
  of mEqStr:
    result = newIntNodeT(toInt128(ord(getStr(a) == getStr(b))), n, idgen, g)
  of mLtU:
    result = newIntNodeT(
      toInt128(ord(`<%`(toInt64(getOrdValue(a)), toInt64(getOrdValue(b))))), n, idgen, g
    )
  of mLeU:
    result = newIntNodeT(
      toInt128(ord(`<=%`(toInt64(getOrdValue(a)), toInt64(getOrdValue(b))))),
      n,
      idgen,
      g,
    )
  of mBitandI, mAnd:
    result = newIntNodeT(bitand(a.getInt, b.getInt), n, idgen, g)
  of mBitorI, mOr:
    result = newIntNodeT(bitor(getInt(a), getInt(b)), n, idgen, g)
  of mBitxorI, mXor:
    result = newIntNodeT(bitxor(getInt(a), getInt(b)), n, idgen, g)
  of mAddU:
  ...
  of mDivU:
    ...
    if argB != Zero:
      result = newIntNodeT(argA div argB, n, idgen, g)
  of mLeSet:
    result = newIntNodeT(toInt128(ord(containsSets(g.config, a, b))), n, idgen, g)
  of mEqSet:
    result = newIntNodeT(toInt128(ord(equalSets(g.config, a, b))), n, idgen, g)
  of mLtSet:
    result = newIntNodeT(
      toInt128(ord(containsSets(g.config, a, b) and not equalSets(g.config, a, b))),
      n,
      idgen,
      g,
    )
  ...
  of mMinusSet:
    ...
    result.info = n.info
  of mConStrStr:
    result = newStrNodeT(getStrOrChar(a) & getStrOrChar(b), n, g)
  of mInSet:
    result = newIntNodeT(toInt128(ord(inSet(a, b))), n, idgen, g)
  of mRepr:
    # BUGFIX: we cannot eval mRepr here for reasons that I forgot.
    discard
  of mIntToStr, mInt64ToStr:
    result = newStrNodeT($(getOrdValue(a)), n, g)
  of mBoolToStr:
    if getOrdValue(a) == 0:
      result = newStrNodeT("false", n, g)
    else:
      result = newStrNodeT("true", n, g)
  of mFloatToStr:
    result = newStrNodeT($getFloat(a), n, g)
  of mCStrToStr, mCharToStr:
    result = newStrNodeT(getStrOrChar(a), n, g)
  of mStrToStr:
    result = newStrNodeT(getStrOrChar(a), n, g)
  of mEnumToStr:
    result = newStrNodeT(ordinalValToString(a, g), n, g)
  of mArrToSeq:
    result = copyTree(a)
    result.typ = n.typ
  of mCompileOption:
    result = newIntNodeT(
      toInt128(ord(commands.testCompileOption(g.config, a.getStr, n.info))), n, idgen, g
    )
  of mCompileOptionArg:
    result = newIntNodeT(
      toInt128(ord(testCompileOptionArg(g.config, getStr(a), getStr(b), n.info))),
      n,
      idgen,
      g,
    )
  of mEqProc:
    result = newIntNodeT(
      toInt128(ord(exprStructuralEquivalent(a, b, strictSymEquality = true))),
      n,
      idgen,
      g,
    )
...
proc getConstIfExpr(c: PSym, n: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  for i in 0 ..< n.len:
    ...
    if it.len == 2:
      ...
      if result == nil:
        result = getConstExpr(c, it[0], idgen, g)
    else:
      internalError(g.config, it.info, "getConstIfExpr()")
...
proc leValueConv*(a, b: PNode): bool =
  ...
  of nkCharLit .. nkUInt64Lit:
    ...
    of nkFloatLit .. nkFloat128Lit:
      result = a.intVal <= round(b.floatVal).int
    else:
      result = false
      #internalError(a.info, "leValueConv")
  of nkFloatLit .. nkFloat128Lit:
    case b.kind
    of nkFloatLit .. nkFloat128Lit:
      result = a.floatVal <= b.floatVal
    of nkCharLit .. nkUInt64Lit:
      result = a.floatVal <= toFloat64(b.getInt)
    else:
      result = false
      # internalError(a.info, "leValueConv")
  else:
...
proc rangeCheck(n: PNode, value: Int128, g: ModuleGraph) =
  if value < firstOrd(g.config, n.typ) or value > lastOrd(g.config, n.typ):
    localError(
      g.config, n.info, "cannot convert " & $value & " to " & typeToString(n.typ)
    )
...
proc foldArrayAccess(m: PSym, n: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  of nkPar, nkTupleConstr:
    if idx >= 0 and idx < x.len:
      result = x.sons[idx]
      if result.kind == nkExprColonExpr:
        result = result[1]
    else:
...
proc foldFieldAccess(m: PSym, n: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  if n[1].kind != nkSym:
    return nil
  var x = getConstExpr(m, n[0], idgen, g)
  if x == nil or x.kind notin {nkObjConstr, nkPar, nkTupleConstr}:
    return
    ...
    of mIntDefine:
      try:
        ...
        localError(
          g.config,
          s.info,
          "{.intdefine.} const was set to an invalid integer: '" & str & "'",
        )
    of mStrDefine:
    ...
    of mBoolDefine:
      try:
        ...
        localError(
          g.config,
          s.info,
          "{.booldefine.} const was set to an invalid bool: '" & str & "'",
        )
    of mGenericDefine:
      let rawTyp = s.typ
      ...
      try:
        ...
        of tyEnum:
          ...
          for e in typ.n:
            if e.kind != nkSym:
              internalError(g.config, "foldDefine for enum")
            let es = e.sym
            let match =
              if es.ast.isNil:
                es.name.id == ident.id
          ...
          localError(
            g.config,
            s.info,
            "unsupported type $1 for define '$2'" % [name, typeToString(rawTyp)],
          )
...
proc getConstExpr(m: PSym, n: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  of nkSym:
    ...
    of skConst:
      case s.magic
      of mIsMainModule:
        result = newIntNodeT(toInt128(ord(sfMainModule in m.flags)), n, idgen, g)
      of mCompileDate:
        result = newStrNodeT(getDateStr(), n, g)
      of mCompileTime:
        result = newStrNodeT(getClockStr(), n, g)
      of mCpuEndian:
        result =
          newIntNodeT(toInt128(ord(CPU[g.config.target.targetCPU].endian)), n, idgen, g)
      of mHostOS:
        result =
          newStrNodeT(toLowerAscii(platform.OS[g.config.target.targetOS].name), n, g)
      of mHostCPU:
        result =
          newStrNodeT(platform.CPU[g.config.target.targetCPU].name.toLowerAscii, n, g)
      of mBuildOS:
        result =
          newStrNodeT(toLowerAscii(platform.OS[g.config.target.hostOS].name), n, g)
      of mBuildCPU:
        result =
          newStrNodeT(platform.CPU[g.config.target.hostCPU].name.toLowerAscii, n, g)
      of mAppType:
        result = getAppType(n, g)
  ...
  of nkCallKinds:
    if n[0].kind != nkSym:
      return
    var s = n[0].sym
    if s.kind != skProc and s.kind != skFunc:
      return
    try:
      case s.magic
      of mNone:
      ...
      of mHigh:
        if skipTypes(n[1].typ, abstractVar + {tyUserTypeClassInst}).kind notin
          ...
          if skipTypes(n[1].typ, abstractVarRange).kind in tyFloat .. tyFloat64:
            ...
            result = newIntNodeT(
              lastOrd(g.config, skipTypes(n[1].typ, abstractVar)), n, idgen, g
            )
        else:
  ...
  of nkCast:
    ...
    if n.typ != nil and n.typ.kind in NilableTypes:
      ...
      result.typ = n.typ
  of nkBracketExpr:
    result = foldArrayAccess(m, n, idgen, g)
  of nkDotExpr:
    result = foldFieldAccess(m, n, idgen, g)
  ...
  of nkStmtListExpr:
    var i = 0
    while i <= n.len - 2:
      if n[i].kind in {nkComesFrom, nkCommentStmt, nkEmpty}:
        i.inc
      else:
        break
    if i == n.len - 1:
