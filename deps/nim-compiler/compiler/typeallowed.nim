...
import intsets, ast, renderer, options, semdata, types
...
proc typeAllowedAux(
  ...
  if typ == nil:
    return nil
  if containsOrIncl(marker, typ.id):
    return nil
  ...
  of tyVar, tyLent:
    if kind in {skProc, skFunc, skConst} and (views notin c.features):
      ...
      (kind != skResult and views notin c.features) or
      (kind == skParam and {taIsCastable, taField} * flags == {})
    ):
      # lent cannot be used as parameters.
      ...
      case t2.kind
      of tyVar, tyLent:
        if taHeap notin flags:
          result = t2
          # ``var var`` is illegal on the heap
      of tyOpenArray:
        if (kind != skParam and views notin c.features) or taIsOpenArray in flags:
          result = t
        else:
          result = typeAllowedAux(marker, t2[0], kind, c, flags + {taIsOpenArray})
      of tyUncheckedArray:
        if kind != skParam and views notin c.features:
          result = t
        else:
          result = typeAllowedAux(marker, t2[0], kind, c, flags)
      of tySink:
        result = t
      else:
        if kind notin {skParam, skResult} and views notin c.features:
          result = t
        else:
          result = typeAllowedAux(marker, t2, kind, c, flags)
  of tyProc:
  ...
  of tyUntyped, tyTyped:
    if kind notin {skParam, skResult} or taNoUntyped in flags:
      result = t
  of tyIterable:
    if kind notin {skParam} or taNoUntyped in flags:
      result = t
      # tyIterable is only for templates and macros.
  of tyStatic:
    if kind notin {skParam}:
      result = t
  of tyVoid:
    if taField notin flags:
      result = t
  of tyTypeClasses:
    if tfGenericTypeParam in t.flags or taConcept in flags: #or taField notin flags:
      ...
      result = t
  of tyGenericBody, tyGenericParam, tyGenericInvocation, tyNone, tyForward, tyFromExpr:
    result = t
  of tyNil:
    if kind != skConst and kind != skParam:
      result = t
  ...
  of tyRange:
    if skipTypes(t[0], abstractInst - {tyTypeDesc}).kind notin
        {tyChar, tyEnum, tyInt .. tyFloat128, tyInt .. tyUInt64, tyRange}:
      result = t
  of tyOpenArray:
  ...
  of tySet:
    for i in 0 ..< t.len:
      ...
      if result != nil:
        break
  of tyObject, tyTuple:
    if kind in {skProc, skFunc, skConst} and t.kind == tyObject and t[0] != nil:
      result = t
    else:
...
proc typeAllowed*(
    t: PType, kind: TSymKind, c: PContext, flags: TTypeAllowedFlags = {}
): PType =
...
proc classifyViewTypeAux(marker: var IntSet, t: PType): ViewTypeKind =
  ...
  of tyGenericInst, tyDistinct, tyAlias, tyInferred, tySink, tyOwned, tyUncheckedArray,
      tySequence, tyArray, tyRef, tyStatic:
