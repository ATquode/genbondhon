...
proc setResult*(a: VmArgs, v: BiggestInt) =
  setX(rkInt, intVal)

proc setResult*(a: VmArgs, v: BiggestFloat) =
  setX(rkFloat, floatVal)
...
proc setResult*(a: VmArgs, v: AbsoluteDir) =
  setResult(a, v.string)

proc setResult*(a: VmArgs, v: seq[string]) =
  a.slots[a.ra].ensureKind(rkNode)
  var n = newNode(nkBracket)
  for x in v:
    n.add newStrNode(nkStrLit, x)
  a.slots[a.ra].node = n
...
proc getInt*(a: VmArgs, i: Natural): BiggestInt =
  getX(rkInt, intVal)

proc getBool*(a: VmArgs, i: Natural): bool =
  getInt(a, i) != 0

proc getFloat*(a: VmArgs, i: Natural): BiggestFloat =
  getX(rkFloat, floatVal)

proc getNode*(a: VmArgs, i: Natural): PNode =
  getX(rkNode, node)

proc getString*(a: VmArgs, i: Natural): string =
  getX(rkNode, node).strVal
