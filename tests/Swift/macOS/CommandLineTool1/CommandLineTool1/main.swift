...
print("\(l), \(m)")
let position = (1, 2)
let position3D = extendTo3D(point: position, zValue: 3)
print("\(position3D.0), \(position3D.1), \(position3D.2)")
let newPosition3D = translate3D(point: (1, 2, 3), zDistance: -2)
print("\(newPosition3D.0), \(newPosition3D.1), \(newPosition3D.2)")
let quaternion = inverseQuaternion(q: (1, -2, 2, 1))
print("\(quaternion.0), \(quaternion.1), \(quaternion.2), \(quaternion.3)")
let swappedRects = swapBoundingBoxes(rects: (1, 2, 3, 4, "order"))
print("\(swappedRects.0), \(swappedRects.1), \(swappedRects.2), \(swappedRects.3), \(swappedRects.4)")
